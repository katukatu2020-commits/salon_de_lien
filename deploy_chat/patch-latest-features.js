...
for (const name of fs.readdirSync(chunks)) {
  ...
  edit(file, s => {
    ...
    const safeWidget = widget
      .replace('return((0,b.useEffect)', '(0,b.useEffect)')
      .replace('},[])),(0,r.jsxs)', '},[]);return(0,r.jsxs)')
      .replace(']}))} ', ']})} ')
    s = s.replace(anchor, safeWidget + anchor)
...
edit(account, s => {
  ...
  return s.replace(end, form.replace(/\n\+/g, '\n'))
})
...
edit(detail, s => {
  ...
  return s.replace(anchor, block.replace(/\n\+/g, '\n') + anchor)
})
