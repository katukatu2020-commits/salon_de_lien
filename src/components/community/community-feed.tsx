        ...
        <article key={post.id} className="community-feed-card overflow-hidden rounded-[20px] border border-[#e8ded2] bg-white shadow-[0_12px_35px_rgba(47,42,37,0.07)]">
          ...
          <div className="community-feed-content p-4 sm:p-5">
              ...
              <div className="mt-3 flex items-start justify-between gap-3">
                ...
                {(post.canEdit || post.canDelete) ? (
                  <div className="flex shrink-0 items-center gap-1">
                    {post.canEdit ? (
                      <button type="button" onClick={() => { setEditingPostId(post.id); setPostDrafts((current) => ({ ...current, [post.id]: post.caption ?? "" })); closeDeleteConfirmation(); }} className="inline-flex h-11 w-11 items-center justify-center rounded-full text-[#6f6259] hover:bg-[#f6efe6]" aria-label="投稿を編集">
                        <Pencil className="h-4 w-4" />
                      </button>
                    ) : null}
                    {post.canDelete ? (
                      <button type="button" onClick={() => openDeleteConfirmation(`post:${post.id}`)} className="inline-flex h-11 w-11 items-center justify-center rounded-full text-[#9d4a43] hover:bg-[#fff0ee]" aria-label="投稿を削除">
            ...
            {confirmDeleteKey === `post:${post.id}` ? (
              <div className="mt-3 grid gap-3 rounded-2xl border border-[#efc6c1] bg-[#fff4f2] px-4 py-3">
                <p className="text-sm font-semibold text-[#8c3832]">この投稿をスタイル共有から削除しますか？</p>
                <label className="flex min-h-12 cursor-pointer items-center gap-3 rounded-xl border border-[#e5c9c3] bg-white px-3 py-2 text-sm font-semibold text-[#5b5149] focus-within:ring-4 focus-within:ring-[#e9c9be]/40">
                  <input type="checkbox" checked={confirmedDeleteKey === `post:${post.id}`} disabled={Boolean(pendingKey)} onChange={(event) => setConfirmedDeleteKey(event.target.checked ? `post:${post.id}` : null)} className="h-5 w-5 shrink-0 accent-[#9d443c]" />
                  <span>削除する内容を確認しました</span>
                </label>
                <div className="flex justify-end gap-2">
                  <button type="button" onClick={closeDeleteConfirmation} className="min-h-11 rounded-full border border-[#e4cfc9] bg-white px-4 text-sm font-semibold">戻る</button>
                  <button type="button" onClick={() => void deletePost(post.id)} disabled={confirmedDeleteKey !== `post:${post.id}` || Boolean(pendingKey)} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#9d443c] px-4 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">
                ...
                {post.comments.map((comment) => (
                  <div key={comment.id} className={`rounded-2xl px-4 py-3 ${comment.isStylistComment ? "border border-[#d9e6d8] bg-[#f2f7f1]" : "bg-[#fbf7f0]"}`}>
                    ...
                    {confirmDeleteKey === `comment:${comment.id}` ? (
                      <div className="mt-3 grid gap-3 border-t border-[#ead8d3] pt-3">
                        <span className="text-xs font-semibold text-[#8c3832]">このコメントを削除しますか？</span>
                        <label className="flex min-h-12 cursor-pointer items-center gap-3 rounded-xl border border-[#e5c9c3] bg-white px-3 py-2 text-sm font-semibold text-[#5b5149] focus-within:ring-4 focus-within:ring-[#e9c9be]/40">
                          <input type="checkbox" checked={confirmedDeleteKey === `comment:${comment.id}`} disabled={Boolean(pendingKey)} onChange={(event) => setConfirmedDeleteKey(event.target.checked ? `comment:${comment.id}` : null)} className="h-5 w-5 shrink-0 accent-[#9d443c]" />
                          <span>削除する内容を確認しました</span>
                        </label>
                        <div className="flex justify-end gap-2">
                          <button type="button" onClick={closeDeleteConfirmation} className="min-h-11 rounded-full border border-[#e4cfc9] bg-white px-4 text-xs font-semibold">戻る</button>
                          <button type="button" onClick={() => void deleteComment(post.id, comment.id)} disabled={confirmedDeleteKey !== `comment:${comment.id}` || Boolean(pendingKey)} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-[#9d443c] px-4 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50">
