...
function redirectError(request: NextRequest, error: string) {
  ...
  url.searchParams.set("account", error);
  url.hash = "login-settings";
...
export async function POST(request: NextRequest) {
  ...
  if (newPassword && (newPassword.length < 8 || newPassword.length > 128 || newPassword !== newPasswordConfirm)) return redirectError(request, "password");

  const user = await prisma.appUser.findFirst({
    where: { id: session.userId, customerId: session.customerId, organizationId: session.organizationId, role: "CUSTOMER", active: true },
    select: { id: true, loginId: true, email: true, passwordHash: true }
  });
  ...
  const response = NextResponse.redirect(getExternalRequestUrl(request, "/u/login?account=updated"), 303);
  response.cookies.set({ name: CUSTOMER_SESSION_COOKIE, value: "", httpOnly: true, sameSite: "lax", secure: isExternalHttpsRequest(request), path: "/", maxAge: 0 });
