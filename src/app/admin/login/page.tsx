    ...
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#fbf7f0] px-4 py-10 text-[#2f2a25]">
      ...
      <section className="relative grid w-full max-w-5xl overflow-hidden rounded-[28px] border border-[#e8ded2] bg-white shadow-[0_24px_70px_rgba(47,42,37,0.12)] lg:grid-cols-[minmax(0,1.05fr)_minmax(24rem,0.95fr)]">
        <BrandVisual variant="customerCrm" className="h-52 lg:h-full lg:min-h-[620px]" imageClassName="object-[67%_52%]" sizes="(max-width: 1023px) 100vw, 520px" priority overlay="strong">
          <div className="flex h-full items-end p-6 sm:p-8"><div className="max-w-sm text-white"><p className="text-xs font-semibold">ORIMIA CRM</p><p className="mt-2 text-2xl font-semibold leading-tight">接客の記録を、次のご来店へつなげる。</p></div></div>
        </BrandVisual>
        <div className="p-6 sm:p-8 lg:p-10">
          <div className="flex items-center gap-3"><a href="https://salon-de-lien.com/" aria-label="ORIMIAトップへ" className="block h-12 w-12 rounded-2xl bg-white bg-cover bg-center shadow-sm" style={{ backgroundImage: "url(/brand/orimia-icon-192.png)" }} /><div><p className="text-lg font-semibold">ORIMIA</p><p className="text-xs font-medium text-[#7c7168]">スタッフ管理画面</p></div></div>
