param(
  ...
  [string]$SourceTaskDefinition = 'salon-de-lien-staging-web:345',
  [string]$Image = '009293460979.dkr.ecr.ap-northeast-1.amazonaws.com/salon-de-lien-staging-app:customer-registration-resend-v346'
)
...
$input = [ordered]@{
  family = $task.family; taskRoleArn = $task.taskRoleArn; executionRoleArn = $task.executionRoleArn
  networkMode = $task.networkMode; containerDefinitions = $task.containerDefinitions; volumes = $task.volumes
  placementConstraints = $task.placementConstraints; requiresCompatibilities = $task.requiresCompatibilities
