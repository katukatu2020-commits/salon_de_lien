(() => {
  ...
  if (window.__lienStyleCommunityControlsV615) return;
  window.__lienStyleCommunityControlsV615 = true;

  const audience = location.pathname.startsWith("/admin/")
    ? "staff"
    : "customer";
  const pencilIcon =
  ...
  const moreIcon =
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round" aria-hidden="true"><circle cx="5" cy="12" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/></svg>';
  const eyeIcon =
  ...
  function addStyles() {
    if (document.getElementById("lien-style-community-controls-v615-style"))
      return;
    const style = document.createElement("style");
    style.id = "lien-style-community-controls-v615-style";
    style.textContent = `
      ...
      .lien-comment-menu-shell{position:absolute;top:-10px;right:-8px;z-index:2;display:block;margin-left:0;flex:0 0 auto}
      .lien-comment-menu-trigger{display:grid;width:40px;height:40px;place-items:center;border:0;border-radius:50%;background:transparent;color:#756960;cursor:pointer;transition:background .16s ease,color .16s ease}
      .lien-comment-menu-trigger:hover,.lien-comment-menu-trigger:focus-visible,.lien-comment-menu-shell.is-open .lien-comment-menu-trigger{background:rgba(255,255,255,.9);color:#2f2a25;outline:none;box-shadow:0 0 0 3px rgba(233,201,190,.34)}
      .lien-comment-menu-trigger svg{width:20px;height:20px}
      .lien-comment-menu{position:absolute;top:calc(100% + 5px);right:0;z-index:90;display:grid;min-width:144px;padding:6px;border:1px solid #e3d7cc;border-radius:8px;background:#fff;box-shadow:0 14px 36px rgba(47,42,37,.18)}
      .lien-comment-menu-shell.opens-up .lien-comment-menu{top:auto;bottom:calc(100% + 5px)}
      .lien-comment-menu[hidden]{display:none!important}
      .lien-comment-menu-item{display:flex;min-height:44px;width:100%;align-items:center;gap:10px;padding:0 12px;border:0;border-radius:6px;background:transparent;color:#3f3833;font:inherit;font-size:14px;font-weight:700;text-align:left;white-space:nowrap;cursor:pointer}
      .lien-comment-menu-item:hover,.lien-comment-menu-item:focus-visible{background:#f7f1eb;outline:none}.lien-comment-menu-item.is-danger{color:#963e37}.lien-comment-menu-item.is-danger:hover,.lien-comment-menu-item.is-danger:focus-visible{background:#fff0ee}
      .lien-comment-menu-item svg{width:17px;height:17px;flex:none}
      [data-lien-comment-id].has-open-comment-menu{position:relative;z-index:10}
      ...
      .lien-content-dialog__confirmation{margin-top:18px}.lien-content-dialog__confirmation label{display:flex;min-height:52px;align-items:center;gap:12px;margin:0;padding:12px 14px;border:1px solid #e5d5cb;border-radius:12px;background:#fff7f5;color:#4b423c;font-size:14px;font-weight:700;line-height:1.55;cursor:pointer;box-sizing:border-box}.lien-content-dialog__confirmation input[type="checkbox"]{width:22px;height:22px;flex:0 0 22px;margin:0;accent-color:#9d443c;cursor:pointer}.lien-content-dialog__confirmation label:focus-within{border-color:#8f4f42;box-shadow:0 0 0 4px rgba(233,201,190,.4)}.lien-content-dialog__confirmation:has(input:checked) label{border-color:#d5b3aa;background:#fff1ef}
  ...
  function ensureDialog() {
    ...
    dialog.innerHTML =
      '<div class="lien-content-dialog__body"><h2 data-dialog-title></h2><p data-dialog-description></p><textarea data-dialog-input maxlength="2000"></textarea><div class="lien-content-dialog__confirmation" data-dialog-confirmation hidden><label for="lien-delete-confirmation"><input id="lien-delete-confirmation" type="checkbox" data-dialog-confirmation-input><span>削除する内容を確認しました</span></label></div><p class="lien-content-dialog__error" data-dialog-error role="alert"></p><div class="lien-content-dialog__actions"><button type="button" class="lien-content-dialog__button" data-dialog-cancel>戻る</button><button type="button" class="lien-content-dialog__button is-primary" data-dialog-confirm>保存</button></div></div>';
  ...
  function openDialog({
    ...
    const confirmationInput = dialog.querySelector(
      "[data-dialog-confirmation-input]",
    );
    ...
    confirm.onclick = async () => {
      ...
      if (requireConfirmation && !confirmationInput.checked) {
        dialog.querySelector("[data-dialog-error]").textContent =
          "確認欄にチェックを入れてください。";
        return;
      }
    ...
    if (input) window.setTimeout(() => textarea.focus(), 30);
      ...
      window.setTimeout(() => confirmationInput.focus(), 30);
  }
  ...
  async function requestJson(url, options) {
    ...
    if (!response.ok)
      throw new Error(result?.error || "操作を完了できませんでした。");
    return result;
  }
  ...
  function closeCommentMenus(except = null) {
    document
      .querySelectorAll(".lien-comment-menu-shell.is-open")
      .forEach((shell) => {
        if (shell === except) return;
        shell.classList.remove("is-open");
        shell.classList.remove("opens-up");
        shell
          .querySelector(".lien-comment-menu-trigger")
          ?.setAttribute("aria-expanded", "false");
        const menu = shell.querySelector(".lien-comment-menu");
        if (menu) menu.hidden = true;
        shell
          .closest("[data-lien-comment-id]")
          ?.classList.remove("has-open-comment-menu");
      });
  }

  function commentMenuItem(label, icon, onSelect, danger = false) {
    const button = document.createElement("button");
    button.type = "button";
    button.className = `lien-comment-menu-item${danger ? " is-danger" : ""}`;
  ...
  function buildCustomerCommentMenu(comment, onEdit, onDelete) {
    const shell = document.createElement("div");
    shell.className = "lien-comment-actions lien-comment-menu-shell";
    shell.dataset.lienCustomerCommentMenu = "v615";

    const safeId = String(comment.id).replace(/[^a-zA-Z0-9_-]/g, "") || "owned";
    const menuId = `lien-comment-menu-${safeId}`;
    const trigger = document.createElement("button");
    trigger.type = "button";
    trigger.className = "lien-comment-menu-trigger";
    trigger.setAttribute("aria-label", "コメントの操作");
    trigger.setAttribute("aria-haspopup", "menu");
    trigger.setAttribute("aria-expanded", "false");
    trigger.setAttribute("aria-controls", menuId);
    trigger.title = "コメントの操作";
    trigger.innerHTML = moreIcon;

    const menu = document.createElement("div");
    menu.id = menuId;
    menu.className = "lien-comment-menu";
    menu.setAttribute("role", "menu");
    menu.setAttribute("aria-label", "コメントの操作");
    menu.hidden = true;
    if (comment.canEdit)
      menu.appendChild(commentMenuItem("編集", pencilIcon, onEdit));
    if (comment.canDelete)
      menu.appendChild(commentMenuItem("削除", trashIcon, onDelete, true));
    ...
    trigger.addEventListener("click", (event) => {
      ...
      shell
        .closest("[data-lien-comment-id]")
        ?.classList.toggle("has-open-comment-menu", opening);
      if (opening)
        window.setTimeout(() => {
          const bottomGuard = window.matchMedia("(max-width: 639px)").matches
            ? 104
            : 12;
  ...
  function visibleCommentSignature(content) {
    const cards = Array.from(content.querySelectorAll(".mt-4.grid > div"));
    return cards
      .map((card) => {
  ...
  function updateVisibleCommentCount(content) {
    const count = content.querySelectorAll(".mt-4.grid > div").length;
    const counter = content.querySelector(
      "svg.lucide-message-circle",
    )?.parentElement;
    if (!counter) return;
    Array.from(counter.childNodes).forEach((node) => {
      if (node.nodeType === Node.TEXT_NODE) node.remove();
    });
    counter.append(`${count}件`);
  }
  ...
  function confirmDeletePost(postId, nextPath, onDeleted) {
    openDialog({
      title: "スタイル投稿を削除しますか？",
      description:
        "共有投稿と、その投稿に付いたコメント・いいねを削除します。来店履歴と施術写真の原本は残ります。この操作は取り消せません。",
      confirmLabel: "投稿を削除",
      danger: true,
      input: false,
  ...
  function hiddenMedia(post) {
    ...
    link.className =
      "group relative aspect-[4/5] min-w-0 overflow-hidden bg-[#f4eee8] lien-style-card__media";
    link.setAttribute(
      "aria-label",
      post.published ? "スタイル詳細を開く" : "非公開のスタイル投稿",
    );
    if (post.coverPhotoUrl) {
      ...
      placeholder.innerHTML =
        '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect width="18" height="18" x="3" y="3" rx="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.1-3.1a2 2 0 0 0-2.8 0L6 21"/></svg>';
  ...
  function buildManagedCard(post, media) {
    ...
    visibility.addEventListener("click", async () => {
      ...
      try {
        ...
        showToast(
          nextPublished ? "投稿を公開しました。" : "投稿を非公開にしました。",
        );
      } catch (error) {
        showToast(
          error instanceof Error
    ...
    remove.addEventListener("click", () =>
      confirmDeletePost(post.id, "/admin/community", () => card.remove()),
    );
  ...
  function managedGridIsComplete(grid) {
    ...
    const cards = Array.from(
      grid.querySelectorAll(":scope > .lien-style-card"),
    );
    ...
    return (
      ...
      cards.every((card) => {
        return (
          card.querySelectorAll(":scope > .lien-style-card__footer").length ===
            1 &&
  ...
  async function enhanceCommunityList() {
    if (
      audience !== "staff" ||
      !/^\/admin\/community\/?$/.test(location.pathname)
    )
      return;
    delete document.body.dataset.lienCommunityOwnerEnhancedV471;
    document
      .querySelectorAll("[data-lien-post-manager]")
      .forEach((manager) => manager.remove());
    const currentGrid = document.querySelector(
      "[data-lien-style-grid-managed-v509]",
    );
    ...
    try {
      payload = await requestJson(
        "/api/lien-content-management?audience=staff&scope=posts",
      );
    } catch {
      return;
    } finally {
    ...
    const restoredGrid = document.querySelector(
      "[data-lien-style-grid-managed-v509]",
    );
    ...
    const publicLinks = Array.from(
      main.querySelectorAll('a[href^="/admin/community/"]'),
    ).filter((link) => {
      return (
        postIdFromHref(link.href) &&
        (link.classList.contains("aspect-[4/5]") ||
          Boolean(link.querySelector('[style*="background-image"]')))
      );
    });
    const publicGrid =
      publicLinks[0]?.closest(".lien-style-card")?.parentElement ||
      publicLinks[0]?.parentElement;
    const emptyMessage = Array.from(main.querySelectorAll("p")).find((node) =>
      node.textContent?.includes("条件に合うスタイルはありません"),
    );
    const emptyState = emptyMessage?.parentElement;
    const resultSection =
      publicGrid?.parentElement ||
      Array.from(main.querySelectorAll("section")).find((section) =>
        section.textContent?.includes("件を表示"),
      ) ||
    ...
    if (emptyState?.isConnected && !resultSection.contains(emptyState))
      emptyState.remove();

    const summary = document.createElement("div");
    summary.className =
      "flex items-center justify-between gap-3 text-xs text-[#7c7168]";
    ...
    ).some((key) => !ignoredQueryKeys.has(key));
    ...
    const supplementalCards = supplementalPosts.map((post) =>
      buildManagedCard(post, hiddenMedia(post)),
    );
    ...
    const staleManagedGrid = resultSection.querySelector(
      "[data-lien-style-grid-managed-v471]",
    );
    if (staleManagedGrid) staleManagedGrid.remove();
    resultSection.replaceChildren(summary, grid);
  }

  function findCommentCard(content, comment, assignedCards) {
    const escapedId = window.CSS?.escape
      ? window.CSS.escape(comment.id)
      : comment.id.replace(/[^a-zA-Z0-9_-]/g, "");
    const byId = content.querySelector(`[data-lien-comment-id="${escapedId}"]`);
    if (byId && !assignedCards.has(byId)) {
      assignedCards.add(byId);
      return byId;
    }
    const matched =
      Array.from(content.querySelectorAll(".mt-4.grid > div")).find((card) => {
        if (assignedCards.has(card)) return false;
        const text = (card.textContent || "").replace(/\s+/g, " ").trim();
        return (
          text.includes(comment.authorDisplayName) &&
          text.includes(comment.body)
        );
      }) || null;
    if (matched) assignedCards.add(matched);
    return matched;
  }

  let commentMenuRetryCount = 0;

  async function enhanceCommunityDetail() {
    const match = location.pathname.match(/^\/(admin|u)\/community\/([^/?#]+)/);
    if (!match || window.__lienCommunityDetailLoadingV615 === match[2]) return;
    let article = document.querySelector(
      ".community-detail-page article, main article",
    );
    if (!article) return;
    const initialContent =
      article.querySelector(".community-feed-content") ||
      article.querySelector(":scope > div:last-of-type");
    if (!initialContent) return;
    const initialSignature = visibleCommentSignature(initialContent);
    const expectedMenuCount = initialContent.dataset.lienOwnedCommentMenuCountV615;
    const ownershipMenusIntact =
      audience !== "customer" ||
      (expectedMenuCount !== undefined &&
        initialContent.querySelectorAll(
          '[data-lien-customer-comment-menu="v615"]',
        ).length === Number(expectedMenuCount));
    if (
      document.body.dataset.lienCommunityOwnerEnhancedV615 === match[2] &&
      initialContent.dataset.lienCommentSignatureV615 === initialSignature &&
      ownershipMenusIntact
    )
      return;
    const postId = decodeURIComponent(match[2]);
    let payload;
    window.__lienCommunityDetailLoadingV615 = match[2];
    try {
      payload = await requestJson(
        `/api/lien-content-management?audience=${audience}&postId=${encodeURIComponent(postId)}`,
      );
    } catch {
      return;
    } finally {
      if (window.__lienCommunityDetailLoadingV615 === match[2])
        window.__lienCommunityDetailLoadingV615 = "";
    }
    if (!location.pathname.includes(`/community/${match[2]}`)) return;
    ...
    const content =
      ...
      article.querySelector(":scope > div:last-of-type");
    let meta =
      content?.querySelector(".community-feed-meta") ||
      content?.querySelector(":scope > div:first-child");
    if (!content) return;
    if (!meta && audience === "customer")
      meta = content.querySelector(".orimia-style-details-v602");
    if (!meta) return;
    const commentOnlyCustomer =
      audience === "customer" &&
      meta.classList.contains("orimia-style-details-v602");
    document.body.dataset.lienCommunityOwnerEnhancedV615 = postId;
    ...
    if (
      ...
      payload.post.canDelete ||
      payload.post.canChangeVisibility
    ) {
          ...
          openDialog({
            ...
            onConfirm: async (body) => {
              const result = await requestJson(
                `/api/lien-content-management?audience=${audience}`,
                {
                  method: "PATCH",
      ...
      if (payload.post.canChangeVisibility) {
        const visibility = actionButton(
          payload.post.published ? "非公開にする" : "公開する",
          payload.post.published ? eyeOffIcon : eyeIcon,
          payload.post.published ? "" : "is-primary",
        );
        visibility.addEventListener("click", async () => {
          visibility.disabled = true;
          try {
      ...
      if (payload.post.canDelete) {
        ...
        remove.addEventListener("click", () =>
          confirmDeletePost(
            postId,
            audience === "staff" ? "/admin/community" : "/u/community",
          ),
        );
    ...
    const assignedCommentCards = new Set();
    let expectedOwnedCommentMenus = 0;
    let attachedOwnedCommentMenus = 0;
    payload.comments.forEach((comment) => {
      const canManageComment = comment.canEdit || comment.canDelete;
      if (audience === "customer" && canManageComment)
        expectedOwnedCommentMenus += 1;
      const card = findCommentCard(content, comment, assignedCommentCards);
      if (!card || !canManageComment) return;
      ...
      const editComment = () =>
        openDialog({
          ...
          onConfirm: async (body) => {
            ...
            content.dataset.lienCommentSignatureV615 =
              visibleCommentSignature(content);
      ...
      const deleteComment = () =>
        openDialog({
          ...
          onConfirm: async () => {
            ...
            card.remove();
            updateVisibleCommentCount(content);
            content.dataset.lienCommentSignatureV615 = "";
    ...
    content.dataset.lienCommentSignatureV615 = visibleCommentSignature(content);
    if (audience === "customer") {
      content.dataset.lienOwnedCommentMenuCountV615 = String(
        expectedOwnedCommentMenus,
      );
      if (
        attachedOwnedCommentMenus < expectedOwnedCommentMenus &&
        commentMenuRetryCount < 6
      ) {
        commentMenuRetryCount += 1;
        window.setTimeout(schedule, 180);
      } else if (attachedOwnedCommentMenus === expectedOwnedCommentMenus) {
        commentMenuRetryCount = 0;
      }
    }
  }
  ...
  function enhanceChatMessages() {
    document
      .querySelectorAll(
        "[data-lien-chat-message]:not([data-lien-chat-layout-v509])",
      )
      .forEach((row) => {
        row.dataset.lienChatLayoutV509 = "1";
        const bubble = row.querySelector(
          "[data-lien-chat-body], .lien-chat-v294__message",
        );
        if (!bubble) return;
        ...
        bubble.addEventListener("dblclick", (event) => {
          ...
          if (document.querySelector("[data-lien-content-dialog]")?.open)
            return;
          openDialog({
            title: "メッセージを削除しますか？",
  ...
  function useStableCommunityNavigation(event) {
    ...
    if (
      ...
      !/^\/admin\/community\/?$/.test(url.pathname)
    )
      return;
    if (/^\/admin\/community\/?$/.test(location.pathname)) return;
    event.preventDefault();
  ...
  document.addEventListener("click", (event) => {
    if (
      ...
      !event.target.closest(".lien-comment-menu-shell")
    )
      closeCommentMenus();
  });
  document.addEventListener("keydown", (event) => {
    ...
    const trigger = document.querySelector(
      ".lien-comment-menu-shell.is-open .lien-comment-menu-trigger",
    );
    if (!trigger) return;
    closeCommentMenus();
    trigger.focus();
  ...
  const activateControls = () => {
    ...
    const afterDocumentReady = () => {
      if (document.documentElement.dataset.orimiaUiReady === "v516") {
        afterDomReady();
        return;
      }
      window.addEventListener("orimia:ui-transition-finished", afterDomReady, {
        once: true,
      });
      window.setTimeout(afterDomReady, 1800);
    };
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", afterDocumentReady, {
        once: true,
      });
    } else {
      afterDocumentReady();
  ...
  window.__lienStyleCommunityRepairV615 = () => {
    activateControls();
    scheduleAfterRouteChange();
  };
  activateControls();
  ...
  communityRouteObserver.observe(document.documentElement, {
    childList: true,
    subtree: true,
  });
})();
