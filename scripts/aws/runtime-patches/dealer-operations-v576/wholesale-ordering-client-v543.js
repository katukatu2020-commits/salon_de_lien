...
/* dealer-auth-self-service-v548 */
  ...
  function salonOrderProducts() {
    const local = salon.data.products.map(function (product) { return Object.assign({ source: 'SALON' }, product) })
    const localCodes = new Set(local.map(function (product) { return String(product.productCode || '').toLowerCase() }).filter(Boolean))
    const catalog = (salon.data.catalogProducts || []).filter(function (product) { return !product.productCode || !localCodes.has(String(product.productCode).toLowerCase()) })
    return local.concat(catalog)
  }
  ...
  function orderProductRows(products) {
    ...
    return products.map(function (product) {
      const dealerCatalog = product.source === 'DEALER'
      const low = !dealerCatalog && Number(product.stockQuantity) <= 3
      return '<article class="wo-product-row">' +
        ...
        '<dl class="wo-product-data"><div><dt>' + (dealerCatalog ? '商品区分' : '現在庫') + '</dt><dd class="' + (low ? 'low' : '') + '">' + (dealerCatalog ? '<span class="wo-catalog-label">取扱商品</span>' : Number(product.stockQuantity).toLocaleString('ja-JP') + '<small>点</small>') + '</dd></div><div><dt>前回発注</dt><dd>' + (product.lastOrderQuantity == null ? 'なし' : Number(product.lastOrderQuantity).toLocaleString('ja-JP') + '<small>点</small>') + '<small class="wo-subdate">' + (product.lastOrderedAt ? date(product.lastOrderedAt) : '') + '</small></dd></div><div><dt>発注単位</dt><dd>' + Number(product.orderUnit || 1) + '<small>点</small></dd></div></dl>' +
  ...
  function orderView() {
    ...
    return '<section class="wo-workspace"><header class="wo-workspace-head"><div><p class="wo-section-label">PURCHASE ORDER</p><h2>商品を発注</h2><p>在庫を確認しながら、必要な商品の発注数を入力してください。</p></div>' +
      ...
      (!activeContracts.length ? emptyState('truck', '発注先がまだ連携されていません', '上の「ディーラーを追加」から固有コードで連携を依頼してください。') : productFilters(sourceProducts) + '<div class="wo-order-layout"><div class="wo-product-list">' + orderProductRows(products) + '</div>' + orderSummary() + '</div>') + '</section>'
  }
  ...
  async function submitOrder(button) {
    ...
    try {
      const result = await post('/api/admin/wholesale/orders', {
        ...
        lines: selectedOrder().map(function (item) {
          return item.product.source === 'DEALER'
            ? { dealerProductId: item.product.dealerProductId, quantity: item.quantity }
  ...
  function dealerProducts() {
    ...
    return '<section class="wo-workspace wo-product-management"><header class="wo-workspace-head"><div><p class="wo-section-label">DEALER CATALOG</p><h2>商品を登録</h2><p>ここで登録した商品は、連携中の美容室の発注画面に表示されます。</p></div><span class="wo-count-chip">' + products.length + '商品</span></header><form id="dealer-product-form" class="wo-dealer-product-form">' + dealerProductFields() + '<div class="wo-form-actions"><button class="wo-button wo-button-primary" type="submit">' + icon('plus') + '商品を登録</button></div></form>' +
  ...
  function renderDealer() {
    const content = dealer.view === 'salons' ? dealerContracts() : dealer.view === 'products' ? dealerProducts() : dealerOrders()
  ...
  function bindDealer() {
    ...
    root.addEventListener('change', function (event) {
      if (!event.target.matches('[data-remove-confirm]')) return
