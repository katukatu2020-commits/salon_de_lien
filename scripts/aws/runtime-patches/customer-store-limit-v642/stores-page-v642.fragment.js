  async function storesPage(req, res, url) {
    ...
    const maxStores = customerStoreMembershipV642.MAX_STORES
    const atLimit = rows.length >= maxStores
    ...
    const cards = rows.map(store => {
      ...
      return `<article class="registered-store-card ${store.current ? 'current' : ''}">
        ...
        <div class="registered-store-copy"><strong>${escapeHtml(store.name)}</strong><p>${status}</p></div>
        <div class="registered-store-card-actions">${primaryAction}<button class="registered-store-remove" type="button" data-remove-store="${escapeHtml(store.organizationId)}" data-store-name="${escapeHtml(store.name)}"${canRemove ? '' : ' disabled title="最後の1店舗は解除できません"'}>登録を解除</button></div>
      </article>`
    }).join('')
    const initialCode = storeCodeFromQuery(url.searchParams.get('store'))
    const limitMessage = `登録できる美容室は最大${maxStores}店舗です。新しい店舗を追加するには、登録済みの店舗を1件解除してください。`
