(() => {
  ...
  if (window.__lienStyleCommunityControlsV482) return
  window.__lienStyleCommunityControlsV482 = true
  ...
  function addStyles() {
    if (document.getElementById('lien-style-community-controls-v482-style')) return
    const style = document.createElement('style')
    style.id = 'lien-style-community-controls-v482-style'
    style.textContent = `
  ...
  async function enhanceCommunityList() {
    ...
    if (document.querySelector('[data-lien-style-grid-managed-v482]')) return
    if (window.__lienStyleGridLoadingV482) return
    const main = document.querySelector('main')
    if (!main) return

    let payload
    window.__lienStyleGridLoadingV482 = true
    try {
      payload = await requestJson('/api/lien-content-management?audience=staff&scope=posts')
    } catch {
      return
    } finally {
      window.__lienStyleGridLoadingV482 = false
    }
    if (!/^\/admin\/community\/?$/.test(location.pathname) || document.querySelector('[data-lien-style-grid-managed-v482]')) return
    ...
    grid.dataset.lienStyleGridManagedV482 = '1'
