...
const page = await fetch(`${baseUrl}/admin/community?smoke=v481`, {
  headers: { cookie: sessionCookie, 'Cache-Control': 'no-cache' },
})
assert.equal(page.status, 200)
const html = await page.text()
assert.match(html, /content-edit-delete-client-v481\.js/)
assert.match(html, /data-lien-community-bootstrap="v481"/)
for (const postId of historicalPostIds) assert.ok(html.includes(postId), `SSR post is missing: ${postId}`)

const client = await fetch(`${baseUrl}/content-edit-delete-client-v481.js?smoke=v481`, {
...
assert.match(source, /data-lien-style-grid-managed-v481/)

const sharedAdmin = await fetch(`${baseUrl}/admin-staff-experience-v276.js?smoke=v481`, {
  headers: { 'Cache-Control': 'no-cache' },
})
assert.equal(sharedAdmin.status, 200)
const sharedSource = await sharedAdmin.text()
assert.match(sharedSource, /__lienStyleCommunityLoaderV481/)
assert.match(sharedSource, /content-edit-delete-client-v481\.js/)
