(() => {
  ...
  if (window.__lienStyleCommunityControlsV481) return
  window.__lienStyleCommunityControlsV481 = true
  ...
  function addStyles() {
    if (document.getElementById('lien-style-community-controls-v481-style')) return
    const style = document.createElement('style')
    style.id = 'lien-style-community-controls-v481-style'
    style.textContent = `
  ...
  async function enhanceCommunityList() {
    ...
    if (document.querySelector('[data-lien-style-grid-managed-v481]')) return
    if (window.__lienStyleGridLoadingV481) return
    const main = document.querySelector('main')
    if (!main) return

    let payload
    window.__lienStyleGridLoadingV481 = true
    try {
      payload = await requestJson('/api/lien-content-management?audience=staff&scope=posts')
    } catch {
      return
    } finally {
      window.__lienStyleGridLoadingV481 = false
    }
    if (!/^\/admin\/community\/?$/.test(location.pathname) || document.querySelector('[data-lien-style-grid-managed-v481]')) return
    ...
    grid.dataset.lienStyleGridManagedV481 = '1'
