...
async function loadCustomerIdentityIds(prisma, session) {
  const identities = new Set()
  if (!session || !session.customerId || !session.organizationId) return identities
  identities.add(String(session.customerId))
  try {
    ...
    for (const row of rows) {
      if (row.customerId) identities.add(String(row.customerId))
    }
  } catch (error) {
    console.warn('[customer-comment-owner-v559] customer lineage unavailable', {
...
function canManageComment(session, comment, customerIdentityIds = new Set()) {
  if (!session || !comment || !session.userId) return false
  if (String(comment.appUserId || '') === String(session.userId)) return true
  if (session.role !== 'CUSTOMER' || comment.authorRole !== 'CUSTOMER' || !session.customerId) return false
  return [comment.ownerDirectCustomerId, comment.ownerLinkedCustomerId]
    .some(customerId => customerId && customerIdentityIds.has(String(customerId)))
}
...
function createContentManagementService({ prisma, staffSessionProvider, customerSessionProvider, canAccessThread, resolvePostCover = resolvePostCoverReference }) {
  ...
  async function contentGet(req, res, url, session, audience) {
    ...
    const customerIdentityIds = audience === 'customer'
      ? await loadCustomerIdentityIds(prisma, session)
      : new Set()
    const comments = await prisma.$queryRawUnsafe(
      `SELECT c."id",c."appUserId",c."authorDisplayName",c."authorRole",c."body",c."createdAt",c."updatedAt",
              u."customerId" AS "ownerDirectCustomerId",l."customerId" AS "ownerLinkedCustomerId"
         FROM "VisitCommunityComment" c
         LEFT JOIN "AppUser" u ON u."id"=c."appUserId"
         LEFT JOIN "CustomerStoreLink" l
           ON l."appUserId"=c."appUserId" AND l."organizationId"=$2
        WHERE c."postId"=$1 AND c."deletedAt" IS NULL AND c."isAiAssistant"=FALSE
        ORDER BY c."createdAt" ASC LIMIT 100`,
      post.id,
      session.organizationId,
    )
    return json(res, 200, {
      ...
      comments: comments.map(comment => ({
        ...
        canEdit: canManageComment(session, comment, customerIdentityIds),
        canDelete: canManageComment(session, comment, customerIdentityIds),
      })),
    })
  }
  ...
  async function contentMutation(req, res, session, audience) {
    ...
    if (target === 'comment') {
      ...
      const customerIdentityIds = audience === 'customer'
        ...
        : new Set()
      const comments = await prisma.$queryRawUnsafe(
        `SELECT c."id",c."postId",c."appUserId",c."authorRole",
                u."customerId" AS "ownerDirectCustomerId",l."customerId" AS "ownerLinkedCustomerId"
           FROM "VisitCommunityComment" c
           LEFT JOIN "AppUser" u ON u."id"=c."appUserId"
           LEFT JOIN "CustomerStoreLink" l
             ON l."appUserId"=c."appUserId" AND l."organizationId"=$3
          WHERE c."id"=$1 AND c."postId"=$2 AND c."deletedAt" IS NULL LIMIT 1`,
        commentId,
        post.id,
        session.organizationId,
      )
      ...
      if (!canManageComment(session, comment, customerIdentityIds)) return json(res, 403, { error: 'このコメントを変更する権限がありません。' })
      if (req.method === 'PATCH') {
