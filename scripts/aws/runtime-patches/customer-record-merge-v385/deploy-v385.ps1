...
$tag = "customer-record-merge-v385"
...
if ($imageExists) {
  ...
  docker build --pull=false --build-arg BASE_IMAGE="$accountId.dkr.ecr.$region.amazonaws.com/${repository}:staff-days-off-identity-v384" -f "scripts/aws/runtime-patches/customer-record-merge-v385/Dockerfile" -t $image .
...
$taskFile = Join-Path $env:TEMP "salon-v385-task-definition.json"
...
$invalidation = aws cloudfront create-invalidation --distribution-id $distributionId --paths "/admin/customers*" "/api/admin/customers*" "/tenant-setup-client.js" "/customer-merge-v385.js*" --query "Invalidation.Id" --output text
