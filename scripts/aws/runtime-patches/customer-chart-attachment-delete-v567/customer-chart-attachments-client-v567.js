;(() => {
  ...
  const icon = name => {
    const paths = {
      ...
      alert: '<path d="M12 9v4"></path><path d="M12 17h.01"></path><path d="M10.3 3.6 2.4 18a2 2 0 0 0 1.8 3h15.6a2 2 0 0 0 1.8-3L13.7 3.6a2 2 0 0 0-3.4 0Z"></path>',
    }
  ...
  function installStyles() {
    if (document.querySelector('style[data-customer-chart-v567]')) return
    const style = document.createElement('style')
    style.dataset.customerChartV567 = '1'
    style.textContent = `
      ...
      .lien-chart-card *,.lien-chart-history *,.lien-chart-preview-dialog *,.lien-chart-delete-dialog *{box-sizing:border-box;letter-spacing:0}
      ...
      .lien-chart-meta{display:flex;align-items:stretch;flex-direction:column;background:#fff;padding:30px 26px;color:#342b27;font-size:11px}.lien-chart-meta-kicker{color:#b54663;font-size:9px;font-weight:900}.lien-chart-meta h3{margin:9px 0 5px;font-family:"Yu Mincho","Hiragino Mincho ProN",serif;font-size:23px;line-height:1.35}.lien-chart-meta-sub{margin:0 0 20px;color:#8a7b74;font-size:10px}.lien-chart-meta-list{display:grid;margin:0}.lien-chart-meta-row{display:grid;gap:5px;border-top:1px solid #eee4df;padding:12px 0}.lien-chart-meta-row dt{color:#94857e;font-size:9px;font-weight:800}.lien-chart-meta-row dd{min-width:0;margin:0;overflow-wrap:anywhere;color:#3d322e;font-size:11px;font-weight:900;line-height:1.55}.lien-chart-file-actions{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;margin-top:18px}.lien-chart-file-action{min-width:0;padding:0 8px}.lien-chart-file-action.download{border-color:#ead4da;color:#a93f5d}.lien-chart-file-action.danger{border-color:#e8c7cf;background:#fff;color:#a93e57}.lien-chart-file-action.danger:hover{border-color:#bb5068;background:#fff3f6;color:#953149}.lien-chart-file-action:disabled{cursor:wait;opacity:.55}.lien-chart-archive-summary{display:flex;align-items:center;gap:11px;margin-top:auto;border-top:1px solid #eee4df;padding-top:18px;color:#6e5b53}.lien-chart-archive-summary>span{display:grid;width:34px;height:34px;place-items:center;border-radius:7px;background:#f9edf0;color:#b54663}.lien-chart-archive-summary svg{width:17px;height:17px}.lien-chart-archive-summary div{display:grid}.lien-chart-archive-summary strong{font-size:15px}.lien-chart-archive-summary small{color:#91817a;font-size:9px}
      ...
      .lien-chart-delete-dialog{width:min(480px,calc(100vw - 32px));max-height:calc(100dvh - 32px);overflow:auto;border:1px solid #dfcec6;border-radius:8px;background:#fff;padding:0;color:#332b27;box-shadow:0 24px 80px rgba(42,28,23,.3)}.lien-chart-delete-dialog::backdrop{background:rgba(37,28,25,.62)}.lien-chart-delete-head{display:flex;align-items:flex-start;gap:13px;border-bottom:1px solid #eaded8;padding:20px}.lien-chart-delete-head>span{display:grid;width:44px;height:44px;flex:0 0 auto;place-items:center;border:1px solid #edcbd3;border-radius:8px;background:#fff1f4;color:#a93e57}.lien-chart-delete-head svg{width:21px;height:21px}.lien-chart-delete-head h2{margin:0;font-family:"Yu Mincho","Hiragino Mincho ProN",serif;font-size:21px}.lien-chart-delete-head p{margin:5px 0 0;color:#887870;font-size:11px;line-height:1.65}.lien-chart-delete-body{display:grid;gap:14px;padding:20px}.lien-chart-delete-file{display:grid;gap:5px;border:1px solid #e8ddd7;border-radius:7px;background:#faf7f5;padding:13px}.lien-chart-delete-file small{color:#91827b;font-size:9px;font-weight:800}.lien-chart-delete-file strong{overflow-wrap:anywhere;font-size:12px}.lien-chart-delete-check{display:grid;grid-template-columns:20px minmax(0,1fr);gap:10px;align-items:start;border:1px solid #ead8dd;border-radius:7px;background:#fff8fa;padding:13px;color:#5b4841;font-size:11px;font-weight:800;line-height:1.6;cursor:pointer}.lien-chart-delete-check input{width:18px;height:18px;margin:0;accent-color:#b94662}.lien-chart-delete-feedback{min-height:17px;margin:0;color:#a13b52;font-size:10px;font-weight:800}.lien-chart-delete-actions{display:flex;justify-content:flex-end;gap:8px;border-top:1px solid #eaded8;padding:15px 20px}.lien-chart-delete-actions .lien-chart-button{min-width:120px}.lien-chart-delete-actions .danger{border-color:#a93e57;background:#a93e57;color:#fff}.lien-chart-delete-actions .danger:hover{border-color:#913047;background:#913047}.lien-chart-delete-actions .danger:disabled{border-color:#d9c9cd;background:#d9c9cd;color:#fff;cursor:not-allowed;box-shadow:none}
  ...
  function deleteApi(customerId, attachmentId) {
    return `/api/admin/customers/${encodeURIComponent(customerId)}/chart-photos/${encodeURIComponent(attachmentId)}`
  }
  ...
  function setStatus(statusNode, message, ok = false) {
    if (!statusNode) return
    statusNode.className = `lien-chart-status${ok ? ' ok' : ''}`
    statusNode.textContent = message
  }

  async function removeAttachment(customerId, attachmentId) {
    const response = await fetch(deleteApi(customerId, attachmentId), {
      method:'DELETE',
      credentials:'same-origin',
      headers:{ Accept:'application/json' },
    })
    return payload(response, 'カルテファイルを削除できませんでした。')
  }

  function confirmDelete(customerId, item, statusNode, refresh) {
    if (!item?.id) return
    let dialog = document.querySelector('[data-chart-delete-dialog-v567]')
    if (!dialog) {
      dialog = document.createElement('dialog')
      dialog.className = 'lien-chart-delete-dialog'
      dialog.dataset.chartDeleteDialogV567 = '1'
      document.body.appendChild(dialog)
    }
    dialog.innerHTML = `<header class="lien-chart-delete-head"><span>${icon('alert')}</span><div><h2>カルテファイルを削除</h2><p>この操作は取り消せません。内容を確認してから削除してください。</p></div></header><div class="lien-chart-delete-body"><div class="lien-chart-delete-file"><small>削除するファイル</small><strong>${esc(itemName(item))}</strong></div><label class="lien-chart-delete-check"><input type="checkbox" data-chart-delete-check><span>このカルテファイルを完全に削除することを確認しました。</span></label><p class="lien-chart-delete-feedback" data-chart-delete-feedback role="status" aria-live="polite"></p></div><footer class="lien-chart-delete-actions"><button type="button" class="lien-chart-button" data-chart-delete-cancel>キャンセル</button><button type="button" class="lien-chart-button danger" data-chart-delete-confirm disabled>${icon('trash')}削除する</button></footer>`
    const checkbox = dialog.querySelector('[data-chart-delete-check]')
    const cancel = dialog.querySelector('[data-chart-delete-cancel]')
    const confirm = dialog.querySelector('[data-chart-delete-confirm]')
    const feedback = dialog.querySelector('[data-chart-delete-feedback]')
    let busy = false
    checkbox.addEventListener('change', () => { confirm.disabled = !checkbox.checked || busy })
    cancel.addEventListener('click', () => { if (!busy) dialog.close() })
    dialog.onclick = event => { if (event.target === dialog && !busy) dialog.close() }
    ...
    confirm.addEventListener('click', async () => {
      ...
      confirm.innerHTML = `${icon('trash')}削除しています…`
      feedback.textContent = ''
      try {
        await removeAttachment(customerId, item.id)
        dialog.close()
        setStatus(statusNode, 'カルテファイルを削除しました。', true)
        try {
          await refresh()
        } catch {
          setStatus(statusNode, '削除しました。表示を更新するためページを再読み込みしてください。', true)
        }
      } catch (error) {
        ...
        confirm.innerHTML = `${icon('trash')}削除する`
      }
    })
    dialog.showModal()
  ...
  function latestMarkup(item, count) {
    ...
    return `<div class="lien-chart-image-shell"><button type="button" class="lien-chart-media" data-chart-open aria-label="${esc(itemName(item))}を開く">${mediaMarkup(item, true)}</button><aside class="lien-chart-meta"><span class="lien-chart-meta-kicker">LATEST CHART</span><h3>最新のカルテ</h3><p class="lien-chart-meta-sub">店舗内限定ファイル</p><dl class="lien-chart-meta-list"><div class="lien-chart-meta-row"><dt>ファイル名</dt><dd>${esc(itemName(item))}</dd></div><div class="lien-chart-meta-row"><dt>形式・容量</dt><dd>${esc(typeLabel(item))} / ${esc(formatSize(item.byteSize))}</dd></div><div class="lien-chart-meta-row"><dt>登録日時</dt><dd>${esc(formatDate(item.createdAt))}</dd></div><div class="lien-chart-meta-row"><dt>登録者</dt><dd>${esc(item.uploadedByName || '店舗スタッフ')}</dd></div></dl><div class="lien-chart-file-actions"><a class="lien-chart-file-action" href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${icon('external')}開く</a><a class="lien-chart-file-action download" href="${esc(item.downloadUrl || item.url)}">${icon('download')}保存</a><button type="button" class="lien-chart-file-action danger" data-chart-delete="${esc(item.id)}">${icon('trash')}削除</button></div><div class="lien-chart-archive-summary"><span>${icon('history')}</span><div><strong>${Number(count || 0).toLocaleString('ja-JP')}件</strong><small>保存済みカルテ</small></div></div></aside></div>`
  }
