import fs from 'node:fs'

function replaceOnce(source, before, after, label) {
  const count = source.split(before).length - 1
  if (count !== 1) throw new Error(`${label}: expected one match, found ${count}`)
  return source.replace(before, after)
}

const file = '/app/store-profile.js'
let source = fs.readFileSync(file, 'utf8')

source = replaceOnce(
  source,
  `    const websiteUrl = normalizedWebsite(data.websiteUrl)
    const organization = await prisma.organization.update({`,
  `    const websiteUrl = normalizedWebsite(data.websiteUrl)
    const previousScheduleRows = await prisma.$queryRawUnsafe(
      'SELECT "businessOpenMinutes","businessCloseMinutes","closedWeekdays" FROM "OrganizationStoreProfile" WHERE "organizationId"=$1 LIMIT 1',
      session.organizationId,
    ).catch(() => [])
    const previousSchedule = previousScheduleRows[0] || {}
    const previousOpenMinutes = Number.isInteger(Number(previousSchedule.businessOpenMinutes))
      ? Number(previousSchedule.businessOpenMinutes)
      : DEFAULT_BUSINESS_OPEN_MINUTES
    const previousCloseMinutes = Number.isInteger(Number(previousSchedule.businessCloseMinutes))
      ? Number(previousSchedule.businessCloseMinutes)
      : DEFAULT_BUSINESS_CLOSE_MINUTES
    const previousClosedWeekdays = normalizedClosedWeekdays(
      previousSchedule.closedWeekdays == null ? DEFAULT_CLOSED_WEEKDAYS : previousSchedule.closedWeekdays,
    )
    const scheduleChanged = previousOpenMinutes !== schedule.openMinutes
      || previousCloseMinutes !== schedule.closeMinutes
      || previousClosedWeekdays.join(',') !== schedule.closedWeekdays.join(',')
    const organization = await prisma.organization.update({`,
  'capture previous base business schedule',
)

source = replaceOnce(
  source,
  `    await prisma.$executeRawUnsafe(
      'UPDATE "StaffBookingSetting" SET "workStartMinutes"=$2,"workEndMinutes"=$3,"updatedAt"=NOW() WHERE "organizationId"=$1',`,
  `    /* current-day-business-hours-v491: a new base schedule must take effect today, too. */
    if (scheduleChanged) {
      const weekdayLabel = new Intl.DateTimeFormat('en-US', {
        timeZone: 'Asia/Tokyo',
        weekday: 'short',
      }).format(new Date())
      const todayWeekday = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].indexOf(weekdayLabel)
      await prisma.$executeRawUnsafe(
        \`UPDATE "OrganizationDailySchedule"
         SET "openMinutes"=$2,"closeMinutes"=$3,"isClosed"=$4,"updatedAt"=NOW()
         WHERE "organizationId"=$1
           AND "date"=TO_CHAR(NOW() AT TIME ZONE 'Asia/Tokyo','YYYY-MM-DD')\`,
        session.organizationId,
        schedule.openMinutes,
        schedule.closeMinutes,
        schedule.closedWeekdays.includes(todayWeekday),
      )
    }
    await prisma.$executeRawUnsafe(
      'UPDATE "StaffBookingSetting" SET "workStartMinutes"=$2,"workEndMinutes"=$3,"updatedAt"=NOW() WHERE "organizationId"=$1',`,
  'synchronize current day before staff hours',
)

fs.writeFileSync(file, source)
console.log('Current-day business hours v491 runtime patched.')
