...
$Tag = 'reservation-financial-breakdown-v392'
...
if ($LASTEXITCODE -ne 0) {
  ...
  $DockerConfig = Join-Path $env:TEMP ("salon-ecr-v392-" + [Guid]::NewGuid().ToString('N'))
...
docker build -f scripts/aws/runtime-patches/reservation-financial-breakdown-v392/Dockerfile -t $Image .
...
$TaskFile = Join-Path $env:TEMP 'salon-de-lien-task-v392.json'
[System.IO.File]::WriteAllText(
  $TaskFile,
