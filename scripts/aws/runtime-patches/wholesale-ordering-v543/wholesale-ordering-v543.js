...
function validSameOrigin(req) {
  ...
  try { return Boolean(host) && new URL(source).host === host } catch { return false }
}
...
function createWholesaleOrderingService({ prisma, crypto, adminSessionProvider }) {
  ...
  async function dealerSession(req) {
    ...
    const rows = await prisma.$queryRawUnsafe('SELECT "id","name","loginId","email","phone","postalCode","address" FROM "WholesaleDealer" WHERE "id"=$1 AND "active"=TRUE LIMIT 1', payload.dealerId)
    return rows[0] ? { ...payload, ...rows[0] } : null
  }

  async function inviteInfo(token) {
    const tokenHash = crypto.createHash('sha256').update(String(token || '')).digest('hex')
    const rows = await prisma.$queryRawUnsafe(`SELECT i."id" AS "inviteId",i."contractId",d."id" AS "dealerId",d."name" AS "dealerName",d."loginId",d."phone",d."postalCode",d."address",o."name" AS "organizationName"
  ...
  async function createDealerInvite(req, session, payload) {
    ...
    const email = cleanText(payload.email, 'メールアドレス', 200, false).toLowerCase()
    ...
    if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new WholesaleError('メールアドレスを確認してください。')
    const result = await prisma.$transaction(async tx => {
      const existingRows = await tx.$queryRawUnsafe('SELECT "id","name","loginId","email","passwordHash" FROM "WholesaleDealer" WHERE LOWER("loginId")=LOWER($1) OR ($2<>\'\' AND LOWER(COALESCE("email",\'\'))=LOWER($2)) LIMIT 1 FOR UPDATE', loginId, email)
  ...
  async function handle(req, res, url) {
    ...
    if (pathname === '/dealer/login' && req.method === 'GET') {
      ...
      html(res, 200, dealerLoginPage({ error: url.searchParams.get('error'), setup: url.searchParams.get('setup') })); return true
    }
    ...
    if (pathname === '/api/dealer/auth/login' && req.method === 'POST') {
      ...
      const rows = await prisma.$queryRawUnsafe('SELECT "id","name","loginId","email","passwordHash" FROM "WholesaleDealer" WHERE "active"=TRUE AND (LOWER("loginId")=LOWER($1) OR LOWER(COALESCE("email",\'\'))=LOWER($1)) LIMIT 1', loginId)
    ...
    if (pathname === '/api/dealer/auth/setup' && req.method === 'POST') {
      ...
      await prisma.$transaction(async tx => {
        await tx.$executeRawUnsafe('UPDATE "WholesaleDealer" SET "passwordHash"=$2,"phone"=NULLIF($3,\'\'),"postalCode"=NULLIF($4,\'\'),"address"=NULLIF($5,\'\'),"active"=TRUE,"updatedAt"=NOW() WHERE "id"=$1', invite.dealerId, hash, phone, postalCode, address)
