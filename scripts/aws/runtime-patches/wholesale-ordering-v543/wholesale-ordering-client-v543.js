(function () {
  ...
  function contractPanel() {
    ...
    return '<section class="wo-contract-band ' + (active.length ? 'connected' : '') + '">' +
      ...
      (roleCanInvite ? '<details class="wo-contract-setup" ' + (!contracts.length ? 'open' : '') + '><summary>' + icon('plus') + 'ディーラーを追加</summary><form id="dealer-invite-form" class="wo-inline-form"><label><span>ディーラー名</span><input name="dealerName" required maxlength="140" placeholder="例：きくや美粧堂"></label><label><span>ログインID</span><input name="loginId" required maxlength="100" autocomplete="off" placeholder="dealer@example.jp"></label><label><span>メールアドレス（任意）</span><input name="email" type="email" maxlength="200" placeholder="order@example.jp"></label><label><span>電話番号（任意）</span><input name="phone" maxlength="40" inputmode="tel"></label><button class="wo-button wo-button-primary" type="submit">' + icon('link') + '連携を依頼</button></form><div id="dealer-invite-result"></div></details>' : '') +
      '</section>'
  }
