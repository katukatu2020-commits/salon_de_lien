  function dealerFilterOptionsV659(values, selected, emptyLabel) {
    return '<option value="">' + esc(emptyLabel) + '</option>' + values.map(function (value) {
  ...
  function syncDealerProductUrl() {
    ...
    url.searchParams.delete('productManufacturer')
    ...
    if (dealer.productManufacturer) url.searchParams.set('productManufacturer', dealer.productManufacturer)
    if (dealer.productCategory) url.searchParams.set('productCategory', dealer.productCategory)
  ...
  function dealerProducts() {
    const pagination = dealer.data.productPagination || {
      ...
      manufacturer: dealer.productManufacturer,
      category: dealer.productCategory,
    }
    ...
    const hasFilters = Boolean(dealer.productQuery || dealer.productManufacturer || dealer.productCategory)
    ...
    return '<section class="wo-workspace wo-product-management"><header class="wo-workspace-head"><div><p class="wo-section-label">DEALER CATALOG</p><h2>商品を登録</h2><p>登録後、「契約価格」で美容室ごとの取扱と割引率を設定すると発注画面へ公開されます。</p></div><span class="wo-count-chip">' + totalCount.toLocaleString('ja-JP') + '商品</span></header><form id="dealer-product-form" class="wo-dealer-product-form">' + dealerProductFields() + '<div class="wo-form-actions"><button class="wo-button wo-button-primary" type="submit">' + icon('plus') + '商品を登録</button></div></form>' +
      '<div class="wo-product-toolbar wo-catalog-toolbar-v659"><label class="wo-search">' + icon('search') + '<input id="dealer-product-search" value="' + esc(dealer.productQuery) + '" placeholder="商品名・メーカー・商品コードで検索" autocomplete="off" enterkeyhint="search"></label><div class="wo-product-filters-v659"><label><span>メーカー</span><select id="dealer-product-manufacturer-filter">' + dealerFilterOptionsV659(filterOptions.manufacturers || [], dealer.productManufacturer, 'すべてのメーカー') + '</select></label><label><span>種別</span><select id="dealer-product-category-filter">' + dealerFilterOptionsV659(filterOptions.categories || [], dealer.productCategory, 'すべての種別') + '</select></label><button class="wo-filter-reset-v659" type="button" data-action="reset-product-filters" ' + (!hasFilters ? 'disabled' : '') + '>リセット</button></div><span class="wo-product-result-v628" role="status">' + resultSummary + '</span></div><div class="wo-product-results-v641">' +
      ...
      }).join('') + '</div>' : emptyState('package', hasFilters ? '条件に一致する商品はありません' : '登録商品はありません', hasFilters ? '検索語や絞り込み条件を変更してください。' : '上のフォームから、最初の商品を登録してください。')) + pager + '</div></section>'
  }
