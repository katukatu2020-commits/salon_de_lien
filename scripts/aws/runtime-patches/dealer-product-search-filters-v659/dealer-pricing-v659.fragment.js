  ...
  function syncDealerPricingUrl() {
    ...
    url.searchParams.delete('pricingManufacturer')
    url.searchParams.delete('pricingCategory')
    ...
    if (dealer.pricingManufacturer) url.searchParams.set('pricingManufacturer', dealer.pricingManufacturer)
    if (dealer.pricingCategory) url.searchParams.set('pricingCategory', dealer.pricingCategory)
  ...
  function dealerPricing() {
    ...
    const pagination = dealer.data.pricingPagination || {
      ...
      manufacturer: dealer.pricingManufacturer,
      category: dealer.pricingCategory,
    }
    ...
    const filterOptions = dealer.data.productFilters || { manufacturers:[], categories:[] }
    const hasFilters = Boolean(dealer.pricingQuery || dealer.pricingManufacturer || dealer.pricingCategory)
    const visibleProductIds = products.map(function (product) { return product.id })
    const allVisibleSelected = visibleProductIds.length > 0 && visibleProductIds.every(function (productId) { return dealer.pricingSelection.has(productId) })
    const selectedCount = dealer.pricingSelection.size
    ...
    const rows = products.map(function (product) {
      ...
      return '<article class="wo-pricing-row" data-pricing-row data-product-id="' + esc(product.id) + '">' +
        '<label class="wo-pricing-select-v659"><input type="checkbox" data-pricing-selected ' + (dealer.pricingSelection.has(product.id) ? 'checked' : '') + ' aria-label="' + esc(product.name) + 'を一括操作の対象に選択"><span></span></label>' +
        ...
        '<label class="wo-pricing-publish-v659"><input type="checkbox" data-pricing-enabled ' + (value.enabled ? 'checked' : '') + ' aria-label="' + esc(product.name) + 'を美容室へ公開"><span class="wo-pricing-visibility ' + (value.enabled ? 'active' : '') + '">' + (value.enabled ? '公開中' : '非公開') + '</span></label></article>'
    }).join('')
    ...
    return '<section class="wo-workspace wo-pricing-management"><header class="wo-workspace-head"><div><p class="wo-section-label">SALON-SPECIFIC PRICING</p><h2>美容室別の取扱・割引設定</h2><p>同じ商品でも、美容室との取り決めに合わせて個別の割引率を設定できます。</p></div><label class="wo-pricing-salon"><span>設定する美容室</span><select id="dealer-pricing-salon">' + contracts.map(function (item) { return '<option value="' + esc(item.id) + '" ' + (item.id === contract.id ? 'selected' : '') + '>' + esc(item.organizationName) + '</option>' }).join('') + '</select><small>' + configuredCount.toLocaleString('ja-JP') + ' / ' + totalProductCount.toLocaleString('ja-JP') + '商品を公開中</small></label></header>' +
      (totalProductCount ? '<div class="wo-pricing-toolbar wo-pricing-toolbar-v659"><label class="wo-search">' + icon('search') + '<input id="dealer-pricing-search" value="' + esc(dealer.pricingQuery) + '" placeholder="商品名・メーカー・商品コードで検索" autocomplete="off" enterkeyhint="search"></label><div class="wo-product-filters-v659"><label><span>メーカー</span><select id="dealer-pricing-manufacturer-filter">' + dealerFilterOptionsV659(filterOptions.manufacturers || [], dealer.pricingManufacturer, 'すべてのメーカー') + '</select></label><label><span>種別</span><select id="dealer-pricing-category-filter">' + dealerFilterOptionsV659(filterOptions.categories || [], dealer.pricingCategory, 'すべての種別') + '</select></label><button class="wo-filter-reset-v659" type="button" data-action="reset-pricing-filters" ' + (!hasFilters ? 'disabled' : '') + '>リセット</button></div><span class="wo-product-result-v628" role="status">' + resultSummary + '</span><div class="wo-pricing-bulk"><div class="wo-pricing-selection-v659"><strong class="wo-pricing-selection-count-v659">' + selectedCount.toLocaleString('ja-JP') + '件選択中</strong><button type="button" data-action="select-visible-pricing">表示中を一括選択</button><button type="button" data-action="clear-pricing-selection" ' + (!selectedCount ? 'disabled' : '') + '>選択解除</button></div><label><span>一括割引率</span><span><input id="dealer-pricing-bulk-rate" type="number" min="0" max="100" step="0.01" inputmode="decimal" value="' + esc(dealer.pricingBulkRate) + '"><b>%</b></span></label><button class="wo-button wo-button-secondary" type="button" data-action="apply-bulk-rate" ' + (!selectedCount ? 'disabled' : '') + '>選択商品へ適用</button></div></div><form id="dealer-pricing-form"><div class="wo-pricing-table"><div class="wo-pricing-head"><label><input type="checkbox" id="dealer-pricing-select-all" ' + (allVisibleSelected ? 'checked' : '') + ' aria-label="表示中の商品をすべて選択"><span></span></label><span>商品</span><span>定価（税抜）</span><span>割引率</span><span>契約単価（税抜）</span><span>公開設定</span></div>' + (rows || '<div class="wo-table-empty">条件に一致する商品はありません。</div>') + '</div>' + pager + '<footer class="wo-pricing-save"><p>' + saveStatus + '</p><button class="wo-button wo-button-primary" type="button" data-action="save-contract-pricing">' + icon('check') + '変更を保存</button></footer></form>' : emptyState('package', '商品がまだ登録されていません', '先に「商品管理」で商品と定価を登録してください。')) + '</section>'
  }
