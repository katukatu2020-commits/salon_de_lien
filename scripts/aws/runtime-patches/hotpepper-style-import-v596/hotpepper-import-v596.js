...
function summary(job, offset = 0) {
  ...
  return {
    id: job.id, status: job.status, sourceUrl: job.sourceUrl, salonName: job.salonName || '', error: job.error || '', fatal: Boolean(job.fatal),
...
function createHotpepperImportService({ prisma, publishing, sessionProvider, fetchRemote = retrieve }) {
  ...
  async function step(db, job, session) {
    if (!['SCANNING', 'PUBLISHING'].includes(job.status)) return
    ...
    if (job.status === 'SCANNING') {
      ...
      if (!job.items.some(i => i.status === 'QUEUED')) job.status = 'DONE'
    }
  ...
  async function operate(input, session) {
    return prisma.$transaction(async db => {
      ...
      if (input.action === 'start') {
        if (input.rightsConfirmed !== true || input.permissionConfirmed !== true) throw failure('自店舗の掲載内容の転載権限・必要な同意と、掲載元の自動取得許諾を確認してください。')
        ...
        job = { id: randomUUID(), status: 'SCANNING', sourceUrl: rootUrl, salonId: target.salonId, pages: [rootUrl], visited: [], items: [], startedAt: Date.now(), actorId: session.userId, rightsConfirmedAt: new Date().toISOString(), permissionConfirmed: true }
      } else {
        if (!job || job.id !== input.jobId) throw failure('取り込み情報が更新されています。画面を開き直してください。', 409)
        if (input.action === 'cancel') { job.status = 'CANCELLED'; job.items = []; job.pages = []; job.error = '' }
