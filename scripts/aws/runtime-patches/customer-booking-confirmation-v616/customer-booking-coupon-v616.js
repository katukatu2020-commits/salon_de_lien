...
function serializeCoupon(row) {
  return {
    ...
    targetMenus: parseTargets(row.targetMenusJson),
...
function createCustomerBookingCouponService({ prisma, sessionProvider }) {
  async function ensureSchema() {
    ...
    await prisma.$executeRawUnsafe(`CREATE OR REPLACE FUNCTION lien_release_appointment_coupon_v366()
      RETURNS trigger AS $$ BEGIN
        IF NEW."status" IN ('キャンセル','キャンセル済み','無断キャンセル','cancelled','canceled','no-show','no_show') THEN NEW."couponIssueId" := NULL; END IF;
        RETURN NEW;
    ...
    if (repeat) {
      const rows = await prisma.$queryRawUnsafe(
        `SELECT a."menu",a."staffName",s."paidAt"
         ...
         WHERE a."customerId"=$1 AND c."organizationId"=$2 AND c."deletedAt" IS NULL
    ...
    const rows = await prisma.$queryRawUnsafe(
      `SELECT ci."id",ci."couponCode",ci."discountRate",ci."targetMenusJson",ci."expiresAt",
       ...
       LEFT JOIN "Appointment" a ON a."couponIssueId"=ci."id"
         AND a."status" NOT IN ('キャンセル','キャンセル済み','無断キャンセル','cancelled','canceled','no-show','no_show')
       WHERE ci."customerId"=$1 AND c."organizationId"=$2 AND c."deletedAt" IS NULL
         AND ci."status"='issued' AND ci."issuedAt"<=CURRENT_TIMESTAMP AND ci."expiresAt">=CURRENT_TIMESTAMP
       ORDER BY ci."expiresAt",ci."createdAt" DESC LIMIT 200`,
      session.customerId,
      session.organizationId,
    )
    const coupons = rows.map(serializeCoupon)
    const coupon = requestedCouponId ? coupons.find(item => item.id === requestedCouponId) || null : null
    return json(res, 200, { previous, coupon, coupons })
  }
    ...
    const result = await prisma.$transaction(async tx => {
      const appointments = await tx.$queryRawUnsafe(
        `SELECT a."id",a."menu",a."note",a."status" FROM "Appointment" a
         ...
         WHERE a."id"=$1 AND a."customerId"=$2 AND c."organizationId"=$3 AND c."deletedAt" IS NULL
           AND a."status" NOT IN ('キャンセル','キャンセル済み','無断キャンセル','cancelled','canceled','no-show','no_show') FOR UPDATE OF a`,
        appointmentId,
        session.customerId,
        session.organizationId,
      )
      ...
      const issues = await tx.$queryRawUnsafe(
        `SELECT ci."id",ci."couponCode",ci."discountRate",ci."targetMenusJson",ci."expiresAt"
         ...
         WHERE ci."id"=$1 AND ci."customerId"=$2 AND c."organizationId"=$3 AND c."deletedAt" IS NULL
           AND ci."status"='issued' AND ci."issuedAt"<=CURRENT_TIMESTAMP AND ci."expiresAt">=CURRENT_TIMESTAMP FOR UPDATE OF ci`,
        couponIssueId,
        session.customerId,
        session.organizationId,
      )
      ...
      const inUse = await tx.$queryRawUnsafe(
        `SELECT "id" FROM "Appointment" WHERE "couponIssueId"=$1 AND "id"<>$2
         AND "status" NOT IN ('キャンセル','キャンセル済み','無断キャンセル','cancelled','canceled','no-show','no_show') LIMIT 1`,
        issue.id,
        appointment.id,
      )
  ...
  async function handle(req, res, url, adminSessionProvider) {
    try {
      ...
      console.error('[customer-booking-coupon-v616]', { error: error instanceof Error ? error.message : String(error) })
...
module.exports = { couponMatchesMenu, createCustomerBookingCouponService }
