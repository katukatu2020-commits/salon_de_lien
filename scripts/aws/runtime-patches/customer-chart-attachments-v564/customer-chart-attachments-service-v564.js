...
function createCustomerChartAttachmentsService({ prisma, crypto, sessionProvider, storageProvider }) {
  ...
  async function handle(req, res, url) {
    const customerId = chartAttachmentRoute(url.pathname)
    ...
    try {
      ...
      if (req.method === 'GET') await listAttachments(res, url, session, customer)
      else if (req.method === 'POST') await uploadAttachment(req, res, session, customer)
      else {
        res.statusCode = 405
