...
$baseTag = "customer-record-merge-scope-v386"
$tag = "shift-staff-availability-v387"
...
if (-not $imageExists) {
  ...
  docker build --pull=false --build-arg BASE_IMAGE="$accountId.dkr.ecr.$region.amazonaws.com/${repository}:$baseTag" -f "scripts/aws/runtime-patches/shift-staff-availability-v387/Dockerfile" -t $image .
...
$taskFile = Join-Path $env:TEMP "salon-v387-task-definition.json"
