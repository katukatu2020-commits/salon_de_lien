...
$tag = 'customer-community-mobile-runtime-v376'
...
if (-not $existingImage.imageDetails) {
  ...
  docker build --pull=false -f scripts/aws/runtime-patches/customer-community-mobile-runtime-v376/Dockerfile -t $image .
...
$taskFile = Join-Path $env:TEMP 'salon-de-lien-task-v376.json'
