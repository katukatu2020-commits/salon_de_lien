...
$Tag = 'store-icon-tenant-isolation-v397'
...
if ($LASTEXITCODE -ne 0) {
  ...
  $DockerConfig = Join-Path $env:TEMP ("salon-ecr-v397-" + [Guid]::NewGuid().ToString('N'))
...
docker build -f scripts/aws/runtime-patches/store-icon-tenant-isolation-v397/Dockerfile -t $Image .
...
$TaskFile = Join-Path $env:TEMP 'salon-de-lien-task-v397.json'
[System.IO.File]::WriteAllText(
  $TaskFile,
