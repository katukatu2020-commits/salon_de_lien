...
const RELEASE = 'shared-account-identity-scope-v552'
    ...
    return await prisma.$transaction(async tx => {
      ...
      if (loginIdChanged) {
        const duplicates = await tx.$queryRawUnsafe(
          'SELECT "id" FROM "AppUser" WHERE "id"<>$2 AND "role"::text IN (\'ADMIN\',\'STAFF\',\'MANUFACTURER\') AND (LOWER(COALESCE("loginId",\'\'))=$1 OR LOWER(COALESCE("email",\'\'))=$1) LIMIT 1',
          normalizedLoginId,
          currentId,
        )
