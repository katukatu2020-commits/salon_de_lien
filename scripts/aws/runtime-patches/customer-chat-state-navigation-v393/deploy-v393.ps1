...
$Tag = 'customer-chat-state-navigation-v393'
...
if ($LASTEXITCODE -ne 0) {
  ...
  $DockerConfig = Join-Path $env:TEMP ("salon-ecr-v393-" + [Guid]::NewGuid().ToString('N'))
...
docker build -f scripts/aws/runtime-patches/customer-chat-state-navigation-v393/Dockerfile -t $Image .
...
$TaskFile = Join-Path $env:TEMP 'salon-de-lien-task-v393.json'
[System.IO.File]::WriteAllText(
  $TaskFile,
