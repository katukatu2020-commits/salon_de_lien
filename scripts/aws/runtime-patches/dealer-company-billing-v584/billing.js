...
function stripeConfig(env = process.env) {
  ...
  const dealerPrice = String(env.STRIPE_PRICE_DEALER || env.DEALER_STRIPE_PRICE_ID || prices.take || '').trim()
...
function postmarkMailConfig() {
  ...
  const fromName = String(process.env.POSTMARK_FROM_NAME || 'ORIMIA').trim()
...
async function sendRegistrationVerificationMail(input) {
  const subject = 'ORIMIA 店舗登録用メールアドレス確認'
  const body = [
    'ORIMIAの店舗登録を開始するには、以下のリンクを開いてください。',
    '',
...
function pageShell(title, content, session, options = {}) {
  ...
  const organization = htmlEscape(options.organizationName || 'ORIMIA')
  const setup = Boolean(options.setup)
  const navigation = session
    ? '<aside class="side"><div class="brand"><span class="brandMark">S</span><div><strong>ORIMIA</strong><small>店舗管理</small></div></div>' +
      ...
      (setup
        ...
        : '<nav><a href="/admin/appointments">予約・シフト・会計</a><a href="/admin/customers">顧客・カルテ・配信</a><a href="/admin/products">メニュー・商品・在庫</a><a href="/admin/owner-analytics?section=billing" class="' + (options.billing ? 'current' : '') + '">経営・会計管理</a></nav>') +
  ...
  return '<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">' +
    '<meta name="robots" content="noindex,nofollow"><meta name="color-scheme" content="light"><title>' + htmlEscape(title) + ' | ORIMIA</title>' +
    ...
    '<body><div class="layout">' + navigation + '<main class="main"><header class="top"><div class="topBrand"><strong>ORIMIA</strong><small>' + (session ? organization : '店舗運営を、もっと美しく。') + '</small></div>' + (account ? '<span class="account">' + account + '</span>' : '<a class="btn secondary" href="/admin/login">店舗ログイン</a>') + '</header><div class="content">' + content + '</div></main></div></body></html>'
}
...
function createBillingService(dependencies) {
  ...
  function getStripe(config) {
    ...
    if (!stripeClient) {
      ...
      stripeClient = new Stripe(config.secretKey, {
        appInfo: { name: 'ORIMIA Billing', version: '1.0.0' },
        maxNetworkRetries: 2,
        timeout: 20000,
      })
    }
  ...
  async function verifiedPrice(stripe, billing, config) {
    ...
    if (!correct) {
      const error = new Error('Stripe Price does not match the ORIMIA plan')
  ...
  async function locateDealerBillingForSubscription(subscription) {
    const customerId = stripeObjectId(subscription.customer)
    const metadataDealerId = subscription.metadata && subscription.metadata.dealerId
    if (metadataDealerId) {
      const billing = await getDealerBilling(metadataDealerId)
      if (billing && (!billing.stripeCustomerId || billing.stripeCustomerId === customerId)) return billing
    }
    const rows = await prisma.$queryRawUnsafe(`SELECT b.*,d."name" AS "dealerName",d."email" AS "dealerEmail"
      FROM "WholesaleDealerBilling" b JOIN "WholesaleDealer" d ON d."id"=b."dealerId"
      WHERE b."stripeSubscriptionId"=$1 OR b."stripeCustomerId"=$2 LIMIT 1`, subscription.id, customerId || '')
    return rows[0] || null
  }

  async function syncDealerSubscription(stripe, subscription, config, eventCreatedAt) {
    const billing = await locateDealerBillingForSubscription(subscription)
    ...
    const item = subscription.items && subscription.items.data && subscription.items.data[0]
    const priceId = item && stripeObjectId(item.price)
    if (priceId !== config.dealerPrice || (subscription.metadata && subscription.metadata.planKey && subscription.metadata.planKey !== 'dealer')) {
      throw Object.assign(new Error('Stripe subscription contains an unrecognized dealer price'), { code: 'UNKNOWN_DEALER_PRICE' })
    }
    const period = subscriptionPeriod(subscription)
    const payment = await paymentMethodSummary(stripe, subscription)
    const status = String(subscription.status || 'none')
    ...
    const lastEventAt = eventCreatedAt ? dateFromUnix(eventCreatedAt) : new Date()
    await prisma.$executeRawUnsafe(
      'UPDATE "WholesaleDealerBilling" SET "planKey"=\'dealer\',"onboardingStatus"=$2,"subscriptionStatus"=$3,"stripeCustomerId"=COALESCE("stripeCustomerId",$4),"stripeSubscriptionId"=$5,"trialStartedAt"=$6,"trialEndsAt"=$7,"trialUsedAt"=CASE WHEN $6::timestamptz IS NOT NULL THEN COALESCE("trialUsedAt",$6) ELSE "trialUsedAt" END,"currentPeriodStart"=$8,"currentPeriodEnd"=$9,"cancelAtPeriodEnd"=$10,"canceledAt"=$11,"paymentMethodBrand"=COALESCE($12,"paymentMethodBrand"),"paymentMethodLast4"=COALESCE($13,"paymentMethodLast4"),"paymentMethodExpMonth"=COALESCE($14,"paymentMethodExpMonth"),"paymentMethodExpYear"=COALESCE($15,"paymentMethodExpYear"),"paymentMethodRegisteredAt"=CASE WHEN $13::text IS NOT NULL THEN COALESCE("paymentMethodRegisteredAt",NOW()) ELSE "paymentMethodRegisteredAt" END,"lastStripeEventAt"=$16,"updatedAt"=NOW() WHERE "dealerId"=$1',
      billing.dealerId,
      billingOnboardingStatus(status),
      status,
      stripeCustomerId,
  ...
  async function syncAnySubscription(stripe, subscription, config, eventCreatedAt) {
    const metadata = subscription.metadata || {}
    if (metadata.billingAccountType === 'dealer' || metadata.dealerId) {
      return { type: 'dealer', id: await syncDealerSubscription(stripe, subscription, config, eventCreatedAt) }
    }
    const organizationBilling = await locateBillingForSubscription(subscription)
    if (organizationBilling) return { type: 'organization', id: await syncSubscription(stripe, subscription, config, eventCreatedAt) }
    const dealerBilling = await locateDealerBillingForSubscription(subscription)
    if (dealerBilling) return { type: 'dealer', id: await syncDealerSubscription(stripe, subscription, config, eventCreatedAt) }
    throw Object.assign(new Error('No billing account matches the Stripe subscription'), { code: 'BILLING_ACCOUNT_NOT_FOUND' })
  }

  async function syncAnySubscriptionById(stripe, subscriptionId, config, eventCreatedAt) {
    if (!subscriptionId) return null
    const subscription = await stripe.subscriptions.retrieve(subscriptionId, {
      expand: ['default_payment_method', 'items.data.price'],
    })
    return syncAnySubscription(stripe, subscription, config, eventCreatedAt)
  }
  ...
  async function processWebhookEvent(stripe, event, config) {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const dealerId = object.metadata && object.metadata.dealerId
      if ((object.metadata && object.metadata.billingAccountType === 'dealer') || dealerId) {
        if (!dealerId) throw Object.assign(new Error('Dealer Checkout metadata is missing dealerId'), { code: 'CHECKOUT_METADATA_MISSING' })
        const dealerBilling = await getDealerBilling(dealerId)
        if (!dealerBilling) throw Object.assign(new Error('Checkout dealer is not billing enabled'), { code: 'DEALER_BILLING_NOT_FOUND' })
        if (dealerBilling.stripeCustomerId && dealerBilling.stripeCustomerId !== stripeObjectId(object.customer)) throw Object.assign(new Error('Checkout customer does not match dealer'), { code: 'CUSTOMER_MISMATCH' })
        if (object.metadata.planKey !== 'dealer') throw Object.assign(new Error('Checkout plan does not match the dealer plan'), { code: 'CHECKOUT_PLAN_MISMATCH' })
        await prisma.$executeRawUnsafe(
          'UPDATE "WholesaleDealerBilling" SET "stripeCustomerId"=COALESCE("stripeCustomerId",$2),"stripeCheckoutSessionId"=$3,"updatedAt"=NOW() WHERE "dealerId"=$1',
          dealerId,
          stripeObjectId(object.customer),
          object.id
        )
        await syncAnySubscriptionById(stripe, stripeObjectId(object.subscription), config, event.created)
        return
      }
    ...
    if (['customer.subscription.created', 'customer.subscription.updated'].includes(event.type)) {
      await syncAnySubscriptionById(stripe, object.id, config, event.created)
      return
    }
    if (event.type === 'customer.subscription.deleted') {
      await syncAnySubscription(stripe, object, config, event.created)
      return
    }
    if (['invoice.payment_succeeded', 'invoice.payment_failed'].includes(event.type)) {
      await syncAnySubscriptionById(stripe, invoiceSubscriptionId(object), config, event.created)
      return
    }
    if (event.type === 'customer.subscription.trial_will_end') {
      const account = await syncAnySubscriptionById(stripe, object.id, config, event.created)
      if (account && account.type === 'dealer') {
        await prisma.$executeRawUnsafe(
          'UPDATE "WholesaleDealerBilling" SET "trialEndingNoticeAt"=COALESCE("trialEndingNoticeAt",NOW()),"updatedAt"=NOW() WHERE "dealerId"=$1',
          account.id
        )
      } else if (account) {
        await prisma.$executeRawUnsafe(
  ...
  async function onboardingPage(req, res, url) {
    ...
    const content = onboardingSteps(currentStep) + '<section class="hero"><span class="eyebrow">WELCOME TO ORIMIA</span><h1>' + (started ? 'セットアップが完了しました' : '最後に、お支払い方法を登録') + '</h1><p><strong>' + htmlEscape(billing.organizationName) + '</strong> の店舗専用環境です。別の端末や別店舗のログインで、この店舗の契約・データが切り替わることはありません。</p><div style="margin-top:18px">' + configurationNotice + '</div></section>' +
  ...
  function maskSystemBillingAmounts(html) { /* billing-display-mask-v531 */
  ...
  async function billingPage(req, res) {
    ...
    if (!billing) {
      ...
      sendHtml(res, 200, pageShell('システム利用料', maskSystemBillingAmounts(content), session, { billing: true }))
      return
    }
