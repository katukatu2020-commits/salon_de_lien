...
const { validDealerRequestOrigin } = require('./dealer-origin-policy-v554') /* dealer-form-origin-v554 */
const { sendDealerAuthPostmark } = require('./dealer-auth-postmark-v555') /* dealer-auth-postmark-v555 */
const { dealerAuthMailHtml } = require('./dealer-auth-mail-content-v556') /* dealer-auth-postmark-html-v556 */
...
const DEALER_BILLING_ALLOWED_STATUSES = new Set(['trialing', 'active', 'past_due'])
const DEALER_PLAN_KEY = 'dealer'
const DEALER_OPERATIONS_RELEASE = 'dealer-company-billing-v584'
...
function dealerBillingConfig(env = process.env) {
  const secretKey = String(env.STRIPE_SECRET_KEY || '').trim()
  const webhookSecret = String(env.STRIPE_WEBHOOK_SECRET || '').trim()
  const isLive = secretKey.startsWith('sk_live_')
  const allowLive = env.STRIPE_ALLOW_LIVE_BILLING === 'true'
  const enabled = String(env.DEALER_BILLING_ONBOARDING_ENABLED || env.BILLING_ONBOARDING_ENABLED || '') === 'true'
  const priceId = String(env.STRIPE_PRICE_DEALER || env.DEALER_STRIPE_PRICE_ID || env.STRIPE_PRICE_TAKE || '').trim()
  const appUrl = String(env.APP_URL || env.NEXT_PUBLIC_APP_URL || '').replace(/\/+$/, '')
  const errors = []
  if (!enabled) errors.push('dealer billing onboarding is disabled')
  ...
  if (!priceId.startsWith('price_')) errors.push('the dealer Stripe Price is not configured')
  ...
  return {
    ...
    priceId,
    appUrl,
    monthlyAmount: positiveInteger(env.DEALER_MONTHLY_AMOUNT, 9800, 0, 10000000),
    trialDays: positiveInteger(env.DEALER_SUBSCRIPTION_TRIAL_DAYS || env.SUBSCRIPTION_TRIAL_DAYS, 30, 1, 365),
    portalConfiguration: String(env.STRIPE_PORTAL_CONFIGURATION_ID || '').trim() || null,
    mode: isLive ? 'live' : 'test',
  }
}

function buildDealerCheckoutParams(input) {
  return {
    mode: 'subscription',
    customer: input.customerId,
    client_reference_id: input.dealerId,
    ...
    locale: 'ja',
    success_url: input.appUrl + '/dealer/billing?checkout=success&session_id={CHECKOUT_SESSION_ID}',
    cancel_url: input.appUrl + '/dealer/billing?checkout=cancelled',
  }
}
...
function dealerBillingOnboardingStatus(status) {
  if (status === 'trialing') return 'TRIALING'
  if (status === 'active') return 'ACTIVE'
  if (status === 'past_due') return 'PAYMENT_ATTENTION'
  if (['unpaid', 'canceled', 'paused', 'incomplete_expired'].includes(status)) return 'BLOCKED'
  return 'PAYMENT_REQUIRED'
}

function dealerBillingStatusLabel(status) {
  return {
    none: 'カード未登録',
    incomplete: '登録未完了',
...
function percentage(value, label) {
  ...
  if (!Number.isFinite(number) || number < 0 || number > 100) {
    throw new WholesaleError(label + 'は0〜100%で入力してください。')
  }
...
function newDealerCode(crypto) {
  return 'DLR-' + crypto.randomBytes(5).toString('hex').toUpperCase()
}
...
function adminOrderingPage(actor) {
  ...
  <script src="/wholesale-ordering-client-v543.js?v=576" defer></script></body></html>`
}
...
function dealerPortalPage(dealer, initialView = 'orders') {
  const pages = {
    orders: { title: '受注管理', eyebrow: 'ORDER OPERATIONS', description: '契約美容室から届いた注文を確認し、受注・出荷・納品まで更新します。' },
    salons: { title: '契約美容室', eyebrow: 'SALON CONTRACTS', description: '取引中の美容室を登録・確認し、不要になった連携を解除できます。' },
    products: { title: '商品管理', eyebrow: 'DEALER CATALOG', description: '美容室へ提供する商品、JANコード、定価、発注単位を管理します。' },
    pricing: { title: '契約価格', eyebrow: 'CONTRACT PRICING', description: '美容室ごとに、取扱商品と定価からの割引率を一括管理します。' },
    company: { title: '会社・店舗情報', eyebrow: 'COMPANY PROFILE', description: '取引先や納品書に使用する、自社の会社・営業所情報を管理します。' },
    billing: { title: 'システム利用料', eyebrow: 'BILLING & PLAN', description: 'Stripeでカードを登録し、ORIMIA Partnerの契約状況を管理します。' },
  }
  const current = pages[initialView] || pages.orders
  const nav = [
  ...
  const navMarkup = nav.map(item => `<a class="${initialView === item[0] ? 'active' : ''}" href="${item[1]}">${icon(item[2])}<span>${item[3]}</span></a>`).join('')
  return `${sharedHead('ディーラー ' + current.title)}<body class="wo-body wo-dealer-body" data-wholesale-page="dealer" data-dealer-view="${escapeHtml(initialView)}"><div class="wo-dealer-layout"><aside class="wo-dealer-sidebar"><a class="wo-brand" href="/dealer/orders"><img src="/brand/orimia-icon-192.png" alt=""><span><strong>ORIMIA Partner</strong><small>Dealer operations</small></span></a><nav aria-label="ディーラー管理">${navMarkup}</nav><form method="post" action="/api/dealer/auth/logout"><button type="submit">${icon('logout')}<span>ログアウト</span></button></form></aside><div class="wo-dealer-stage"><header class="wo-dealer-topbar"><div><small>DEALER PORTAL</small><strong>${escapeHtml(current.title)}</strong></div><span>${escapeHtml(dealer.name)}</span></header><main class="wo-main"><section class="wo-page-head"><div><p class="wo-eyebrow">${escapeHtml(current.eyebrow)}</p><h1>${escapeHtml(current.title)}</h1><p>${escapeHtml(current.description)}</p></div></section><div id="wholesale-app" class="wo-app-root"><div class="wo-loading"><span></span><p>管理情報を読み込んでいます</p></div></div></main></div></div><nav class="wo-dealer-mobile-nav" aria-label="ディーラー管理">${navMarkup}</nav><script src="/wholesale-ordering-client-v543.js?v=584-company-billing1" defer></script></body></html>`
}
...
function createWholesaleOrderingService({ prisma, crypto, adminSessionProvider, dealerAuthMailSender = sendDealerAuthMail }) {
  ...
  async function ensureSchema() {
    ...
    schemaPromise = (async () => {
      ...
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "dealerCode" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "storeName" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "representativeName" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "invoiceRegistrationNumber" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "prefecture" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "city" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "addressLine1" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "addressLine2" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "websiteUrl" TEXT')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleDealer" ADD COLUMN IF NOT EXISTS "businessHours" TEXT')
      await prisma.$executeRawUnsafe('UPDATE "WholesaleDealer" SET "addressLine1"="address" WHERE NULLIF(BTRIM(COALESCE("addressLine1",\'\')),\'\') IS NULL AND NULLIF(BTRIM(COALESCE("address",\'\')),\'\') IS NOT NULL')
      await prisma.$executeRawUnsafe(`UPDATE "WholesaleDealer" SET "dealerCode"='DLR-' || UPPER(SUBSTRING(MD5("id") FROM 1 FOR 10)) WHERE "dealerCode" IS NULL OR BTRIM("dealerCode")=''`)
      await prisma.$executeRawUnsafe('CREATE UNIQUE INDEX IF NOT EXISTS "WholesaleDealer_dealerCode_lower_key" ON "WholesaleDealer"(LOWER("dealerCode")) WHERE "dealerCode" IS NOT NULL')
      await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "WholesaleDealerBilling" (
        "dealerId" TEXT PRIMARY KEY REFERENCES "WholesaleDealer"("id") ON DELETE CASCADE,
        "planKey" TEXT NOT NULL DEFAULT 'dealer',
        "displayName" TEXT NOT NULL DEFAULT 'ディーラープラン',
        "monthlyAmount" INTEGER NOT NULL DEFAULT 9800,
        ...
        "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        "updatedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK ("monthlyAmount" BETWEEN 0 AND 10000000)
      )`)
      await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "WholesaleDealerBilling_status_idx" ON "WholesaleDealerBilling"("subscriptionStatus","updatedAt")')
      const billingConfig = dealerBillingConfig()
      await prisma.$executeRawUnsafe(`INSERT INTO "WholesaleDealerBilling" ("dealerId","planKey","displayName","monthlyAmount","currency","onboardingStatus","subscriptionStatus","billingRequiredAt","createdAt","updatedAt")
        SELECT d."id",$1,'ディーラープラン',$2,'jpy','PAYMENT_REQUIRED','none',NOW(),NOW(),NOW()
        FROM "WholesaleDealer" d WHERE d."active"=TRUE
      ...
      await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "WholesaleDealerProduct" (
        "id" TEXT PRIMARY KEY,
        "dealerId" TEXT NOT NULL REFERENCES "WholesaleDealer"("id") ON DELETE CASCADE,
        ...
        "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        "updatedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE("dealerId", "productCode"),
        CHECK ("wholesalePrice" BETWEEN 0 AND 10000000),
        CHECK ("suggestedRetailPrice" IS NULL OR "suggestedRetailPrice" BETWEEN 0 AND 10000000),
        CHECK ("orderUnit" BETWEEN 1 AND 999)
      )`)
      await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "WholesaleDealerProduct_dealer_active_idx" ON "WholesaleDealerProduct"("dealerId","active","manufacturerName","name")')
      await prisma.$executeRawUnsafe(`CREATE TABLE IF NOT EXISTS "WholesaleContractProductPrice" (
        "id" TEXT PRIMARY KEY,
        "contractId" TEXT NOT NULL REFERENCES "WholesaleDealerContract"("id") ON DELETE CASCADE,
        "dealerProductId" TEXT NOT NULL REFERENCES "WholesaleDealerProduct"("id") ON DELETE CASCADE,
        "discountRate" NUMERIC(5,2) NOT NULL,
        "active" BOOLEAN NOT NULL DEFAULT TRUE,
        "createdAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        "updatedAt" TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        UNIQUE("contractId", "dealerProductId"),
        CHECK ("discountRate" BETWEEN 0 AND 100)
      )`)
      await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "WholesaleContractProductPrice_contract_active_idx" ON "WholesaleContractProductPrice"("contractId","active","dealerProductId")')
      await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "WholesaleContractProductPrice_product_idx" ON "WholesaleContractProductPrice"("dealerProductId","active")')
      ...
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleOrderLine" ADD COLUMN IF NOT EXISTS "dealerProductId" TEXT REFERENCES "WholesaleDealerProduct"("id") ON DELETE SET NULL')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleOrderLine" ADD COLUMN IF NOT EXISTS "listPrice" INTEGER')
      await prisma.$executeRawUnsafe('ALTER TABLE "WholesaleOrderLine" ADD COLUMN IF NOT EXISTS "discountRate" NUMERIC(5,2)')
      await prisma.$executeRawUnsafe(`DO $migration$ BEGIN
        ALTER TABLE "WholesaleOrderLine" ADD CONSTRAINT "WholesaleOrderLine_listPrice_check" CHECK ("listPrice" IS NULL OR "listPrice" BETWEEN 0 AND 10000000) NOT VALID;
      EXCEPTION WHEN duplicate_object THEN NULL; END $migration$`)
      await prisma.$executeRawUnsafe(`DO $migration$ BEGIN
        ALTER TABLE "WholesaleOrderLine" ADD CONSTRAINT "WholesaleOrderLine_discountRate_check" CHECK ("discountRate" IS NULL OR "discountRate" BETWEEN 0 AND 100) NOT VALID;
      EXCEPTION WHEN duplicate_object THEN NULL; END $migration$`)
      await prisma.$executeRawUnsafe('CREATE INDEX IF NOT EXISTS "WholesaleOrderLine_dealer_product_idx" ON "WholesaleOrderLine"("dealerProductId") WHERE "dealerProductId" IS NOT NULL')
  ...
  async function dealerSession(req) {
    ...
    const rows = await prisma.$queryRawUnsafe('SELECT "id","name","storeName","representativeName","invoiceRegistrationNumber","loginId","email","phone","postalCode","prefecture","city","addressLine1","addressLine2","address","websiteUrl","businessHours","dealerCode","authVersion" FROM "WholesaleDealer" WHERE "id"=$1 AND "active"=TRUE LIMIT 1', payload.dealerId)
    if (!rows[0] || Number(payload.authVersion || 1) !== Number(rows[0].authVersion || 1)) return null
    return { ...payload, ...rows[0] }
  }

  async function ensureDealerBillingRow(dealerId, tx = prisma) {
    const config = dealerBillingConfig()
    await tx.$executeRawUnsafe(`INSERT INTO "WholesaleDealerBilling" ("dealerId","planKey","displayName","monthlyAmount","currency","onboardingStatus","subscriptionStatus","billingRequiredAt","createdAt","updatedAt")
      VALUES ($1,$2,'ディーラープラン',$3,'jpy','PAYMENT_REQUIRED','none',NOW(),NOW(),NOW())
      ON CONFLICT ("dealerId") DO NOTHING`, dealerId, DEALER_PLAN_KEY, config.monthlyAmount)
  }

  async function getDealerBilling(dealerId) {
    await ensureDealerBillingRow(dealerId)
    const rows = await prisma.$queryRawUnsafe(`SELECT b.*,d."name" AS "dealerName",d."email" AS "dealerEmail"
      FROM "WholesaleDealerBilling" b JOIN "WholesaleDealer" d ON d."id"=b."dealerId"
      WHERE b."dealerId"=$1 AND d."active"=TRUE LIMIT 1`, dealerId)
    return rows[0] || null
  }
  ...
  async function dealerAccessState(dealerId) {
    const billing = await getDealerBilling(dealerId)
    return { billing, allowed: dealerBillingAllowed(billing) }
  }

  function dealerProfilePayload(dealer) {
    return {
      id: dealer.id,
      companyName: dealer.name || '',
      storeName: dealer.storeName || '',
      representativeName: dealer.representativeName || '',
      invoiceRegistrationNumber: dealer.invoiceRegistrationNumber || '',
      loginId: dealer.loginId || '',
      email: dealer.email || '',
      phone: dealer.phone || '',
      ...
      city: dealer.city || '',
      addressLine1: dealer.addressLine1 || dealer.address || '',
  ...
  async function updateDealerProfile(session, payload) {
    const companyName = cleanText(payload.companyName, '会社名', 140, true)
    const storeName = cleanText(payload.storeName, '営業所・店舗名', 140, false)
    const representativeName = cleanText(payload.representativeName, '代表者名', 120, false)
    const invoiceRegistrationNumber = String(payload.invoiceRegistrationNumber || '').toUpperCase().replace(/[\s-]+/g, '')
    if (invoiceRegistrationNumber && !/^T\d{13}$/.test(invoiceRegistrationNumber)) throw new WholesaleError('適格請求書発行事業者の登録番号は、Tから始まる13桁の数字で入力してください。')
    ...
    const prefecture = cleanText(payload.prefecture, '都道府県', 40, false)
    const city = cleanText(payload.city, '市区町村', 100, false)
    const addressLine1 = cleanText(payload.addressLine1, '番地', 180, false)
    const addressLine2 = cleanText(payload.addressLine2, '建物名・部屋番号', 180, false)
    const websiteUrl = cleanText(payload.websiteUrl, 'Webサイト', 300, false)
    if (websiteUrl) {
      let parsed
      try { parsed = new URL(websiteUrl) } catch { throw new WholesaleError('Webサイトは https:// から始まるURLで入力してください。') }
      if (!['http:', 'https:'].includes(parsed.protocol)) throw new WholesaleError('Webサイトは http:// または https:// から始まるURLで入力してください。')
    }
    const businessHours = cleanText(payload.businessHours, '営業時間', 200, false)
    const conflicts = await prisma.$queryRawUnsafe('SELECT "id" FROM "WholesaleDealer" WHERE "id"<>$1 AND LOWER(COALESCE("email",\'\'))=LOWER($2) LIMIT 1', session.id, email)
    if (conflicts[0]) throw new WholesaleError('このメールアドレスは別のディーラーアカウントで使用されています。', 409)
    const legacyAddress = [prefecture, city, addressLine1, addressLine2].filter(Boolean).join(' ')
    const rows = await prisma.$queryRawUnsafe(`UPDATE "WholesaleDealer" SET
      "name"=$2,"storeName"=NULLIF($3,''),"representativeName"=NULLIF($4,''),"invoiceRegistrationNumber"=NULLIF($5,''),
      "email"=$6,"phone"=NULLIF($7,''),"postalCode"=NULLIF($8,''),"prefecture"=NULLIF($9,''),"city"=NULLIF($10,''),
      "addressLine1"=NULLIF($11,''),"addressLine2"=NULLIF($12,''),"address"=NULLIF($13,''),"websiteUrl"=NULLIF($14,''),
      "businessHours"=NULLIF($15,''),"updatedAt"=NOW() WHERE "id"=$1 AND "active"=TRUE
      RETURNING "id","name","storeName","representativeName","invoiceRegistrationNumber","loginId","email","phone","postalCode","prefecture","city","addressLine1","addressLine2","address","websiteUrl","businessHours","dealerCode"`,
      session.id, companyName, storeName, representativeName, invoiceRegistrationNumber, email, phone, postalCode, prefecture, city, addressLine1, addressLine2, legacyAddress, websiteUrl, businessHours)
    if (!rows[0]) throw new WholesaleError('会社情報を更新できませんでした。', 404)
    return dealerProfilePayload(rows[0])
  }

  function getDealerStripe(config) {
    if (!config.ready) throw Object.assign(new Error('Dealer Stripe billing is not configured'), { code: 'BILLING_NOT_CONFIGURED' })
    if (!dealerStripeClient) {
      const Stripe = require('stripe')
      dealerStripeClient = new Stripe(config.secretKey, {
        appInfo: { name: 'ORIMIA Partner Billing', version: '1.0.0' },
        maxNetworkRetries: 2,
        timeout: 20000,
      })
    }
    return dealerStripeClient
  }

  async function verifiedDealerPrice(stripe, billing, config) {
    const price = await stripe.prices.retrieve(config.priceId)
    const correct = price.active && price.currency === String(billing.currency || 'jpy').toLowerCase() &&
      Number(price.unit_amount) === Number(billing.monthlyAmount) && price.recurring && price.recurring.interval === 'month'
    if (!correct) throw Object.assign(new Error('Stripe Price does not match the dealer plan'), { code: 'PRICE_CONFIGURATION_MISMATCH' })
    return price
  }

  async function ensureDealerStripeCustomer(stripe, billing) {
    ...
    const customer = await stripe.customers.create({
      name: billing.dealerName,
      email: billing.dealerEmail || undefined,
      metadata: { billingAccountType: 'dealer', dealerId: billing.dealerId, dealerName: billing.dealerName },
    }, { idempotencyKey: 'orimia-dealer-customer-' + billing.dealerId })
    await prisma.$executeRawUnsafe('UPDATE "WholesaleDealerBilling" SET "stripeCustomerId"=$2,"updatedAt"=NOW() WHERE "dealerId"=$1 AND "stripeCustomerId" IS NULL', billing.dealerId, customer.id)
    return customer
  }

  async function dealerPaymentMethodSummary(stripe, subscription) {
  ...
  async function syncDealerSubscription(stripe, subscription, config, eventCreatedAt) {
    const customerId = stripeObjectId(subscription.customer)
    const metadataDealerId = subscription.metadata && subscription.metadata.dealerId
    const rows = metadataDealerId
      ? await prisma.$queryRawUnsafe(`SELECT b.*,d."name" AS "dealerName",d."email" AS "dealerEmail" FROM "WholesaleDealerBilling" b JOIN "WholesaleDealer" d ON d."id"=b."dealerId" WHERE b."dealerId"=$1 AND (b."stripeCustomerId" IS NULL OR b."stripeCustomerId"=$2) LIMIT 1`, metadataDealerId, customerId || '')
      : await prisma.$queryRawUnsafe(`SELECT b.*,d."name" AS "dealerName",d."email" AS "dealerEmail" FROM "WholesaleDealerBilling" b JOIN "WholesaleDealer" d ON d."id"=b."dealerId" WHERE b."stripeSubscriptionId"=$1 OR b."stripeCustomerId"=$2 LIMIT 1`, subscription.id, customerId || '')
    const billing = rows[0]
    if (!billing) throw Object.assign(new Error('No dealer billing account matches the Stripe subscription'), { code: 'DEALER_BILLING_NOT_FOUND' })
    const item = subscription.items && subscription.items.data && subscription.items.data[0]
    if (stripeObjectId(item && item.price) !== config.priceId) throw Object.assign(new Error('Stripe subscription contains an unrecognized dealer price'), { code: 'UNKNOWN_DEALER_PRICE' })
    const period = subscriptionPeriod(subscription)
    const payment = await dealerPaymentMethodSummary(stripe, subscription)
    const status = String(subscription.status || 'none')
    await prisma.$executeRawUnsafe(`UPDATE "WholesaleDealerBilling" SET
      "planKey"=$2,"onboardingStatus"=$3,"subscriptionStatus"=$4,"stripeCustomerId"=COALESCE("stripeCustomerId",$5),"stripeSubscriptionId"=$6,
      "trialStartedAt"=$7,"trialEndsAt"=$8,"trialUsedAt"=CASE WHEN $7::timestamptz IS NOT NULL THEN COALESCE("trialUsedAt",$7) ELSE "trialUsedAt" END,
      "currentPeriodStart"=$9,"currentPeriodEnd"=$10,"cancelAtPeriodEnd"=$11,"canceledAt"=$12,
      "paymentMethodBrand"=COALESCE($13,"paymentMethodBrand"),"paymentMethodLast4"=COALESCE($14,"paymentMethodLast4"),
      "paymentMethodExpMonth"=COALESCE($15,"paymentMethodExpMonth"),"paymentMethodExpYear"=COALESCE($16,"paymentMethodExpYear"),
      "paymentMethodRegisteredAt"=CASE WHEN $14::text IS NOT NULL THEN COALESCE("paymentMethodRegisteredAt",NOW()) ELSE "paymentMethodRegisteredAt" END,
      "lastStripeEventAt"=$17,"updatedAt"=NOW() WHERE "dealerId"=$1`,
      billing.dealerId, DEALER_PLAN_KEY, dealerBillingOnboardingStatus(status), status, customerId, subscription.id,
      dateFromUnix(subscription.trial_start), dateFromUnix(subscription.trial_end), period.start, period.end,
      Boolean(subscription.cancel_at_period_end), dateFromUnix(subscription.canceled_at || subscription.ended_at),
      payment && payment.brand, payment && payment.last4, payment && payment.expMonth, payment && payment.expYear,
      eventCreatedAt ? dateFromUnix(eventCreatedAt) : new Date())
    return billing.dealerId
  }

  async function syncDealerSubscriptionById(stripe, subscriptionId, config, eventCreatedAt) {
    if (!subscriptionId) return null
    const subscription = await stripe.subscriptions.retrieve(subscriptionId, { expand: ['default_payment_method', 'items.data.price'] })
    return syncDealerSubscription(stripe, subscription, config, eventCreatedAt)
  }

  function dealerBillingPayload(billing, config) {
    const trialEnd = billing && billing.trialEndsAt ? new Date(billing.trialEndsAt) : null
    return {
      planKey: DEALER_PLAN_KEY,
      planName: billing && billing.displayName || 'ディーラープラン',
      monthlyAmount: Number(billing && billing.monthlyAmount || config.monthlyAmount),
      currency: billing && billing.currency || 'jpy',
      onboardingStatus: billing && billing.onboardingStatus || 'PAYMENT_REQUIRED',
      subscriptionStatus: billing && billing.subscriptionStatus || 'none',
      subscriptionStatusLabel: dealerBillingStatusLabel(billing && billing.subscriptionStatus || 'none'),
      accessAllowed: dealerBillingAllowed(billing),
      trialStartedAt: billing && billing.trialStartedAt || null,
      trialEndsAt: billing && billing.trialEndsAt || null,
      remainingTrialDays: trialEnd ? Math.max(0, Math.ceil((trialEnd.getTime() - Date.now()) / 86400000)) : null,
      currentPeriodStart: billing && billing.currentPeriodStart || null,
      currentPeriodEnd: billing && billing.currentPeriodEnd || null,
      cancelAtPeriodEnd: Boolean(billing && billing.cancelAtPeriodEnd),
      card: billing && billing.paymentMethodLast4 ? {
  ...
  async function refreshDealerBilling(billing, config, checkoutSessionId) {
    if (!billing || !config.ready) return billing
    const stripe = getDealerStripe(config)
    if (billing.stripeSubscriptionId) {
      await syncDealerSubscriptionById(stripe, billing.stripeSubscriptionId, config)
    } else if (billing.stripeCheckoutSessionId && (!checkoutSessionId || checkoutSessionId === billing.stripeCheckoutSessionId)) {
      const checkout = await stripe.checkout.sessions.retrieve(billing.stripeCheckoutSessionId)
      if (checkout.status === 'complete' && stripeObjectId(checkout.subscription)) await syncDealerSubscriptionById(stripe, stripeObjectId(checkout.subscription), config)
    }
    return getDealerBilling(billing.dealerId)
  }

  async function createDealerCheckout(req, session) {
    const config = dealerBillingConfig()
    if (!config.ready) throw new WholesaleError('現在、カード登録画面を開けません。しばらくしてから再度お試しください。', 503)
    let billing = await getDealerBilling(session.id)
    if (billing.trialUsedAt) throw new WholesaleError('このアカウントでは無料トライアルをすでに利用しています。お支払い管理をご確認ください。', 409)
    const stripe = getDealerStripe(config)
    await verifiedDealerPrice(stripe, billing, config)
    const customer = await ensureDealerStripeCustomer(stripe, billing)
    billing = await getDealerBilling(session.id)
    if (billing.stripeSubscriptionId) {
      const subscription = await stripe.subscriptions.retrieve(billing.stripeSubscriptionId, { expand: ['default_payment_method', 'items.data.price'] })
      if (!['canceled', 'incomplete_expired'].includes(subscription.status)) {
        await syncDealerSubscription(stripe, subscription, config)
        throw new WholesaleError('このアカウントにはすでに進行中の契約があります。', 409)
      }
    }
    const subscriptions = await stripe.subscriptions.list({ customer: customer.id, status: 'all', limit: 10 })
    const openSubscription = subscriptions.data.find(entry => !['canceled', 'incomplete_expired'].includes(entry.status))
    if (openSubscription) {
      await syncDealerSubscriptionById(stripe, openSubscription.id, config)
      throw new WholesaleError('このアカウントにはすでに進行中の契約があります。', 409)
    }
    ...
    const checkout = await stripe.checkout.sessions.create(buildDealerCheckoutParams({
      appUrl: config.appUrl || publicOrigin(req), customerId: customer.id, dealerId: billing.dealerId,
      priceId: config.priceId, trialDays: config.trialDays,
    }), { idempotencyKey: 'orimia-dealer-checkout-' + billing.dealerId + '-' + new Date(billing.updatedAt).getTime() })
    await prisma.$executeRawUnsafe(`UPDATE "WholesaleDealerBilling" SET "stripeCustomerId"=$2,"stripeCheckoutSessionId"=$3,"onboardingStatus"='CHECKOUT_OPEN',"updatedAt"=NOW() WHERE "dealerId"=$1 AND "trialUsedAt" IS NULL`, billing.dealerId, customer.id, checkout.id)
    return { url: checkout.url }
  }

  async function createDealerPortal(req, session) {
    const config = dealerBillingConfig()
    if (!config.ready) throw new WholesaleError('現在、お支払い情報を取得できません。しばらくしてから再度お試しください。', 503)
    const billing = await getDealerBilling(session.id)
    if (!billing.stripeCustomerId) throw new WholesaleError('登録済みのお支払い情報がありません。', 409)
    const parameters = { customer: billing.stripeCustomerId, return_url: (config.appUrl || publicOrigin(req)) + '/dealer/billing' }
    if (config.portalConfiguration) parameters.configuration = config.portalConfiguration
    const portal = await getDealerStripe(config).billingPortal.sessions.create(parameters)
  ...
  async function salonBootstrap(session, requestedDealerId) {
    ...
    const catalogProducts = dealerId ? await prisma.$queryRawUnsafe(`SELECT p."id",p."manufacturerName",p."name",p."category",p."productCode",p."janCode",p."orderUnit",p."description",
      c."id" AS "contractId",d."id" AS "dealerId",d."name" AS "dealerName",COALESCE(p."suggestedRetailPrice",p."wholesalePrice")::int AS "listPrice",a."discountRate",
      last_order."quantity" AS "lastOrderQuantity",last_order."orderedAt" AS "lastOrderedAt"
      FROM "WholesaleDealerContract" c
      JOIN "WholesaleDealer" d ON d."id"=c."dealerId" AND d."active"=TRUE
      JOIN "WholesaleContractProductPrice" a ON a."contractId"=c."id" AND a."active"=TRUE
      JOIN "WholesaleDealerProduct" p ON p."id"=a."dealerProductId" AND p."dealerId"=c."dealerId" AND p."active"=TRUE
      LEFT JOIN LATERAL (SELECT l."quantity",o."orderedAt" FROM "WholesaleOrderLine" l JOIN "WholesaleOrder" o ON o."id"=l."orderId" WHERE l."dealerProductId"=p."id" AND o."organizationId"=$1 AND o."dealerId"=$2 AND o."status"<>'CANCELLED' ORDER BY o."orderedAt" DESC LIMIT 1) last_order ON TRUE
    ...
    return {
      ...
      catalogProducts: catalogProducts.map(product => {
        ...
        return { ...product, id: 'dealer:' + product.id, dealerProductId: product.id, source: 'DEALER', listPrice, discountRate, unitPrice, wholesalePrice: unitPrice, suggestedRetailPrice: listPrice, orderUnit: Number(product.orderUnit), stockQuantity: null, lastOrderQuantity: product.lastOrderQuantity == null ? null : Number(product.lastOrderQuantity), imageUrl: '' }
      }),
  ...
  async function createDealerInvite(req, session, payload) {
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      if (!dealer) {
        ...
        const rows = await tx.$queryRawUnsafe('INSERT INTO "WholesaleDealer" ("id","name","loginId","email","phone","dealerCode","active","createdAt","updatedAt") VALUES ($1,$2,$3,NULLIF($4,\'\'),NULLIF($5,\'\'),$6,TRUE,NOW(),NOW()) RETURNING "id","name","loginId","email","passwordHash","dealerCode"', dealerId, dealerName, loginId, email, phone, newDealerCode(crypto))
        dealer = rows[0]
      }
      await ensureDealerBillingRow(dealer.id, tx)
  ...
  async function requestDealerRegistration(req, payload) {
    ...
    if (existing[0]) {
      ...
      try {
        ...
        console.error('[dealer-auth] existing account mail delivery failed', { dealerId: existing[0].id, provider: 'postmark', error: error && error.message || error })
    ...
    try {
      ...
      console.error('[dealer-auth] registration mail delivery failed', { provider: 'postmark', error: error && error.message || error })
      throw new WholesaleError('確認メールを送信できませんでした。時間をおいて再度お試しください。', 503)
    }
    return { sent: true }
  }
  ...
  async function completeDealerRegistration(payload) {
    ...
    await prisma.$transaction(async tx => {
      ...
      await tx.$executeRawUnsafe('INSERT INTO "WholesaleDealer" ("id","name","loginId","email","passwordHash","phone","postalCode","address","dealerCode","active","authVersion","createdAt","updatedAt") VALUES ($1,$2,$3,$4,$5,NULLIF($6,\'\'),NULLIF($7,\'\'),NULLIF($8,\'\'),$9,TRUE,1,NOW(),NOW())', dealerId, dealerName, loginId, tokenRows[0].email, dealerPasswordHash(crypto, password), phone, postalCode, address, newDealerCode(crypto))
      await ensureDealerBillingRow(dealerId, tx)
  ...
  async function requestDealerPasswordReset(req, payload) {
    ...
    try {
      ...
      console.error('[dealer-auth] password reset mail delivery failed', { dealerId: dealer.id, provider: 'postmark', error: error && error.message || error })
    }
    return { sent: true }
  }
  ...
  async function createOrder(session, payload) {
    ...
    const lines = rawLines.map(line => {
      const dealerProductId = cleanText(line.dealerProductId, 'ディーラー商品', 180, true)
      return { dealerProductId, quantity: integer(line.quantity, '発注数', 1, 999) }
    })
    const unique = new Map()
    for (const line of lines) {
      const current = unique.get(line.dealerProductId)
      unique.set(line.dealerProductId, { ...line, quantity: (current?.quantity || 0) + line.quantity })
    }
    ...
    const dealerProductIds = [...unique.keys()]
    const now = new Date()
    ...
    const result = await prisma.$transaction(async tx => {
      const contractRows = await tx.$queryRawUnsafe(`SELECT c."id",d."id" AS "dealerId",d."name" AS "dealerName"
        ...
        JOIN "WholesaleDealer" d ON d."id"=c."dealerId" AND d."active"=TRUE
        WHERE c."organizationId"=$1 AND c."dealerId"=$2 AND c."status"='ACTIVE'
        LIMIT 1 FOR SHARE OF c,d`, session.organizationId, dealerId)
      ...
      const dealerProducts = await tx.$queryRawUnsafe(`SELECT p."id",p."manufacturerName",p."name",p."category",p."productCode",p."janCode",p."orderUnit",
        ...
        FOR SHARE OF a,p`, contractRows[0].id, dealerId, JSON.stringify(dealerProductIds))
      if (dealerProducts.length !== dealerProductIds.length) throw new WholesaleError('ディーラーが取扱設定した商品のみ発注できます。商品一覧を更新してください。', 409)
      ...
      return { dealerName: contractRows[0].dealerName }
    })
    return { id: orderId, orderNo, status: 'ORDERED', dealerName: result.dealerName }
  }

  async function dealerBootstrap(session) {
    const [contracts, orders, products, contractProductPrices] = await Promise.all([
      ...
      prisma.$queryRawUnsafe(`SELECT "id","manufacturerName","name","category","productCode","janCode","wholesalePrice","suggestedRetailPrice","orderUnit","description","active","createdAt","updatedAt" FROM "WholesaleDealerProduct" WHERE "dealerId"=$1 ORDER BY "active" DESC,"manufacturerName","name"`, session.id),
  ...
  async function updateContractProductPricing(session, contractId, payload) {
    ...
    if (rawItems.length > 1000) throw new WholesaleError('一度に設定できる商品は1000件までです。')
    const items = rawItems.map(item => ({
      dealerProductId: cleanText(item.dealerProductId, '商品', 180, true),
      enabled: item.enabled === true,
      discountRate: percentage(item.discountRate, '割引率'),
    }))
    const unique = new Map(items.map(item => [item.dealerProductId, item]))
    if (unique.size !== items.length) throw new WholesaleError('同じ商品が重複しています。')

    return prisma.$transaction(async tx => {
      const contracts = await tx.$queryRawUnsafe('SELECT "id" FROM "WholesaleDealerContract" WHERE "id"=$1 AND "dealerId"=$2 AND "status"=\'ACTIVE\' LIMIT 1 FOR UPDATE', contractId, session.id)
      if (!contracts[0]) throw new WholesaleError('契約中の美容室を確認してください。', 404)
      const productIds = [...unique.keys()]
      if (productIds.length) {
        const products = await tx.$queryRawUnsafe('SELECT "id" FROM "WholesaleDealerProduct" WHERE "dealerId"=$1 AND "active"=TRUE AND "id" IN (SELECT jsonb_array_elements_text($2::jsonb)) FOR UPDATE', session.id, JSON.stringify(productIds))
        if (products.length !== productIds.length) throw new WholesaleError('設定対象の商品を確認してください。', 409)
      }
      for (const item of items) {
        if (item.enabled) {
          await tx.$executeRawUnsafe(`INSERT INTO "WholesaleContractProductPrice" ("id","contractId","dealerProductId","discountRate","active","createdAt","updatedAt")
            VALUES ($1,$2,$3,$4,TRUE,NOW(),NOW())
            ON CONFLICT ("contractId","dealerProductId") DO UPDATE SET "discountRate"=EXCLUDED."discountRate","active"=TRUE,"updatedAt"=NOW()`, 'contract_price_' + crypto.randomUUID(), contractId, item.dealerProductId, item.discountRate)
        } else {
          await tx.$executeRawUnsafe('DELETE FROM "WholesaleContractProductPrice" WHERE "contractId"=$1 AND "dealerProductId"=$2', contractId, item.dealerProductId)
        }
      }
      const totals = await tx.$queryRawUnsafe('SELECT COUNT(*)::int AS count FROM "WholesaleContractProductPrice" WHERE "contractId"=$1 AND "active"=TRUE', contractId)
      return { configuredCount: Number(totals[0]?.count || 0) }
    })
  }

  async function requestDealerConnectionByCode(session, payload) {
    if (session.role !== 'ADMIN') throw new WholesaleError('ディーラー連携はオーナーのみ設定できます。', 403)
    const dealerCode = normalizeDealerCode(cleanText(payload.dealerCode, 'ディーラー固有コード', 40, true))
    const dealers = await prisma.$queryRawUnsafe('SELECT "id","name","dealerCode" FROM "WholesaleDealer" WHERE "active"=TRUE AND LOWER("dealerCode")=LOWER($1) LIMIT 1', dealerCode)
    const dealer = dealers[0]
    if (!dealer) throw new WholesaleError('入力したディーラー固有コードが見つかりません。コードを確認してください。', 404)
    const organizations = await prisma.$queryRawUnsafe('SELECT "publicCode" FROM "Organization" WHERE "id"=$1 LIMIT 1', session.organizationId)
    const customerCode = String(organizations[0]?.publicCode || session.organizationId).slice(0, 80)
    const rows = await prisma.$queryRawUnsafe(`INSERT INTO "WholesaleDealerContract" ("id","dealerId","organizationId","status","customerCode","createdByUserId","createdAt","updatedAt") VALUES ($1,$2,$3,'PENDING',$4,$5,NOW(),NOW())
      ON CONFLICT ("dealerId","organizationId") DO UPDATE SET "status"=CASE WHEN "WholesaleDealerContract"."status"='ACTIVE' THEN 'ACTIVE' ELSE 'PENDING' END,"customerCode"=EXCLUDED."customerCode","createdByUserId"=EXCLUDED."createdByUserId","updatedAt"=NOW()
      RETURNING "id","status","customerCode"`, 'contract_' + crypto.randomUUID(), dealer.id, session.organizationId, customerCode, session.userId)
    return { dealer: { id: dealer.id, name: dealer.name, dealerCode: dealer.dealerCode }, contract: rows[0] }
  }

  async function registerSalonContract(session, payload) {
    const salonCode = cleanText(payload.salonCode, '美容室コード', 80, true).toUpperCase()
    const organizations = await prisma.$queryRawUnsafe('SELECT "id","name","publicCode" FROM "Organization" WHERE LOWER(COALESCE("publicCode",\'\'))=LOWER($1) LIMIT 1', salonCode)
    const organization = organizations[0]
    if (!organization) throw new WholesaleError('入力した美容室コードが見つかりません。美容室へ店舗コードをご確認ください。', 404)
    const rows = await prisma.$queryRawUnsafe(`INSERT INTO "WholesaleDealerContract" ("id","dealerId","organizationId","status","customerCode","approvedAt","createdAt","updatedAt") VALUES ($1,$2,$3,'ACTIVE',$4,NOW(),NOW(),NOW())
      ON CONFLICT ("dealerId","organizationId") DO UPDATE SET "status"='ACTIVE',"customerCode"=EXCLUDED."customerCode","approvedAt"=COALESCE("WholesaleDealerContract"."approvedAt",NOW()),"updatedAt"=NOW()
      RETURNING "id","status","customerCode","approvedAt"`, 'contract_' + crypto.randomUUID(), session.id, organization.id, String(organization.publicCode || salonCode).slice(0, 80))
    return { organization: { id: organization.id, name: organization.name, publicCode: organization.publicCode }, contract: rows[0] }
  }

  async function removeDealerContract(session, contractId, payload) {
    if (payload.confirmed !== true) throw new WholesaleError('確認欄にチェックを入れてください。')
    const result = await prisma.$executeRawUnsafe('UPDATE "WholesaleDealerContract" SET "status"=\'SUSPENDED\',"updatedAt"=NOW() WHERE "id"=$1 AND "dealerId"=$2 AND "status"<>\'SUSPENDED\'', contractId, session.id)
    if (Number(result) !== 1) throw new WholesaleError('削除対象の契約美容室を確認してください。', 404)
    return { removed: true }
  }

  function dealerProductValues(payload) {
    const manufacturerName = cleanText(payload.manufacturerName, 'メーカー名', 140, true)
    const name = cleanText(payload.name, '商品名', 180, true)
    const category = cleanText(payload.category, 'カテゴリ', 100, false)
    const productCode = cleanText(payload.productCode, '商品コード', 100, true).toUpperCase()
    const janCode = cleanText(payload.janCode, 'JANコード', 32, false)
    if (janCode && !/^(?:\d{8}|\d{13})$/.test(janCode)) throw new WholesaleError('JANコードは8桁または13桁の数字で入力してください。')
    const rawWholesalePrice = String(payload.wholesalePrice ?? '').trim()
    const rawListPrice = String(payload.suggestedRetailPrice ?? payload.listPrice ?? '').trim()
    if (!rawWholesalePrice && !rawListPrice) throw new WholesaleError('定価を入力してください。')
    const suggestedRetailPrice = integer(rawListPrice || rawWholesalePrice, '定価', 0, 10000000)
    const wholesalePrice = integer(rawWholesalePrice || rawListPrice, '標準卸価格', 0, 10000000)
    const orderUnit = integer(payload.orderUnit, '発注単位', 1, 999)
    const description = cleanText(payload.description, '商品メモ', 1000, false)
    return { manufacturerName, name, category, productCode, janCode, wholesalePrice, suggestedRetailPrice, orderUnit, description }
  }

  async function createDealerProduct(session, payload) {
    const values = dealerProductValues(payload)
    try {
      const existing = await prisma.$queryRawUnsafe('SELECT "id","active" FROM "WholesaleDealerProduct" WHERE "dealerId"=$1 AND "productCode"=$2 LIMIT 1', session.id, values.productCode)
      if (existing[0]?.active) throw new WholesaleError('同じ商品コードがすでに登録されています。', 409)
      if (existing[0]) {
        const rows = await prisma.$queryRawUnsafe(`UPDATE "WholesaleDealerProduct" SET "manufacturerName"=$3,"name"=$4,"category"=NULLIF($5,''),"janCode"=NULLIF($6,''),"wholesalePrice"=$7,"suggestedRetailPrice"=$8,"orderUnit"=$9,"description"=NULLIF($10,''),"active"=TRUE,"updatedAt"=NOW() WHERE "id"=$1 AND "dealerId"=$2 RETURNING *`, existing[0].id, session.id, values.manufacturerName, values.name, values.category, values.janCode, values.wholesalePrice, values.suggestedRetailPrice, values.orderUnit, values.description)
        return { product: rows[0] }
      }
      const rows = await prisma.$queryRawUnsafe(`INSERT INTO "WholesaleDealerProduct" ("id","dealerId","manufacturerName","name","category","productCode","janCode","wholesalePrice","suggestedRetailPrice","orderUnit","description","active","createdAt","updatedAt") VALUES ($1,$2,$3,$4,NULLIF($5,''),$6,NULLIF($7,''),$8,$9,$10,NULLIF($11,''),TRUE,NOW(),NOW()) RETURNING *`, 'dealer_product_' + crypto.randomUUID(), session.id, values.manufacturerName, values.name, values.category, values.productCode, values.janCode, values.wholesalePrice, values.suggestedRetailPrice, values.orderUnit, values.description)
      return { product: rows[0] }
    } catch (error) {
      if (error?.code === '23505') throw new WholesaleError('同じ商品コードがすでに登録されています。', 409)
      throw error
    }
  }

  async function updateDealerProduct(session, productId, payload) {
    const values = dealerProductValues(payload)
    try {
      const rows = await prisma.$queryRawUnsafe(`UPDATE "WholesaleDealerProduct" SET "manufacturerName"=$3,"name"=$4,"category"=NULLIF($5,''),"productCode"=$6,"janCode"=NULLIF($7,''),"wholesalePrice"=$8,"suggestedRetailPrice"=$9,"orderUnit"=$10,"description"=NULLIF($11,''),"active"=TRUE,"updatedAt"=NOW() WHERE "id"=$1 AND "dealerId"=$2 RETURNING *`, productId, session.id, values.manufacturerName, values.name, values.category, values.productCode, values.janCode, values.wholesalePrice, values.suggestedRetailPrice, values.orderUnit, values.description)
      if (!rows[0]) throw new WholesaleError('更新対象の商品を確認してください。', 404)
      return { product: rows[0] }
    } catch (error) {
      if (error?.code === '23505') throw new WholesaleError('同じ商品コードがすでに登録されています。', 409)
      throw error
  ...
  async function removeDealerProduct(session, productId, payload) {
    if (payload.confirmed !== true) throw new WholesaleError('確認欄にチェックを入れてください。')
    const result = await prisma.$executeRawUnsafe('UPDATE "WholesaleDealerProduct" SET "active"=FALSE,"updatedAt"=NOW() WHERE "id"=$1 AND "dealerId"=$2 AND "active"=TRUE', productId, session.id)
    if (Number(result) !== 1) throw new WholesaleError('削除対象の商品を確認してください。', 404)
    return { removed: true }
  }
  ...
  async function orderDetailForDealer(session, orderId) {
    ...
    const lines = await prisma.$queryRawUnsafe('SELECT "id","productId","dealerProductId","manufacturerName","productName","category","productCode","janCode","listPrice","discountRate","unitPrice","quantity","deliveredQuantity","lineTotal" FROM "WholesaleOrderLine" WHERE "orderId"=$1 ORDER BY "createdAt","id"', orderId)
    return { order: { ...orders[0], taxRate: Number(orders[0].taxRate), subtotalYen: Number(orders[0].subtotalYen), taxYen: Number(orders[0].taxYen), totalYen: Number(orders[0].totalYen) }, lines: lines.map(line => ({ ...line, listPrice: line.listPrice == null ? null : Number(line.listPrice), discountRate: line.discountRate == null ? null : Number(line.discountRate), unitPrice: Number(line.unitPrice), quantity: Number(line.quantity), deliveredQuantity: Number(line.deliveredQuantity), lineTotal: Number(line.lineTotal) })) }
  }
  ...
  async function updateOrderStatus(session, orderId, payload) {
    ...
    const result = await prisma.$transaction(async tx => {
      ...
      for (const submitted of submittedLines) {
        ...
        } else if (current.dealerProductId) {
          await tx.$executeRawUnsafe('UPDATE "WholesaleDealerProduct" SET "productCode"=COALESCE(NULLIF($3,\'\'),"productCode"),"janCode"=NULLIF($4,\'\'),"updatedAt"=NOW() WHERE "id"=$1 AND "dealerId"=$2', current.dealerProductId, session.id, productCode, janCode)
        }
      }
  ...
  async function handle(req, res, url) {
    ...
    if (pathname === '/admin/products/orders' && req.method === 'GET') {
      ...
      return false /* inventory-orders-common-layout-v570-authenticated-shared-shell */
    }

    if (pathname === '/dealer' && req.method === 'GET') {
      const session = await dealerSession(req)
      if (!session) { redirect(res, '/dealer/login', 302); return true }
    ...
    if (pathname === '/dealer/login' && req.method === 'GET') {
      const session = await dealerSession(req)
      if (session) { redirect(res, (await dealerAccessState(session.id)).allowed ? '/dealer/orders' : '/dealer/billing', 302); return true }
    ...
    if (dealerPageMatch && req.method === 'GET') {
      const session = await dealerSession(req)
      if (!session) { redirect(res, '/dealer/login?next=' + encodeURIComponent(pathname), 302); return true }
      ...
      html(res, 200, dealerPortalPage(session, dealerPageMatch[1])); return true
    }
    ...
    if (pathname === '/api/dealer/auth/setup' && req.method === 'POST') {
      ...
      await prisma.$transaction(async tx => {
        ...
        await ensureDealerBillingRow(invite.dealerId, tx)
    ...
    if (adminApi) {
      ...
      try {
        ...
        const payload = await readPayload(req)
        if (pathname === '/api/admin/wholesale/contracts/code') { json(res, 201, { ok: true, ...(await requestDealerConnectionByCode(session, payload)) }); return true }
    ...
    if (dealerApi) {
      ...
      if (!session) { json(res, 401, { ok: false, error: 'ログインし直してください。' }); return true }
      try {
        if (pathname === '/api/dealer/profile' && req.method === 'GET') {
          json(res, 200, { ok: true, profile: dealerProfilePayload(session) }); return true
        }
        if (pathname === '/api/dealer/billing/status' && req.method === 'GET') {
          const config = dealerBillingConfig()
          let billing = await getDealerBilling(session.id)
          if (url.searchParams.get('refresh') === '1') {
            try { billing = await refreshDealerBilling(billing, config, url.searchParams.get('session_id')) }
            catch (error) { console.warn('[dealer-billing] Stripe status refresh unavailable', { dealerId: session.id, error: error && (error.code || error.message) || error }) }
          }
          json(res, 200, { ok: true, billing: dealerBillingPayload(billing, config) }); return true
        }
        if (req.method === 'POST' && ['/api/dealer/profile', '/api/dealer/billing/checkout', '/api/dealer/billing/portal'].includes(pathname)) {
          if (!validSameOrigin(req)) throw new WholesaleError('安全性を確認できないため処理できませんでした。', 403)
          const payload = await readPayload(req)
          if (pathname === '/api/dealer/profile') { json(res, 200, { ok: true, profile: await updateDealerProfile(session, payload) }); return true }
          if (pathname === '/api/dealer/billing/checkout') { json(res, 200, { ok: true, ...(await createDealerCheckout(req, session)) }); return true }
          if (pathname === '/api/dealer/billing/portal') { json(res, 200, { ok: true, ...(await createDealerPortal(req, session)) }); return true }
        }
        const access = await dealerAccessState(session.id)
        if (!access.allowed) {
          json(res, 402, { ok: false, error: 'ORIMIA Partnerを利用するには、クレジットカードの登録が必要です。', billingUrl: '/dealer/billing' }); return true
        }
        ...
        if (!validSameOrigin(req)) throw new WholesaleError('安全性を確認できないため処理できませんでした。', 403)
        const payload = await readPayload(req)
        if (pathname === '/api/dealer/contracts') { json(res, 201, { ok: true, ...(await registerSalonContract(session, payload)) }); return true }
        if (pathname === '/api/dealer/products') { json(res, 201, { ok: true, ...(await createDealerProduct(session, payload)) }); return true }
        ...
        const contractRemoveMatch = pathname.match(/^\/api\/dealer\/contracts\/([^/]+)\/remove$/)
        if (contractRemoveMatch) { json(res, 200, { ok: true, ...(await removeDealerContract(session, decodeURIComponent(contractRemoveMatch[1]), payload)) }); return true }
        const contractPricingMatch = pathname.match(/^\/api\/dealer\/contracts\/([^/]+)\/product-pricing$/)
        if (contractPricingMatch) { json(res, 200, { ok: true, ...(await updateContractProductPricing(session, decodeURIComponent(contractPricingMatch[1]), payload)) }); return true }
        const productUpdateMatch = pathname.match(/^\/api\/dealer\/products\/([^/]+)\/update$/)
        if (productUpdateMatch) { json(res, 200, { ok: true, ...(await updateDealerProduct(session, decodeURIComponent(productUpdateMatch[1]), payload)) }); return true }
        const productRemoveMatch = pathname.match(/^\/api\/dealer\/products\/([^/]+)\/remove$/)
        if (productRemoveMatch) { json(res, 200, { ok: true, ...(await removeDealerProduct(session, decodeURIComponent(productRemoveMatch[1]), payload)) }); return true }
  ...
  return { ensureSchema, handle, adminSession, dealerSession, getDealerBilling, dealerAccessState, salonBootstrap, dealerBootstrap, requestDealerConnectionByCode, registerSalonContract, createDealerProduct, updateContractProductPricing }
}
...
/* dealer-auth-postmark-v555 */

/* dealer-auth-postmark-html-v556 */
