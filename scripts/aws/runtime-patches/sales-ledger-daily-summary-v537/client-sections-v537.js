...
/* render-block:start */
  ...
  function detailRows() {
    if (!state.detailDate) return state.rows
  ...
  function renderSummary(root) {
    ...
    body.querySelectorAll('[data-summary-day]').forEach(button => button.addEventListener('click', () => {
      state.detailDate = button.closest('tr').dataset.summaryDate
      state.selected.clear()
      renderSummary(root)
      renderRows(root)
      root.querySelector('[data-sl-detail-card]').scrollIntoView({ behavior:'smooth', block:'start' })
    }))
  }
  ...
  function renderRows(root) {
    ...
    const detailFilter = root.querySelector('[data-sl-detail-filter]')
    const clearDay = root.querySelector('[data-sl-clear-day]')
    if (state.detailDate) {
      const label = summaryDateLabel(state.detailDate)
      detailFilter.textContent = `${label.date}（${label.weekday}）の決済`
      clearDay.hidden = false
    } else {
      detailFilter.textContent = '表示期間内の全決済'
      clearDay.hidden = true
    }
    if (!rows.length) {
      body.innerHTML = '<tr><td colspan="11" class="sl-empty">条件に一致する会計データがありません。</td></tr>'
      return
    }
...
/* markup-block:start */
  function ledgerMarkup() {
    ...
    return `<div class="sl-page"><nav class="sl-tabs" aria-label="経営ページ切替"><a href="/admin/owner-analytics">経営分析</a><a aria-current="page" class="active" href="/admin/owner-analytics?salesLedger=1">会計データ管理</a><a href="/admin/owner-analytics?section=billing">システム利用料</a></nav>
      ...
      <section class="sl-card sl-table-card sl-detail-card" data-sl-detail-card><div class="sl-table-head"><div class="sl-section-title">${icon('list')}<div><h2>決済明細</h2><span class="sl-status"><span data-sl-detail-filter>表示期間内の全決済</span> ・ <span data-sl-count>0件</span> / <span data-sl-selected>0件選択</span></span></div></div><div class="sl-action-group"><button class="sl-button" type="button" data-sl-clear-day hidden>${icon('close')}日付指定を解除</button><button class="sl-button" type="button" data-sl-bulk disabled>${icon('edit')}選択項目を一括修正</button></div></div><div class="sl-table-wrap"><table class="sl-table sl-detail-table"><thead><tr><th><input type="checkbox" data-select-all aria-label="表示中をすべて選択"></th><th>売上日 / 売上No.</th><th>顧客名</th><th>主担当</th><th>施術・売上内容</th><th>合計金額</th><th>支払方法</th><th>商品</th><th>施術No.</th><th>履歴</th><th>操作</th></tr></thead><tbody data-sl-rows></tbody></table></div></section><dialog class="sl-dialog" data-sl-dialog></dialog></div>`
  }
/* markup-block:end */
...
/* wire-block:start */
    ...
    root.querySelector('[data-sl-clear-day]').addEventListener('click', () => {
      state.detailDate = ''
      state.selected.clear()
      renderSummary(root)
      renderRows(root)
