...
function listSql(sort) {
  ...
  return `WITH base AS (
    SELECT p."id",p."displayOrder",p."published",p."publishedAt",p."postKind",p."styleStaffKey",
           ...
           (SELECT COUNT(*)::int FROM "VisitCommunityComment" comment
             WHERE comment."postId"=p."id" AND comment."deletedAt" IS NULL AND comment."isAiAssistant"=FALSE) AS "commentCount"
      FROM "VisitCommunityPost" p
      LEFT JOIN "Visit" v ON v."id"=p."visitId"
      LEFT JOIN "Customer" c ON c."id"=p."customerId" AND c."organizationId"=p."organizationId"
...
function createStyleAdminControlsService({ prisma, staffSessionProvider }) {
  ...
  async function handleOrder(req, res) {
    ...
    await ensureStyleSchema(prisma)
    if (req.method === 'GET') {
      const order = await stylePostOrderSummary(prisma, current.organizationId)
      const engagementRows = await prisma.$queryRawUnsafe(
        `SELECT COUNT(*) FILTER (WHERE NOT EXISTS (
                ...
                (SELECT COUNT(*)::int FROM "VisitCommunityComment" demo
      ...
      return json(res, 200, {
        ...
        targetOrganization: current.organizationId === 'org_salon_de_lien',
        ...order,
        zeroLikeCount: Number(engagement.zeroLikeCount || 0),
        zeroCommentCount: Number(engagement.zeroCommentCount || 0),
        demoCommentCount: Number(engagement.demoCommentCount || 0),
      })
    }
    if (req.method !== 'PATCH') {
    ...
    const postId = text(input.postId, 220, '投稿', true)
    const moved = await moveStylePost(prisma, current.organizationId, postId, input.displayOrder)
    return json(res, 200, { success: true, release: 'style-demo-ordering-v634', ...moved })
  }
  ...
  async function handle(req, res, url) {
    ...
    if (!['/api/lien-style-admin-v618', '/api/lien-style-editor-v618', '/api/lien-style-order-v634'].includes(url.pathname)) return false
    try {
      if (url.pathname === '/api/lien-style-admin-v618') await handleList(req, res, url)
      else if (url.pathname === '/api/lien-style-order-v634') await handleOrder(req, res)
