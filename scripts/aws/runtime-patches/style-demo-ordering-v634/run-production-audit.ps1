...
$networkPath = Join-Path $env:RUNNER_TEMP 'style-demo-ordering-v634-network.json'
$overridesPath = Join-Path $env:RUNNER_TEMP 'style-demo-ordering-v634-overrides.json'
...
try {
  ...
  if ($LASTEXITCODE -ne 0 -or -not $taskArn -or $taskArn -eq 'None') {
    throw 'The Salon de Lien data audit task could not start.'
  }
} finally {
...
if ($LASTEXITCODE -ne 0) { throw 'The Salon de Lien data audit did not finish.' }
...
if ($LASTEXITCODE -ne 0 -or $result.exitCode -ne 0) {
  throw "The Salon de Lien data audit failed: $($result.reason)"
}
