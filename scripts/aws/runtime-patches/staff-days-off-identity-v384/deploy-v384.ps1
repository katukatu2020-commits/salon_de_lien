...
$tag = "staff-days-off-identity-v384"
...
if ($imageExists) {
  ...
  docker build --pull=false --build-arg BASE_IMAGE="$accountId.dkr.ecr.$region.amazonaws.com/${repository}:community-image-aspect-v383" -f "scripts/aws/runtime-patches/staff-days-off-identity-v384/Dockerfile" -t $image .
...
$taskFile = Join-Path $env:TEMP "salon-v384-task-definition.json"
...
$shiftChunkPath = "/_next/static/chunks/app/admin/appointments/page-shift-layout-20260812-02.capacity-persist-v44.business-schedule-v294.shift-day-nav-v382.js"
$invalidation = aws cloudfront create-invalidation --distribution-id $distributionId --paths "/admin/appointments*" "/admin/settings*" "/api/admin/staff-management*" "/tenant-setup-client.js" $shiftChunkPath --query "Invalidation.Id" --output text
