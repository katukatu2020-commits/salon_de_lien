...
$tag = 'customer-community-content-height-v375'
...
if (-not $existingImage.imageDetails) {
  ...
  docker build --pull=false -f scripts/aws/runtime-patches/customer-community-content-height-v375/Dockerfile -t $image .
...
$taskFile = Join-Path $env:TEMP 'salon-de-lien-task-v375.json'
