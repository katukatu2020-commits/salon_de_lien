...
async function readJson(req, limit = 12 * 1024) {
  ...
  for await (const chunk of req) {
    raw += chunk
    if (Buffer.byteLength(raw, 'utf8') > limit) throw Object.assign(new Error('送信内容が大きすぎます。'), { status: 413 })
  }
...
function canManagePost(session, audience, post) {
  ...
  if (audience === 'staff') return post.postKind === 'STORE'
...
function createContentManagementService({ prisma, staffSessionProvider, customerSessionProvider, canAccessThread }) {
  ...
  async function contentGet(req, res, url, session, audience) {
    ...
    const posts = await prisma.$queryRawUnsafe(
      'SELECT "id","organizationId","customerId","postKind","caption","updatedAt" FROM "VisitCommunityPost" WHERE "id"=$1 AND "organizationId"=$2 AND "published"=TRUE LIMIT 1',
      postId,
  ...
  async function contentMutation(req, res, session, audience) {
    ...
    if (!postId) return json(res, 400, { error: '投稿を指定してください。' })
    const posts = await prisma.$queryRawUnsafe(
      'SELECT "id","organizationId","customerId","postKind" FROM "VisitCommunityPost" WHERE "id"=$1 AND "organizationId"=$2 AND "published"=TRUE LIMIT 1',
      postId,
      session.organizationId,
    )
    const post = posts[0]
    ...
    if (target === 'post') {
      ...
      if (req.method === 'PATCH') {
        ...
        await prisma.$executeRawUnsafe('UPDATE "VisitCommunityPost" SET "caption"=$1,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$2 AND "organizationId"=$3 AND "published"=TRUE', caption || null, post.id, session.organizationId)
        return json(res, 200, { success: true, body: caption })
      }
      if (req.method === 'DELETE') {
        await prisma.$executeRawUnsafe('UPDATE "VisitCommunityPost" SET "published"=FALSE,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$1 AND "organizationId"=$2 AND "published"=TRUE', post.id, session.organizationId)
    ...
    if (target === 'comment') {
      ...
      if (req.method === 'PATCH') {
        ...
        await prisma.$executeRawUnsafe('UPDATE "VisitCommunityComment" SET "body"=$1,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$2 AND "postId"=$3 AND "deletedAt" IS NULL', body, comment.id, post.id)
        return json(res, 200, { success: true, body })
      }
      if (req.method === 'DELETE') {
        await prisma.$executeRawUnsafe('UPDATE "VisitCommunityComment" SET "deletedAt"=CURRENT_TIMESTAMP,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$1 AND "postId"=$2 AND "deletedAt" IS NULL', comment.id, post.id)
        return json(res, 200, { success: true })
      }
    }
  ...
  async function handle(req, res, url) {
    ...
    if (!session) { json(res, 401, { error: 'ログインが必要です。' }); return true }
    try {
      ...
      console.error('[content-edit-delete-v465]', { organizationId: session.organizationId, audience, path: url.pathname, error: error && error.message })
