...
function createCustomerStoreStaffService({ prisma, staffSessionProvider, customerSessionProvider, renderCustomerShell }) {
  ...
  function privateObjectKey(value) {
    ...
    if (!key.startsWith('private/') || key.includes('..') || key.includes('\\')) {
      throw new CustomerStoreStaffError('画像の保存先を確認できませんでした。', 404)
    }
    return key
  }

  async function streamPrivateObject(res, value, fallbackContentType = 'image/webp') {
    if (!bucket) throw new CustomerStoreStaffError('画像ストレージが設定されていません。', 503)
    const key = privateObjectKey(value)
    try {
      ...
      if (error instanceof CustomerStoreStaffError) throw error
      console.warn('[customer-store-staff] private image read failed', { key, name: error?.name || 'UnknownError' })
      throw new CustomerStoreStaffError('画像を読み込めませんでした。', 404)
    }
  }
  ...
  function requestedAudience(req, url) {
    ...
    if (String(url?.pathname || '').startsWith('/api/admin/')) return 'staff'
    return null
  }
  ...
  async function audienceSession(req, url) {
    const preferred = requestedAudience(req, url)
    if (preferred === 'staff') {
  ...
  async function staffDirectoryApi(req, res, url) {
    const { session, audience } = await audienceSession(req, url)
    const staff = (await directory(session.organizationId, false)).map(item => ({
      ...item,
      avatarUrl: item.avatarUrl ? `${item.avatarUrl}&audience=${audience}` : null,
    }))
  ...
  async function staffAvatar(req, res, url) {
    if (!bucket) throw new CustomerStoreStaffError('画像ストレージが設定されていません。', 503)
    const { session } = await audienceSession(req, url)
    ...
    await streamPrivateObject(res, row.profileImageKey)
  }
  ...
  async function staffProfile(req, res) {
    ...
    if (data.action === 'avatar') {
      ...
      return json(res, 200, { ok: true, avatarUrl: `/api/lien-staff-avatar?staffKey=${encodeURIComponent(target.key)}&audience=staff&v=${Date.now()}` })
    }
  ...
  async function sendStoreIcon(res, organizationId) {
    ...
    if (value.startsWith('private/') || value.startsWith('s3-private://')) {
      return streamPrivateObject(res, value)
    }
  ...
  async function handle(req, res, url) {
    const paths = new Set(['/customer-experience-v276.js', '/customer-experience-v395.js', '/api/lien-staff-directory', '/api/lien-staff-profiles', '/api/lien-staff-avatar', '/api/admin/staff-profile', '/api/admin/staff-management', '/api/lien-customer-nickname', '/api/lien-community-nickname', '/api/lien-customer-stores', '/api/lien-store-icon', '/api/admin/store-icon', '/u/stores'])
    if (!paths.has(url.pathname)) return false
    try {
      if ((url.pathname === '/customer-experience-v276.js' || url.pathname === '/customer-experience-v395.js') && req.method === 'GET') {
        res.statusCode = 200
        res.setHeader('Content-Type', 'application/javascript; charset=utf-8')
        res.setHeader('Cache-Control', 'public, max-age=300')
        res.setHeader('X-Content-Type-Options', 'nosniff')
        res.end(fs.readFileSync(path.join(__dirname, 'customer-experience-v395.js')))
      } else if (url.pathname === '/api/lien-staff-directory' || url.pathname === '/api/lien-staff-profiles') await staffDirectoryApi(req, res, url)
