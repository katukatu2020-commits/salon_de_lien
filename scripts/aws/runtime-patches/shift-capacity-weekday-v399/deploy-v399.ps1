...
$Tag = 'shift-capacity-weekday-v399'
...
if ($LASTEXITCODE -ne 0) {
  ...
  $DockerConfig = Join-Path $env:TEMP ("salon-ecr-v399-" + [Guid]::NewGuid().ToString('N'))
...
docker build -f scripts/aws/runtime-patches/shift-capacity-weekday-v399/Dockerfile -t $Image .
...
$TaskFile = Join-Path $env:TEMP 'salon-de-lien-task-v399.json'
[System.IO.File]::WriteAllText(
  $TaskFile,
