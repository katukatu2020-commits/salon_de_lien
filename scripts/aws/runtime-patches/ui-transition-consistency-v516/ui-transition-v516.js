;(() => {
  ...
  if (window.__orimiaUiTransitionV516) return
  ...
  for (const method of ['pushState', 'replaceState']) {
    ...
    history[method] = function (...args) {
      ...
      const changingProtectedRoute = candidate !== before && protectedUrl(candidate)
      if (changingProtectedRoute) beginNavigation(`history-${method}`)
      const result = original.apply(this, args)
      if (location.href !== before && protectedUrl(location.href)) commitNavigation(`history-${method}`)
      else if (changingProtectedRoute) reveal('history-unchanged')
      return result
    }
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (!protectedUrl(target.href) || (target.pathname === current.pathname && target.search === current.search)) return
  ...
  const observer = new MutationObserver(() => scheduleNavigationReveal())
