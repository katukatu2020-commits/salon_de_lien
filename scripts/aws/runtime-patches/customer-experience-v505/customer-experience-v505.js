(() => {
  ...
  if (window.__orimiaCustomerExperienceV505) return
  window.__orimiaCustomerExperienceV505 = true
  ...
  function enhanceCustomerBrand() {
    ...
    if (!brand || brand.dataset.cxBrandV505 === '1') return
    brand.dataset.cxBrandV505 = '1'
    brand.setAttribute('aria-label', 'Powered by ORIMIA ホーム')
    brand.replaceChildren()
    const title = document.createElement('span')
    title.className = 'cx-v505-brand-title'
    title.textContent = 'Powered by ORIMIA'
    const sub = document.createElement('span')
    sub.className = 'cx-v505-brand-subtitle'
  ...
  function enhanceLogin() {
    ...
    html.classList.toggle('cx-v505-login', active)
    if (!active) return
    ...
    main.classList.add('cx-login-main-v505')
    shell.classList.add('cx-login-shell-v505')
    panel.classList.add('cx-login-panel-v505')
    visual?.classList.add('cx-login-visual-v505')
    footer?.classList.add('cx-login-footer-v505')
    ...
    if (!shell.querySelector('.cx-login-brandbar-v505')) {
      const bar = document.createElement('header')
      bar.className = 'cx-login-brandbar-v505'
      const link = document.createElement('a')
      link.href = 'https://salon-de-lien.com/'
      link.setAttribute('aria-label', 'ORIMIA 公式サイトへ')
      const icon = document.createElement('img')
      icon.src = '/brand/orimia-icon-192.png?v=505'
      icon.alt = ''
      icon.width = 48
      icon.height = 48
    ...
    if (loginForm.dataset.cxLoginSubmitV505 !== '1') {
      loginForm.dataset.cxLoginSubmitV505 = '1'
  ...
  function buildMenuPicker(select) {
    ...
    wrapper.className = 'cx-menu-picker-v505'
    const trigger = document.createElement('button')
    trigger.type = 'button'
    trigger.className = 'cx-menu-trigger-v505'
    trigger.setAttribute('aria-haspopup', 'dialog')
    trigger.setAttribute('aria-expanded', 'false')

    const check = document.createElement('span')
    check.className = 'cx-menu-trigger-check-v505'
    check.setAttribute('aria-hidden', 'true')
    check.textContent = '✓'
    const copy = document.createElement('span')
    copy.className = 'cx-menu-trigger-copy-v505'
    const eyebrow = document.createElement('span')
    eyebrow.className = 'cx-menu-trigger-eyebrow-v505'
    ...
    chevron.className = 'cx-menu-trigger-chevron-v505'
    chevron.setAttribute('aria-hidden', 'true')
    chevron.textContent = '⌄'
    trigger.append(check, copy, chevron)
    wrapper.append(trigger)
    field.insertAdjacentElement('afterend', wrapper)
    field.classList.add('cx-menu-native-field-v505')
    select.classList.add('cx-menu-native-select-v505')

    const overlay = document.createElement('div')
    overlay.className = 'cx-menu-overlay-v505'
    overlay.hidden = true
    const backdrop = document.createElement('div')
    backdrop.className = 'cx-menu-backdrop-v505'
    const dialog = document.createElement('section')
    dialog.className = 'cx-menu-dialog-v505'
    dialog.setAttribute('role', 'dialog')
    dialog.setAttribute('aria-modal', 'true')
    dialog.setAttribute('aria-label', 'メニューを選択')

    const header = document.createElement('header')
    header.className = 'cx-menu-dialog-header-v505'
    ...
    closeButton.className = 'cx-menu-close-v505'
    closeButton.setAttribute('aria-label', '閉じる')
    closeButton.textContent = '×'
    header.append(headingWrap, closeButton)

    const searchLabel = document.createElement('label')
    searchLabel.className = 'cx-menu-search-v505'
    ...
    categories.className = 'cx-menu-categories-v505'
    categories.setAttribute('role', 'group')
    categories.setAttribute('aria-label', 'メニューカテゴリ')
    const summary = document.createElement('p')
    summary.className = 'cx-menu-result-summary-v505'
    summary.setAttribute('aria-live', 'polite')
    const list = document.createElement('div')
    list.className = 'cx-menu-list-v505'
    list.setAttribute('role', 'listbox')
    list.setAttribute('aria-label', 'メニュー一覧')
    const empty = document.createElement('div')
    empty.className = 'cx-menu-empty-v505'
    ...
    function renderCategories(items) {
      ...
      for (const name of categoryNames) {
        ...
        button.type = 'button'
        button.className = 'cx-menu-category-v505'
    ...
    function render() {
      ...
      for (const item of filtered) {
        const button = document.createElement('button')
        button.type = 'button'
        button.className = 'cx-menu-option-v505'
        button.setAttribute('role', 'option')
        const selected = item.value === select.value
        button.dataset.selected = String(selected)
        button.setAttribute('aria-selected', String(selected))
        const mark = document.createElement('span')
        mark.className = 'cx-menu-option-mark-v505'
    ...
    function open() {
      ...
      document.body.classList.add('cx-menu-open-v505')
    ...
    function close() {
      ...
      overlay.hidden = true
      document.body.classList.remove('cx-menu-open-v505')
      trigger.setAttribute('aria-expanded', 'false')
    ...
    syncTrigger()
    select._cxMenuPickerV505 = { wrapper, overlay, syncTrigger }
  }

  function enhanceMenuPicker() {
    if (location.pathname !== '/u/appointments') {
      document.body.classList.remove('cx-menu-open-v505')
      document.querySelectorAll('.cx-menu-overlay-v505').forEach(overlay => overlay.remove())
      return
    }
    ...
    if (!select) return
    const existing = select._cxMenuPickerV505
  ...
  function enhanceProfile() {
    ...
    if (!form) return
    form.classList.add('cx-profile-form-v505')
    const nickname = form.querySelector('input[name="nickname"]')
    const label = nickname?.closest('label')
    if (label && label.dataset.cxOptionalV505 !== '1') {
      label.dataset.cxOptionalV505 = '1'
  ...
  function refreshBookingSlots() {
    ...
    for (const button of buttons) {
      ...
      if (desired) {
        button.classList.toggle('cx-slot-selected-v505', !confirmed)
        button.classList.toggle('cx-slot-confirmed-v505', confirmed)
        ...
        if (!button.querySelector('.cx-slot-check-v505')) {
          const mark = document.createElement('span')
          mark.className = 'cx-slot-check-v505'
          mark.setAttribute('aria-hidden', 'true')
          mark.textContent = '✓'
          button.append(mark)
        }
        ...
        button.classList.remove('cx-slot-selected-v505', 'cx-slot-confirmed-v505')
        if (button.hasAttribute('aria-current')) button.removeAttribute('aria-current')
        button.querySelector('.cx-slot-check-v505')?.remove()
      }
    }

    if (success && success.dataset.cxConfirmedV505 !== '1') {
      success.dataset.cxConfirmedV505 = '1'
      success.classList.add('cx-booking-success-v505')
  ...
  function refresh() {
    ...
    enhanceProfile()
    refreshBookingSlots()
    document.documentElement.dataset.customerExperience = 'v505'
  }
  ...
  document.addEventListener('submit', event => {
    ...
    if (form.dataset.cxSubmittingV505 === '1') {
    ...
    event.preventDefault()
    event.stopImmediatePropagation()
    form.dataset.cxSubmittingV505 = '1'
