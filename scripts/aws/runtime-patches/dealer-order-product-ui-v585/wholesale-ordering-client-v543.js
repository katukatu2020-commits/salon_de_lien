...
/* dealer-auth-self-service-v548 */
  ...
  function quantityControl(product) {
    ...
    return '<div class="wo-stepper" data-product-id="' + esc(product.id) + '" data-unit="' + unit + '"><button class="wo-stepper-button" type="button" data-action="quantity-minus" aria-label="' + esc(product.name) + 'を減らす" title="数量を減らす" ' + (quantity <= 0 ? 'disabled' : '') + '>' + icon('minus') + '</button><input type="number" data-action="quantity-input" min="0" max="999" step="' + unit + '" value="' + quantity + '" aria-label="' + esc(product.name) + 'の発注数"><button class="wo-stepper-button" type="button" data-action="quantity-plus" aria-label="' + esc(product.name) + 'を増やす" title="数量を増やす">' + icon('plus') + '</button></div>'
  }

  function orderProductRows(products) {
    if (!products.length) return '<div class="wo-table-empty">条件に一致する商品はありません。</div>'
    return '<div class="wo-contract-product-table"><div class="wo-contract-product-head"><span>商品情報</span><span>取引ディーラー</span><span>契約価格（税抜）</span><span>発注単位</span><span>発注数</span></div>' + products.map(function (product) {
      const quantity = salon.quantities.get(product.id) || 0
      const metadata = [product.category, product.productCode, product.janCode ? 'JAN ' + product.janCode : ''].filter(Boolean)
      return '<article class="wo-contract-product-row ' + (quantity > 0 ? 'is-selected' : '') + '" data-product-id="' + esc(product.id) + '">' +
        '<div class="wo-contract-product-main"><span class="wo-contract-product-mark">' + icon('package') + '</span><div><small>' + esc(product.manufacturerName || 'メーカー未設定') + '</small><strong>' + esc(product.name) + '</strong><span>' + esc(metadata.join(' ・ ')) + '</span></div></div>' +
        '<div class="wo-contract-dealer"><small class="wo-mobile-label">取引ディーラー</small><span>' + icon('shop') + '<strong>' + esc(product.dealerName) + '</strong></span></div>' +
        '<div class="wo-contract-price"><small class="wo-mobile-label">契約価格（税抜）</small><div><strong>' + yen(product.unitPrice) + '</strong><span class="wo-discount-rate">' + rate(product.discountRate) + ' OFF</span></div><span class="wo-list-price">定価 ' + yen(product.listPrice) + '</span></div>' +
        '<div class="wo-order-unit"><small class="wo-mobile-label">発注単位</small><strong>' + Number(product.orderUnit || 1) + '点単位</strong></div>' +
        '<div class="wo-order-quantity"><small class="wo-mobile-label">発注数</small>' + quantityControl(product) + '<span class="wo-line-subtotal ' + (quantity > 0 ? '' : 'is-empty') + '">' + (quantity > 0 ? '小計 ' + yen(Number(product.unitPrice || 0) * quantity) : '未選択') + '</span></div></article>'
    }).join('') + '</div>'
  }
    ...
    root.addEventListener('input', function (event) {
      if (event.target.id === 'product-search') {
        ...
        if (row) {
          ...
          const minus = stepper.querySelector('[data-action="quantity-minus"]')
