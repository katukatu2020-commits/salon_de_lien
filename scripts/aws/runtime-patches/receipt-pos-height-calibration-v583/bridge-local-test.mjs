...
try {
  ...
  assert.equal(result.status.version, 'v583')
  assert.equal(result.status.available, true)
  assert.equal(result.status.dpi, 203)
  assert.equal(result.status.rollWidthMm, 80)
  assert.equal(result.status.printableWidthMm, 72.1)
  assert.equal(result.printCode, 200)
  assert.equal(result.print.dryRun, true)
  assert.equal(result.print.printed, false)
  assert.equal(result.print.dpi, 203)
  assert.equal(result.print.rollWidthMm, 80)
  assert.ok(result.print.heightMm > 95 && result.print.heightMm < 180)
  assert.equal(result.longCode, 200)
  assert.equal(result.long.dryRun, true)
  assert.ok(result.long.heightMm > result.print.heightMm + 140)
