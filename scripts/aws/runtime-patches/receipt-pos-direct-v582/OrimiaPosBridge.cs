...
namespace Orimia.PosBridge
{
    ...
    internal sealed class ReceiptRenderer : IDisposable
    {
        ...
        private const float BottomFeed = 28f;
        ...
        private float DrawRow(Graphics graphics, ReceiptRow row, float y, bool draw, bool summaryRow)
        {
            ...
            float valueWidth = emphasis ? 238f : 160f;
            float columnGap = 14f;
        ...
        {
            return pixels * 25.4f / 203f;
        }

        public static int PixelsToHundredthsOfInch(float pixels)
        {
            return Math.Max(40, (int)Math.Ceiling(pixels * 100f / 203f));
        }
    ...
    {
        private const string Version = "v582";
        ...
        private static int Main(string[] args)
        {
            ...
            bool created;
            using (Mutex mutex = new Mutex(true, "Local\\ORIMIA_POS_PRINT_BRIDGE_V582", out created))
            {
                if (!created) return 0;
                try
                {
