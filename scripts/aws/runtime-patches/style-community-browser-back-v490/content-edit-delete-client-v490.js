(() => {
  ...
  if (window.__lienStyleCommunityControlsV490) return
  window.__lienStyleCommunityControlsV490 = true
  ...
  function addStyles() {
    if (document.getElementById('lien-style-community-controls-v490-style')) return
    const style = document.createElement('style')
    style.id = 'lien-style-community-controls-v490-style'
    style.textContent = `
  ...
  function managedGridIsComplete(grid) {
    ...
    const expectedCount = Number(grid.dataset.lienStyleExpectedCountV490)
  ...
  async function enhanceCommunityList() {
    ...
    const currentGrid = document.querySelector('[data-lien-style-grid-managed-v490]')
    if (managedGridIsComplete(currentGrid)) return
    currentGrid?.remove()
    if (window.__lienStyleGridLoadingV490) return
    const main = document.querySelector('main')
    if (!main) return

    let payload
    window.__lienStyleGridLoadingV490 = true
    try {
      payload = await requestJson('/api/lien-content-management?audience=staff&scope=posts')
    } catch {
      return
    } finally {
      window.__lienStyleGridLoadingV490 = false
    }
    if (!/^\/admin\/community\/?$/.test(location.pathname)) return
    const restoredGrid = document.querySelector('[data-lien-style-grid-managed-v490]')
    ...
    grid.dataset.lienStyleGridManagedV490 = '1'
    ...
    grid.dataset.lienStyleExpectedCountV490 = String(renderedCount)
  ...
  window.__lienStyleCommunityRepairV490 = () => {
    activateControls()
    scheduleAfterRouteChange()
  }
  activateControls()
