...
if ($LASTEXITCODE -ne 0) {
  ...
  $DockerConfig = Join-Path $env:TEMP ("salon-ecr-v395-" + [Guid]::NewGuid().ToString('N'))
...
docker build -f scripts/aws/runtime-patches/customer-profile-v395/Dockerfile -t $Image .
...
$TaskFile = Join-Path $env:TEMP 'salon-de-lien-task-v395.json'
[System.IO.File]::WriteAllText(
  $TaskFile,
