;(function initializeStyleOrderFocusStability(root) {
  'use strict'

  const RELEASE = 'style-order-focus-stability-v667'
  const LIST_PATH = '/admin/community'
  const INPUT_SELECTOR = '.orimia-admin-style-managed-card-v625 input[name="displayOrder"]'
  if (!root?.document || root.__orimiaStyleOrderFocusStabilityV667) return
  root.__orimiaStyleOrderFocusStabilityV667 = true
  ...
  function restoreDrafts() {
    ...
    if (
      focusTarget
      && root.document.activeElement !== focusTarget
      && focusTarget.isConnected
      && focusedPostId === postIdFor(focusTarget)
      && drafts.has(focusedPostId)
  ...
  function queueRestore() {
    ...
    restoreQueued = true
    root.queueMicrotask(restoreDrafts)
  }
  ...
  style.id = 'orimia-style-order-focus-stability-v667-style'
  style.textContent = `
  ...
  root.document.documentElement.dataset.orimiaStyleOrderFocusStabilityV667 = 'ready'
  root.__orimiaStyleOrderFocusStabilityStateV667 = { drafts, get focusedPostId() { return focusedPostId } }
  queueRestore()
