(() => {
  ...
  if (window.__lienStyleCommunityControlsV470) return
  window.__lienStyleCommunityControlsV470 = true
  ...
  function addStyles() {
    if (document.getElementById('lien-style-community-controls-v470-style')) return
    const style = document.createElement('style')
    style.id = 'lien-style-community-controls-v470-style'
    style.textContent = `
      ...
      .lien-toast-v470{position:fixed;right:20px;bottom:24px;z-index:2147483647;max-width:min(88vw,360px);padding:13px 17px;border:1px solid #cbdcc9;border-radius:14px;background:#f3f8f2;color:#36563a;font-size:14px;font-weight:700;box-shadow:0 12px 36px rgba(47,42,37,.16)}
  ...
  function showToast(message) {
    document.querySelector('.lien-toast-v470')?.remove()
    const toast = document.createElement('div')
    toast.className = 'lien-toast-v470'
  ...
  function hiddenMedia(post) {
    ...
    link.setAttribute('aria-label', '非公開のスタイル投稿')
  ...
  async function enhanceCommunityList() {
    ...
    if (document.querySelector('[data-lien-style-grid-managed-v470]')) return
    if (window.__lienStyleGridLoadingV470) return
    const main = document.querySelector('main')
    if (!main) return

    let payload
    window.__lienStyleGridLoadingV470 = true
    try {
      payload = await requestJson('/api/lien-content-management?audience=staff&scope=posts')
    } catch {
      return
    } finally {
      window.__lienStyleGridLoadingV470 = false
    }
    if (!/^\/admin\/community\/?$/.test(location.pathname) || document.querySelector('[data-lien-style-grid-managed-v470]')) return

    const posts = Array.isArray(payload.posts) ? payload.posts : []
    const postById = new Map(posts.map(post => [String(post.id), post]))
    const publicLinks = Array.from(main.querySelectorAll('a[href^="/admin/community/"]')).filter(link => postIdFromHref(link.href))
    const firstLink = publicLinks[0]
    const currentCard = firstLink?.closest('.lien-style-card')
    const grid = currentCard?.parentElement || firstLink?.parentElement || main.querySelector('.grid.grid-cols-2')
    if (!grid) return
    grid.dataset.lienStyleGridManagedV470 = '1'

    const renderedIds = new Set()
    publicLinks.forEach(link => {
      const postId = postIdFromHref(link.href)
      if (!postId || renderedIds.has(postId) || link.closest('.lien-style-card')) return
      const post = postById.get(postId) || { id: postId, published: true }
      renderedIds.add(postId)
      const position = document.createComment('lien-style-card')
      link.replaceWith(position)
      position.replaceWith(buildManagedCard(post, link))
    })

    posts.filter(post => !post.published && !renderedIds.has(String(post.id))).forEach(post => {
      renderedIds.add(String(post.id))
      grid.appendChild(buildManagedCard(post, hiddenMedia(post)))
    })
  }
  ...
  async function enhanceCommunityDetail() {
    ...
    if (!match || document.body.dataset.lienCommunityOwnerEnhancedV470 === match[2]) return
    if (window.__lienCommunityDetailLoadingV470 === match[2]) return
    const article = document.querySelector('.community-detail-page article, main article')
    if (!article) return
    const postId = decodeURIComponent(match[2])
    let payload
    window.__lienCommunityDetailLoadingV470 = match[2]
    try {
      payload = await requestJson(`/api/lien-content-management?audience=${audience}&postId=${encodeURIComponent(postId)}`)
    } catch {
      return
    } finally {
      if (window.__lienCommunityDetailLoadingV470 === match[2]) window.__lienCommunityDetailLoadingV470 = ''
    }
    if (!location.pathname.includes(`/community/${match[2]}`) || document.body.dataset.lienCommunityOwnerEnhancedV470 === match[2]) return
    ...
    document.body.dataset.lienCommunityOwnerEnhancedV470 = postId
  ...
  addStyles()
  const schedule = () => {
    window.clearTimeout(window.__lienStyleCommunityControlsTimerV470)
    window.__lienStyleCommunityControlsTimerV470 = window.setTimeout(() => {
