...
try {
  ...
  console.log(JSON.stringify({ release: 'salon-branch-inquiry-v675', isolation: true, ownerOnly: true, noPlanRequired: true, pendingUntilApproval: true, groupLinkedOnApproval: true, rejectionMail: true, idempotency: true, initialPasswordGate: true, jstTotals: true, emailFailureRecovery: true, originProtection: true }))
  if (process.env.KEEP_TEST_SERVER === '1') {
    keeping = true
    console.log('V675_BROWSER_READY ' + base)
    fs.writeFileSync('/tmp/v675-browser-fixture.json', JSON.stringify({ base, operatorCookie }))
