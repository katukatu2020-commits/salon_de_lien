(() => {
  ...
  if (window.__lienStyleCommunityControlsV479) return
  window.__lienStyleCommunityControlsV479 = true
  ...
  function addStyles() {
    if (document.getElementById('lien-style-community-controls-v479-style')) return
    const style = document.createElement('style')
    style.id = 'lien-style-community-controls-v479-style'
    style.textContent = `
  ...
  async function enhanceCommunityList() {
    ...
    if (document.querySelector('[data-lien-style-grid-managed-v479]')) return
    if (window.__lienStyleGridLoadingV479) return
    const main = document.querySelector('main')
    if (!main) return

    let payload
    window.__lienStyleGridLoadingV479 = true
    try {
      payload = await requestJson('/api/lien-content-management?audience=staff&scope=posts')
    } catch {
      return
    } finally {
      window.__lienStyleGridLoadingV479 = false
    }
    if (!/^\/admin\/community\/?$/.test(location.pathname) || document.querySelector('[data-lien-style-grid-managed-v479]')) return
    ...
    grid.dataset.lienStyleGridManagedV479 = '1'
    grid.replaceChildren(...posts.map(post => buildManagedCard(post, hiddenMedia(post))))
