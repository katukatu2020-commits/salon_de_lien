(() => {
  ...
  if (window.__lienStyleCommunityControlsV471) return
  window.__lienStyleCommunityControlsV471 = true
  ...
  function addStyles() {
    if (document.getElementById('lien-style-community-controls-v471-style')) return
    const style = document.createElement('style')
    style.id = 'lien-style-community-controls-v471-style'
    style.textContent = `
  ...
  async function enhanceCommunityList() {
    ...
    if (document.querySelector('[data-lien-style-grid-managed-v471]')) return
    if (window.__lienStyleGridLoadingV471) return
    const main = document.querySelector('main')
    if (!main) return

    let payload
    window.__lienStyleGridLoadingV471 = true
    try {
      payload = await requestJson('/api/lien-content-management?audience=staff&scope=posts')
    } catch {
      return
    } finally {
      window.__lienStyleGridLoadingV471 = false
    }
    if (!/^\/admin\/community\/?$/.test(location.pathname) || document.querySelector('[data-lien-style-grid-managed-v471]')) return
    ...
    if (!grid) return
    grid.dataset.lienStyleGridManagedV471 = '1'
  ...
  addStyles()
  const schedule = () => {
    window.clearTimeout(window.__lienStyleCommunityControlsTimerV471)
    window.__lienStyleCommunityControlsTimerV471 = window.setTimeout(() => {
