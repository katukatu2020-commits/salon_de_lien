...
const manifest='/tmp/store-list-v595-parent.json'
...
if(process.argv[2]==='snapshot') {
  ...
  const changes=JSON.parse(fs.readFileSync('/tmp/store-list-v595-changes.json','utf8'))
  for(const [file,digest] of Object.entries(parent)) {
    ...
    if(scoped.length) {
      ...
      for(const c of scoped) { assert.equal(source.split(c.after).length,2);source=source.replace(c.after,c.before) }
      value=Buffer.from(source)
    }
    assert.equal(hash(value),digest,'Unexpected change: '+file)
  }
  console.log(JSON.stringify({protectedFiles:Object.keys(parent).length,sessionAuthorizationUnchanged:true,receiptsAndLoginUnchanged:true}))
}
