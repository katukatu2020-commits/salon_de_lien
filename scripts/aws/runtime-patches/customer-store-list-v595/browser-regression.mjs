...
const base = process.env.SMOKE_BASE_URL || 'http://localhost:3595'
const output = process.env.SCREENSHOT_DIR || 'artifacts/customer-store-list-v595/browser'
...
try {
  for (const viewport of [{ width: 1365, height: 900 }, { width: 390, height: 844 }]) {
    ...
    const target = initial.stores.find(store => store.linked && store.available !== false && !store.current)
