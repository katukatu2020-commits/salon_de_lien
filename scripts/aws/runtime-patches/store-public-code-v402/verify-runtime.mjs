...
if (!workflows.includes('data-lien-store-identity-v402') || !workflows.includes("jsonRequest('/api/admin/store-qr')")) {
  throw new Error('store identity settings card is missing')
}
if (!workflows.includes('initStoreIdentityCard().catch(() => {})')) {
  throw new Error('store identity settings card is not bootstrapped')
}
...
console.log('store public code v402 verified')
