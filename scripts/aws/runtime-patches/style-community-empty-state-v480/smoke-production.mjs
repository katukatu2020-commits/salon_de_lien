...
const page = await fetch(`${baseUrl}/admin/community?smoke=v480`, {
  headers: { cookie: sessionCookie, 'Cache-Control': 'no-cache' },
})
assert.equal(page.status, 200)
const html = await page.text()
assert.match(html, /content-edit-delete-client-v480\.js/)
assert.match(html, /data-lien-community-bootstrap="v480"/)

const client = await fetch(`${baseUrl}/content-edit-delete-client-v480.js?smoke=v480`, {
  headers: { 'Cache-Control': 'no-cache' },
})
assert.equal(client.status, 200)
const source = await client.text()
assert.match(source, /grid\.replaceChildren\(\.\.\.posts\.map/)
assert.match(source, /data-lien-style-grid-managed-v480/)
