...
const clientPath = `${root}/public/content-edit-delete-client-v480.js`
...
const appShellChunks = fs.readdirSync(`${root}/.next/server/chunks`)
  ...
  .filter(source => source.includes('"data-lien-community-bootstrap": "v480"'))
const pageFiles = [
...
const assertions = [
  [appShellChunks.length === 1, 'one authenticated AppShell loads v480'],
  [appShellChunks[0]?.includes('/content-edit-delete-client-v480.js'), 'AppShell uses the cache-busted client'],
  [client.includes('__lienStyleCommunityControlsV480'), 'v480 client guard exists'],
  ...
  [client.includes("node.textContent?.includes('条件に合うスタイルはありません')"), 'empty SSR state is detected'],
  [client.includes('emptyState?.isConnected && !resultSection.contains(emptyState)'), 'stale empty SSR state is removed'],
  [client.includes('data-lien-style-grid-managed-v480'), 'v480 list idempotency exists'],
...
for (const file of pageFiles) {
  ...
  if (!source.includes('/content-edit-delete-client-v480.js')) throw new Error(`v480 client is missing from ${file}`)
}

console.log(`style community empty-state rendering verified (${assertions.length + pageFiles.length} assertions)`)
