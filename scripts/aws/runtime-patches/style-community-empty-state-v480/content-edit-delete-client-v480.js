(() => {
  ...
  if (window.__lienStyleCommunityControlsV480) return
  window.__lienStyleCommunityControlsV480 = true
  ...
  function addStyles() {
    if (document.getElementById('lien-style-community-controls-v480-style')) return
    const style = document.createElement('style')
    style.id = 'lien-style-community-controls-v480-style'
    style.textContent = `
  ...
  async function enhanceCommunityList() {
    ...
    if (document.querySelector('[data-lien-style-grid-managed-v480]')) return
    if (window.__lienStyleGridLoadingV480) return
    const main = document.querySelector('main')
    if (!main) return

    let payload
    window.__lienStyleGridLoadingV480 = true
    try {
      payload = await requestJson('/api/lien-content-management?audience=staff&scope=posts')
    } catch {
      return
    } finally {
      window.__lienStyleGridLoadingV480 = false
    }
    if (!/^\/admin\/community\/?$/.test(location.pathname) || document.querySelector('[data-lien-style-grid-managed-v480]')) return
    ...
    grid.dataset.lienStyleGridManagedV480 = '1'
