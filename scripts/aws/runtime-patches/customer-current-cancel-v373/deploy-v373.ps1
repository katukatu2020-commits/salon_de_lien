...
$tag = 'customer-current-cancel-v373'
...
if (-not $existingImage.imageDetails) {
  ...
  docker build --pull=false -f scripts/aws/runtime-patches/customer-current-cancel-v373/Dockerfile -t $image .
...
$taskFile = Join-Path $env:TEMP 'salon-de-lien-task-v373.json'
