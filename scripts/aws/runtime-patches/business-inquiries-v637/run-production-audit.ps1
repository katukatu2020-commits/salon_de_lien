...
$networkPath = Join-Path $env:RUNNER_TEMP 'business-inquiries-v637-network.json'
$overridesPath = Join-Path $env:RUNNER_TEMP 'business-inquiries-v637-overrides.json'
...
try {
  $taskArn = (aws ecs run-task `
    ...
    --output text `
    --no-cli-pager).Trim()
  if ($LASTEXITCODE -ne 0 -or -not $taskArn -or $taskArn -eq 'None') { throw 'The business inquiry audit task could not start.' }
} finally {
...
if ($LASTEXITCODE -ne 0) { throw 'The business inquiry audit did not finish.' }
$result = aws ecs describe-tasks `
  ...
  --output json `
  --no-cli-pager | ConvertFrom-Json
if ($LASTEXITCODE -ne 0 -or $result.exitCode -ne 0) { throw "The business inquiry audit failed: $($result.reason)" }
