...
const output = process.env.SCREENSHOT_DIR || 'artifacts/style-admin-scroll-stability-v649/browser'
...
function html() {
  return `<!doctype html><html lang="ja" data-orimia-ui-ready="v516"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="/style-admin-controls-v618.css"><style>
    *{box-sizing:border-box}body{margin:0;background:#fffaf7;color:#292522;font-family:sans-serif}.test-banner{height:720px;padding:32px;background:#f7eee9}main{max-width:1560px;margin:auto;padding:24px}img{background:#eee}
    ...
    window.__routeWrapperCallsV649 = []
    window.__scrollToCallsV649 = []
    const nativePushStateV649Harness = History.prototype.pushState
    const nativeReplaceStateV649Harness = History.prototype.replaceState
    nativeReplaceStateV649Harness.call(history, { sentinel: 'keep-v649' }, '', location.href)
    Object.defineProperty(history, 'pushState', { configurable: true, writable: true, value: function(...args) {
      window.__routeWrapperCallsV649.push({ method: 'pushState', url: String(args[2] || '') })
      const result = nativePushStateV649Harness.apply(this, args)
    ...
    Object.defineProperty(history, 'replaceState', { configurable: true, writable: true, value: function(...args) {
      window.__routeWrapperCallsV649.push({ method: 'replaceState', url: String(args[2] || '') })
      const result = nativeReplaceStateV649Harness.apply(this, args)
      setTimeout(() => scrollTo(0, 0), 350)
      return result
    } })
    const nativeScrollToV649Harness = window.scrollTo.bind(window)
    window.scrollTo = function(...args) {
      window.__scrollToCallsV649.push({ at: performance.now(), args })
      return nativeScrollToV649Harness(...args)
    }
...
async function verifyViewport(viewport, label, expectedColumns) {
  ...
  await page.waitForFunction(() => document.querySelectorAll('.orimia-admin-style-card-v618').length === 15)
  ...
  assert.equal(initial.ownPush, true)
  assert.equal(initial.ownReplace, true)
  assert.deepEqual(initial.state, { sentinel: 'keep-v649' })
  ...
  await page.evaluate(() => {
    ...
    scrollTo(0, target)
    window.__scrollToCallsV649.length = 0
  })
  ...
  for (const nextPage of [2, 3]) {
    ...
    assert.equal(new URL(page.url()).searchParams.get('page'), String(nextPage))
  ...
  const settled = await page.evaluate(() => ({
    y: window.scrollY,
    wrappers: window.__routeWrapperCallsV649,
    state: history.state,
    scrollCalls: window.__scrollToCallsV649,
  }))
  assert.ok(Math.abs(settled.y - manualTarget) <= 3, `${label} snapped from ${manualTarget} to ${settled.y}`)
  assert.deepEqual(settled.wrappers, [], `${label} invoked the application history wrapper`)
  assert.deepEqual(settled.state, { sentinel: 'keep-v649' })
  ...
  await context.close()
  return { label, viewport, columns: initial.columns, transitions, manualTarget, settled: settled.y, scrollCalls: settled.scrollCalls.length }
}
