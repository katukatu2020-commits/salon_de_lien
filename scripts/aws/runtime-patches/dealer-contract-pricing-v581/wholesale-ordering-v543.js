...
const DEALER_OPERATIONS_RELEASE = 'dealer-contract-pricing-v581'
...
function createWholesaleOrderingService({ prisma, crypto, adminSessionProvider, dealerAuthMailSender = sendDealerAuthMail }) {
  ...
  async function dealerSession(req) {
    ...
    const rows = await prisma.$queryRawUnsafe('SELECT "id","name","loginId","email","phone","postalCode","address","dealerCode","authVersion" FROM "WholesaleDealer" WHERE "id"=$1 AND "active"=TRUE LIMIT 1', payload.dealerId)
  ...
  async function handle(req, res, url) {
    ...
    const dealerPageMatch = pathname.match(/^\/dealer\/(orders|salons|products|pricing)$/)
  ...
  return { ensureSchema, handle, adminSession, dealerSession, salonBootstrap, dealerBootstrap, requestDealerConnectionByCode, registerSalonContract, createDealerProduct, updateContractProductPricing }
}

module.exports = { createWholesaleOrderingService, WholesaleError, SESSION_COOKIE, ORDER_STATUSES, CONTRACT_STATUSES, DEALER_OPERATIONS_RELEASE, normalizeDealerCode }
