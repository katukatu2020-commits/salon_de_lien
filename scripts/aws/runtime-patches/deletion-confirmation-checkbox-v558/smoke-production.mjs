...
const headers = {
  ...
  "User-Agent": "ORIMIA-deletion-confirmation-checkbox-v558-smoke/1.0",
};

const ready = await fetch(`${baseUrl}/api/health/ready?smoke=v558`, {
  headers,
  cache: "no-store",
});
...
const community = await fetch(`${baseUrl}/u/community?smoke=v558`, {
...
const detail = await fetch(
  `${baseUrl}/u/community/${encodeURIComponent(ownedPost.postId)}?smoke=v558`,
  {
    headers: { ...headers, Cookie: cookie },
    cache: "no-store",
  },
...
assert.match(detailHtml, /content-edit-delete-client-v558\.js/);
assert.doesNotMatch(detailHtml, /content-edit-delete-client-v557\.js/);

const client = await fetch(
  `${baseUrl}/content-edit-delete-client-v558.js?smoke=v558`,
  { headers, cache: "no-store" },
);
assert.equal(client.status, 200);
const source = await client.text();
assert.match(source, /__lienStyleCommunityControlsV558/);
assert.match(source, /buildCustomerCommentMenu/);
assert.match(source, /lienCommentSignatureV558/);
