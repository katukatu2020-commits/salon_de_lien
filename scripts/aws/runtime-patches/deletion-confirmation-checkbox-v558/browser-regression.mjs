...
const screenshotRoot =
  ...
  path.join(os.tmpdir(), "orimia-delete-confirmation-v558");
...
async function findPublishedPost(context) {
  const response = await context.request.get(
    `${baseUrl}/u/community?browser=v558`,
    {
...
async function verifyViewport(name, viewport) {
  ...
  let createdId = "";
  let removed = false;
  const originalBody = `v558 ${name} 動作確認 ${Date.now()}`;
  ...
  try {
    ...
    if (injectedClient) {
      await page.route("**/content-edit-delete-client-v557.js*", (route) =>
        route.fulfill({
          status: 200,
    ...
    await page.goto(
      `${baseUrl}/u/community/${encodeURIComponent(postId)}?browser=v558-${name}`,
      {
    ...
    const trigger = card.locator(
      '[data-lien-customer-comment-menu="v558"] .lien-comment-menu-trigger',
    );
    ...
    assert.equal(
      await page.locator('[data-lien-customer-comment-menu="v558"]').count(),
      ownedCount,
    );
    assert.equal(
...
async function verifyStaffPostDialog() {
  ...
  try {
    ...
    await page.goto(`${baseUrl}/admin/community?browser=v558-staff`, {
      waitUntil: "domcontentloaded",
      timeout: 20_000,
    });
    if (injectedClient)
