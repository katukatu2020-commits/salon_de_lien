...
const client = read("public/content-edit-delete-client-v558.js");
...
const appShellChunks = fs
  ...
  .filter((source) =>
    source.includes('"data-lien-community-bootstrap": "v558"'),
  );
const pageFiles = [
...
assert.match(appShellChunks[0], /content-edit-delete-client-v558\.js/);
assert.doesNotMatch(appShellChunks[0], /content-edit-delete-client-v557\.js/);
assert.match(staffRuntime, /__lienStyleCommunityLoaderV558/);
assert.match(staffRuntime, /content-edit-delete-client-v558\.js/);
assert.doesNotMatch(staffRuntime, /content-edit-delete-client-v557\.js/);
...
for (const file of pageFiles) {
  ...
  assert.match(
    ...
    `${file} does not load v558`,
  );
  assert.doesNotMatch(
    source,
    /content-edit-delete-client-v557\.js/,
    `${file} still loads v557`,
  );
}

assert.match(client, /window\.__lienStyleCommunityControlsV558 = true/);
assert.match(client, /dataset\.lienCustomerCommentMenu = ["']v558["']/);
