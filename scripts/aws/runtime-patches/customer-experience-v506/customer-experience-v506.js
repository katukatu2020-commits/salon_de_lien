(() => {
  ...
  if (window.__orimiaCustomerExperienceV506) return
  window.__orimiaCustomerExperienceV506 = true
  ...
  function enhanceCustomerBrand() {
    ...
    if (!brand || brand.dataset.cxBrandV506 === '1') return
    brand.dataset.cxBrandV506 = '1'
    brand.setAttribute('aria-label', 'Powered by ORIMIA ホーム')
    brand.replaceChildren()
    const title = document.createElement('span')
    title.className = 'cx-v506-brand-title'
    title.textContent = 'Powered by ORIMIA'
    const sub = document.createElement('span')
    sub.className = 'cx-v506-brand-subtitle'
  ...
  function enhanceLogin() {
    ...
    html.classList.toggle('cx-v506-login', active)
    if (!active) return
    ...
    main.classList.add('cx-login-main-v506')
    shell.classList.add('cx-login-shell-v506')
    panel.classList.add('cx-login-panel-v506')
    visual?.classList.add('cx-login-visual-v506')
    footer?.classList.add('cx-login-footer-v506')
    ...
    if (!shell.querySelector('.cx-login-brandbar-v506')) {
      const bar = document.createElement('header')
      bar.className = 'cx-login-brandbar-v506'
    ...
    if (loginForm.dataset.cxLoginSubmitV506 !== '1') {
      loginForm.dataset.cxLoginSubmitV506 = '1'
  ...
  function buildMenuPicker(select) {
    ...
    wrapper.className = 'cx-menu-picker-v506'
    const trigger = document.createElement('button')
    trigger.type = 'button'
    trigger.className = 'cx-menu-trigger-v506'
    trigger.setAttribute('aria-haspopup', 'dialog')
    trigger.setAttribute('aria-expanded', 'false')

    const check = document.createElement('span')
    check.className = 'cx-menu-trigger-check-v506'
    check.setAttribute('aria-hidden', 'true')
    check.textContent = '✓'
    const copy = document.createElement('span')
    copy.className = 'cx-menu-trigger-copy-v506'
    const eyebrow = document.createElement('span')
    eyebrow.className = 'cx-menu-trigger-eyebrow-v506'
    ...
    chevron.className = 'cx-menu-trigger-chevron-v506'
    chevron.setAttribute('aria-hidden', 'true')
    chevron.textContent = '⌄'
    trigger.append(check, copy, chevron)
    wrapper.append(trigger)
    field.insertAdjacentElement('afterend', wrapper)
    field.classList.add('cx-menu-native-field-v506')
    select.classList.add('cx-menu-native-select-v506')

    const overlay = document.createElement('div')
    overlay.className = 'cx-menu-overlay-v506'
    overlay.hidden = true
    const backdrop = document.createElement('div')
    backdrop.className = 'cx-menu-backdrop-v506'
    const dialog = document.createElement('section')
    dialog.className = 'cx-menu-dialog-v506'
    dialog.setAttribute('role', 'dialog')
    dialog.setAttribute('aria-modal', 'true')
    dialog.setAttribute('aria-label', 'メニューを選択')

    const header = document.createElement('header')
    header.className = 'cx-menu-dialog-header-v506'
    ...
    closeButton.className = 'cx-menu-close-v506'
    closeButton.setAttribute('aria-label', '閉じる')
    closeButton.textContent = '×'
    header.append(headingWrap, closeButton)

    const searchLabel = document.createElement('label')
    searchLabel.className = 'cx-menu-search-v506'
    ...
    categories.className = 'cx-menu-categories-v506'
    categories.setAttribute('role', 'group')
    categories.setAttribute('aria-label', 'メニューカテゴリ')
    const summary = document.createElement('p')
    summary.className = 'cx-menu-result-summary-v506'
    summary.setAttribute('aria-live', 'polite')
    const list = document.createElement('div')
    list.className = 'cx-menu-list-v506'
    list.setAttribute('role', 'listbox')
    list.setAttribute('aria-label', 'メニュー一覧')
    const empty = document.createElement('div')
    empty.className = 'cx-menu-empty-v506'
    ...
    function renderCategories(items) {
      ...
      for (const name of categoryNames) {
        ...
        button.type = 'button'
        button.className = 'cx-menu-category-v506'
    ...
    function render() {
      ...
      for (const item of filtered) {
        const button = document.createElement('button')
        button.type = 'button'
        button.className = 'cx-menu-option-v506'
        button.setAttribute('role', 'option')
        const selected = item.value === select.value
        button.dataset.selected = String(selected)
        button.setAttribute('aria-selected', String(selected))
        const mark = document.createElement('span')
        mark.className = 'cx-menu-option-mark-v506'
    ...
    function open() {
      ...
      document.body.classList.add('cx-menu-open-v506')
    ...
    function close() {
      ...
      overlay.hidden = true
      document.body.classList.remove('cx-menu-open-v506')
      trigger.setAttribute('aria-expanded', 'false')
    ...
    syncTrigger()
    select._cxMenuPickerV506 = { wrapper, overlay, syncTrigger }
  }

  function enhanceMenuPicker() {
    if (location.pathname !== '/u/appointments') {
      document.body.classList.remove('cx-menu-open-v506')
      document.querySelectorAll('.cx-menu-overlay-v506').forEach(overlay => overlay.remove())
      return
    }
    ...
    if (!select) return
    const existing = select._cxMenuPickerV506
  ...
  function enhanceProfile() {
    ...
    if (!form) return
    form.classList.add('cx-profile-form-v506')
    const nickname = form.querySelector('input[name="nickname"]')
    const label = nickname?.closest('label')
    if (label && label.dataset.cxOptionalV506 !== '1') {
      label.dataset.cxOptionalV506 = '1'
  ...
  function refreshBookingSlots() {
    ...
    for (const button of buttons) {
      ...
      if (desired) {
        button.classList.toggle('cx-slot-selected-v506', !confirmed)
        button.classList.toggle('cx-slot-confirmed-v506', confirmed)
        ...
        if (!button.querySelector('.cx-slot-check-v506')) {
          const mark = document.createElement('span')
          mark.className = 'cx-slot-check-v506'
          mark.setAttribute('aria-hidden', 'true')
          mark.textContent = '✓'
          button.append(mark)
        }
        ...
        button.classList.remove('cx-slot-selected-v506', 'cx-slot-confirmed-v506')
        if (button.hasAttribute('aria-current')) button.removeAttribute('aria-current')
        button.querySelector('.cx-slot-check-v506')?.remove()
      }
    }

    if (success && success.dataset.cxConfirmedV506 !== '1') {
      success.dataset.cxConfirmedV506 = '1'
      success.classList.add('cx-booking-success-v506')
  ...
  function refresh() {
    ...
    enhanceProfile()
    refreshBookingSlots()
    document.documentElement.dataset.customerExperience = 'v506'
  }
  ...
  document.addEventListener('submit', event => {
    ...
    if (form.dataset.cxSubmittingV506 === '1') {
    ...
    event.preventDefault()
    event.stopImmediatePropagation()
    form.dataset.cxSubmittingV506 = '1'
