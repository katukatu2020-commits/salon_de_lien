...
$networkPath = Join-Path $env:RUNNER_TEMP 'booking-confirmation-name-v635-network.json'
$overridesPath = Join-Path $env:RUNNER_TEMP 'booking-confirmation-name-v635-overrides.json'
...
try {
  ...
  if ($LASTEXITCODE -ne 0 -or -not $taskArn -or $taskArn -eq 'None') {
    throw 'The booking confirmation name audit task could not start.'
  }
} finally {
...
if ($LASTEXITCODE -ne 0) { throw 'The booking confirmation name audit did not finish.' }
...
if ($LASTEXITCODE -ne 0 -or $result.exitCode -ne 0) {
  throw "The booking confirmation name audit failed: $($result.reason)"
}
