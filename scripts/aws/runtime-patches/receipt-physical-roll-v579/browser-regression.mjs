...
const artifactRoot = process.env.VERIFY_SCREENSHOT_DIR || path.join(os.tmpdir(), 'orimia-receipt-physical-roll-v579')
...
async function openReceipt(context, viewport, label, appointmentId, errors) {
  ...
  await page.waitForFunction(() => Boolean(window.__orimiaReceiptPhysicalRollV579), null, { timeout:15_000 })
  await page.waitForFunction(() => !document.fonts || document.fonts.status === 'loaded')
  await page.waitForFunction(() => (
    Boolean(document.getElementById('orimia-receipt-roll-host-v579'))
    && Boolean(document.getElementById('orimia-receipt-roll-page-v579'))
    && document.documentElement.hasAttribute('data-orimia-receipt-roll-ready-v579')
  ))
  ...
  await page.screenshot({ path:path.join(artifactRoot, `receipt-v579-${label}-screen.png`), fullPage:true })
  return { page, screen }
}

async function prepareThroughButton(page) {
  const preparedBeforeClick = await page.evaluate(() => ({
    nativePrintCalled:document.documentElement.dataset.nativeReceiptPrintCalled || '',
    metrics:window.__orimiaReceiptPhysicalRollV579.metrics(),
    pageRule:document.getElementById('orimia-receipt-roll-page-v579')?.textContent || '',
  }))
  assert.equal(preparedBeforeClick.nativePrintCalled, '')
  assert.equal(preparedBeforeClick.metrics.widthMm, 80)
  assert.ok(preparedBeforeClick.pageRule.includes(`80mm ${preparedBeforeClick.metrics.heightMm}mm`))

  await page.getByRole('button').click()
  await page.waitForFunction(() => document.documentElement.dataset.nativeReceiptPrintCalled === '1')
  const metrics = await page.evaluate(() => window.__orimiaReceiptPhysicalRollV579.metrics())
...
async function verifyPrintLayer(page, label, metrics) {
  ...
  const state = await page.evaluate(() => {
    ...
    const copy = host?.querySelector('[data-orimia-receipt-roll-copy-v579="1"]')
    ...
    return {
      heightVariable:getComputedStyle(root).getPropertyValue('--orimia-receipt-roll-height-v579').trim(),
      ...
      host:{ x:hostRect?.x || 0, y:hostRect?.y || 0, width:hostRect?.width || 0, height:hostRect?.height || 0, page:host ? getComputedStyle(host).page : '' },
      copy:{ x:copyRect?.x || 0, y:copyRect?.y || 0, width:copyRect?.width || 0, height:copyRect?.height || 0, page:copy ? getComputedStyle(copy).page : '' },
      topGapPx:hostRect && copyRect ? copyRect.top - hostRect.top : Number.POSITIVE_INFINITY,
      leftGapPx:hostRect && copyRect ? copyRect.left - hostRect.left : Number.POSITIVE_INFINITY,
  ...
  assert.ok(Math.abs(state.host.x) < 0.5 && Math.abs(state.host.y) < 0.5, `${label}: print host is not anchored to the page origin`)
  assert.ok(Math.abs(state.topGapPx) < 0.5, `${label}: receipt has a ${state.topGapPx}px top offset`)
  assert.ok(Math.abs(state.leftGapPx) < 0.5, `${label}: receipt has a ${state.leftGapPx}px left offset`)
  assert.ok(state.tailPx >= 0 && state.tailPx <= mmToPx(1.5), `${label}: blank tail is ${state.tailPx}px`)
  assert.notEqual(state.host.page, 'receipt')
  assert.notEqual(state.copy.page, 'receipt')
  assert.deepEqual(state.visibleBodyChildren, ['orimia-receipt-roll-host-v579'])
  assert.equal(state.containsApplicationChrome, false)
  assert.ok(state.pageRule.includes(`80mm ${metrics.heightMm}mm`))
  assert.ok(state.pageRule.includes('@top-center { content: none; }'))
  assert.ok(state.pageRule.includes('@bottom-center { content: none; }'))

  const printLayerPath = path.join(artifactRoot, `receipt-v579-${label}-print-layer.png`)
  await page.locator('#orimia-receipt-roll-host-v579').screenshot({ path:printLayerPath })

  const pdfPath = path.join(artifactRoot, `receipt-v579-${label}.pdf`)
...
try {
  ...
  await desktop.page.waitForFunction(previousHeight => (
    window.__orimiaReceiptPhysicalRollV579.metrics()?.heightMm > previousHeight + 100
  ), shortMetrics.heightMm)
  const longMetrics = await desktop.page.evaluate(() => window.__orimiaReceiptPhysicalRollV579.metrics())
  ...
  const mobileMetrics = await mobile.page.evaluate(() => window.__orimiaReceiptPhysicalRollV579.metrics())
