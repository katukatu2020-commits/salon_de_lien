...
const ready = await fetch(`${baseUrl}/api/health/ready?integration=v579`, { headers:{ 'Cache-Control':'no-cache' } })
assert.equal(ready.status, 200)
assert.equal(ready.headers.get('x-lien-receipt-physical-roll'), 'v579')
assert.equal(ready.headers.get('x-lien-admin-sidebar-labels'), 'v578')
...
assert.match(receiptHtml, /orimia-receipt-physical-roll-script-v579/)
assert.doesNotMatch(receiptHtml, /orimia-receipt-roll-style-v569/)
assert.doesNotMatch(receiptHtml, /orimia-receipt-roll-script-v569/)
...
assert.doesNotMatch(adminHtml, /orimia-receipt-physical-roll-script-v579/)

const scriptResponse = await fetch(`${baseUrl}/receipt-physical-roll-v579.js?integration=v579`)
const styleResponse = await fetch(`${baseUrl}/receipt-physical-roll-v579.css?integration=v579`)
assert.equal(scriptResponse.status, 200)
assert.equal(styleResponse.status, 200)
assert.match(await scriptResponse.text(), /__orimiaReceiptPhysicalRollV579/)
assert.match(await styleResponse.text(), /--orimia-receipt-roll-height-v579/)
