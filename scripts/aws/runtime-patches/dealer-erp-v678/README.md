...
- `/dealer/receivables`: salon terms/payment schedules, close unbilled deliveries/returns by a chosen date, immutable invoice snapshots, one aggregate discount row (no line discount rates), credit invoices, manually recorded partial payments/refunds, allocation across invoices, allocation reversal and invoice/payment voiding. Duplicate transaction references are rejected. Payment CSV import and live bank integration are excluded per the user's explicit preference for manual entry first.
...
7. Payments are manual-entry only. No payment CSV import, live bank API, bank-format auto-detection, supplier EDI, external accounting submission, SMS/email push notifications or automatic bank debit collection is claimed. Order/report CSV exports remain available. Notifications are in-app unread counts. System usage-fee bank debit configuration is unchanged.
...
`integration.mjs` creates and removes only its own random schema. Never reset the shared local database. It tests the 10/6/4 partial-delivery case, return/refund amounts, manual partial payments, reversals, immutable invoice display, cross-tenant and staff access, CSRF, NFKC search, transaction rollback, concurrent allocation, retry protection, duplicate payment references, unavailable CSV-import routes, old/new sales integration, timezone stability and two-way messages.
