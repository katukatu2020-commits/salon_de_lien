(() => {
  ...
  function toolbar() {
    ...
    if (view === 'receivables' && isAdmin()) buttons.push(button('締め・請求書発行','data-create="invoice"','Plus',true),button('入金を記録','data-create="payment"','Plus'))
  ...
  function actionLabel(a) { return ({ warehouse:'倉庫登録',location:'保管場所登録',stock:'在庫更新',reserve:'引当',ship:'出荷',deliver:'納品','shipment-reverse':'出荷取消',return:'返品',invoice:'請求発行','invoice-void':'請求取消',payment:'入金記録',settle:'入金消込','settlement-undo':'消込解除','payment-void':'入金取消',purchase:'仕入発注',receive:'仕入更新',terms:'取引条件更新','branch-goal':'営業所目標設定',activity:'活動更新',thread:'連絡作成',message:'メッセージ送信','order-edit':'受注変更' })[a] || a }
  function body(d) {
