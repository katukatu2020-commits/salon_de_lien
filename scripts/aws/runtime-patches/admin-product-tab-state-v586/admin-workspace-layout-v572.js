;(() => {
  ...
  function routeMatchScore(value) {
    try {
      ...
      if (normalizedPathname(target.pathname) !== normalizedPathname(current.pathname)) return -1

      const targetEntries = [...target.searchParams.entries()]
      const currentEntries = [...current.searchParams.entries()]
      for (const [key, expected] of targetEntries) {
        if (!current.searchParams.getAll(key).includes(expected)) return -1
      }

      const exact = targetEntries.length === currentEntries.length
      return (exact ? 1000 : 0) + targetEntries.length
    } catch {
      return -1
    }
  }
  ...
  function preservedNavigations(record) {
    const navigations = new Set([...record.preserved].filter(element => element.isConnected))
    for (const selector of record.preserveSelectors) {
      try {
        document.querySelectorAll(selector).forEach(element => navigations.add(element))
      } catch {}
    }
    return navigations
  }
  ...
  function syncSegmentsToLocation(record) {
    for (const nav of preservedNavigations(record)) {
      const links = [...nav.querySelectorAll('a[href]')]
      let activeLink = null
      let activeScore = -1
      for (const link of links) {
        const score = routeMatchScore(link.getAttribute('href'))
