...
assert.equal(count(managed, "SELECT 1::int AS locked FROM (SELECT pg_advisory_xact_lock(hashtext('managed_account_issue_v629'))) AS guard"), 2)
...
assert.match(migration, /createRequire\(path\.join\(process\.env\.LIEN_RUNTIME_ROOT \|\| '\/app', 'package\.json'\)\)/)
...
assert.match(migration, /SELECT 1::int AS locked FROM \(SELECT pg_advisory_xact_lock\(hashtext\('managed_bank_debit_migration_v629'\)\)\) AS guard/)
