...
const headers = {
  ...
  "User-Agent": "ORIMIA-customer-comment-owner-v560-smoke/1.0",
};

const ready = await fetch(`${baseUrl}/api/health/ready?smoke=v560`, {
  headers,
  cache: "no-store",
});
...
const community = await fetch(`${baseUrl}/u/community?smoke=v560`, {
...
const detail = await fetch(
  `${baseUrl}/u/community/${encodeURIComponent(ownedPost.postId)}?smoke=v560`,
  {
    headers: { ...headers, Cookie: cookie },
    cache: "no-store",
  },
...
assert.match(detailHtml, /content-edit-delete-client-v560\.js/);
assert.doesNotMatch(detailHtml, /content-edit-delete-client-v559\.js/);

const client = await fetch(
  `${baseUrl}/content-edit-delete-client-v560.js?smoke=v560`,
  { headers, cache: "no-store" },
);
assert.equal(client.status, 200);
const source = await client.text();
assert.match(source, /__lienStyleCommunityControlsV560/);
assert.match(source, /buildCustomerCommentMenu/);
assert.match(source, /lienCommentSignatureV560/);
