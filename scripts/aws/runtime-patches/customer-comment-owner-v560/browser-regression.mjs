...
const screenshotRoot =
  ...
  path.join(os.tmpdir(), "orimia-delete-confirmation-v560");
...
async function findPublishedPost(context) {
  const response = await context.request.get(
    `${baseUrl}/u/community?browser=v560`,
    {
...
async function verifyViewport(name, viewport) {
  ...
  let createdId = "";
  let removed = false;
  const originalBody = `v560 ${name} 動作確認 ${Date.now()}`;
  ...
  try {
    ...
    if (injectedClient) {
      await page.route("**/content-edit-delete-client-v559.js*", (route) =>
        route.fulfill({
          status: 200,
    ...
    await page.goto(
      `${baseUrl}/u/community/${encodeURIComponent(postId)}?browser=v560-${name}`,
      {
    ...
    const trigger = card.locator(
      '[data-lien-customer-comment-menu="v560"] .lien-comment-menu-trigger',
    );
    await trigger.waitFor({ state: "visible", timeout: 12_000 });
    assert.equal(await trigger.getAttribute("aria-expanded"), "false");
    const headerExpansion = await card.evaluate((element) => {
      const heading = element.firstElementChild;
      const metadata = heading?.firstElementChild;
      if (!heading || !metadata) return Number.POSITIVE_INFINITY;
      return Math.abs(
    ...
    assert.ok(
      headerExpansion <= 1,
      `${name}: action trigger expands the comment header by ${headerExpansion}px`,
    );
    ...
    assert.equal(
      await page.locator('[data-lien-customer-comment-menu="v560"]').count(),
      ownedCount,
    );
    assert.equal(
...
async function verifyStaffPostDialog() {
  ...
  try {
    ...
    await page.goto(`${baseUrl}/admin/community?browser=v560-staff`, {
      waitUntil: "domcontentloaded",
      timeout: 20_000,
    });
    if (injectedClient)
