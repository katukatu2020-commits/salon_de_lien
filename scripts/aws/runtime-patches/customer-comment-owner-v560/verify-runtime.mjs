...
const client = read("public/content-edit-delete-client-v560.js");
...
const appShellChunks = fs
  ...
  .filter((source) =>
    source.includes('"data-lien-community-bootstrap": "v560"'),
  );
const pageFiles = [
...
assert.match(appShellChunks[0], /content-edit-delete-client-v560\.js/);
assert.doesNotMatch(appShellChunks[0], /content-edit-delete-client-v559\.js/);
assert.match(staffRuntime, /__lienStyleCommunityLoaderV560/);
assert.match(staffRuntime, /content-edit-delete-client-v560\.js/);
assert.doesNotMatch(staffRuntime, /content-edit-delete-client-v559\.js/);

for (const file of pageFiles) {
  const source = read(file);
  assert.match(
    source,
    /content-edit-delete-client-v560\.js/,
    `${file} does not load v560`,
  );
  assert.doesNotMatch(
    source,
    /content-edit-delete-client-v559\.js/,
    `${file} still loads v559`,
  );
}

assert.match(client, /window\.__lienStyleCommunityControlsV560 = true/);
assert.match(client, /dataset\.lienCustomerCommentMenu = ["']v560["']/);
assert.match(
  client,
  /\.lien-comment-menu-shell\{position:absolute;top:-10px;right:-8px;z-index:2/,
);
assert.doesNotMatch(client, /\.lien-comment-menu-shell\{position:relative/);
assert.match(client, /heading\.style\.position = ["']relative["']/);
assert.match(client, /heading\.style\.paddingRight = ["']44px["']/);
