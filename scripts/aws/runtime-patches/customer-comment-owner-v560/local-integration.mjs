...
const ids = {
  targetCustomer: `v560-target-${suffix}`,
  sourceCustomer: `v560-source-${suffix}`,
  otherCustomer: `v560-other-${suffix}`,
  targetUser: `v560-target-user-${suffix}`,
  sourceUser: `v560-source-user-${suffix}`,
  otherUser: `v560-other-user-${suffix}`,
  post: `v560-post-${suffix}`,
  ownedComment: `v560-owned-comment-${suffix}`,
  otherComment: `v560-other-comment-${suffix}`,
  merge: `v560-merge-${suffix}`
};
const loginId = `v560.owner.${suffix}`;
...
const artifactDirectory = path.resolve(".artifacts", "customer-comment-owner-v560");
  ...
  await prisma.appUser.createMany({
    data: [
      ...
      },
      {
        ...
        email: `v560.legacy.${suffix}@example.test`,
        loginId: `v560.legacy.${suffix}`,
      ...
      },
      {
        ...
        email: `v560.other.${suffix}@example.test`,
        loginId: `v560.other.${suffix}`,
...
async function verifyViewport(browser, token, name, viewport) {
  ...
  await page.goto(`${baseUrl}/u/community/${encodeURIComponent(ids.post)}?integration=v560-${name}`, {
    waitUntil: "domcontentloaded"
  });
  await page.waitForFunction(() => window.__lienStyleCommunityControlsV560 === true);
  ...
  const ownedTrigger = ownedCard.locator('[data-lien-customer-comment-menu="v560"] .lien-comment-menu-trigger');
  ...
  const [ownedBox, otherBox] = await Promise.all([
    ownedCard.boundingBox(),
    otherCard.boundingBox()
  ]);
  assert.ok(ownedBox && otherBox, `${name}: comment card bounds are unavailable`);
  const cardHeightDelta = Math.abs(ownedBox.height - otherBox.height);
  assert.ok(cardHeightDelta <= 2, `${name}: owned comment is ${cardHeightDelta}px taller than a matching comment`);
  ...
  return { context, page, ownedCard, cardHeightDelta };
}
...
try {
  ...
  console.log(JSON.stringify({
    ...
    mobile: true,
    cardHeightDelta: {
      desktop: desktop.cardHeightDelta,
      mobile: mobile.cardHeightDelta
    },
