...
const screenshotRoot =
  ...
  path.join(os.tmpdir(), "orimia-comment-menu-v557");
...
async function findPublishedPost(context) {
  const response = await context.request.get(
    `${baseUrl}/u/community?browser=v557`,
    {
...
async function verifyViewport(name, viewport) {
  ...
  let createdId = "";
  let removed = false;
  const originalBody = `v557 ${name} 動作確認 ${Date.now()}`;
  ...
  try {
    ...
    if (injectedClient) {
      await page.route("**/content-edit-delete-client-v509.js*", (route) =>
        route.fulfill({
          status: 200,
    ...
    await page.goto(
      `${baseUrl}/u/community/${encodeURIComponent(postId)}?browser=v557-${name}`,
      {
    ...
    const trigger = card.locator(
      '[data-lien-customer-comment-menu="v557"] .lien-comment-menu-trigger',
    );
    ...
    assert.equal(
      await page.locator('[data-lien-customer-comment-menu="v557"]').count(),
      ownedCount,
    );
    assert.equal(
