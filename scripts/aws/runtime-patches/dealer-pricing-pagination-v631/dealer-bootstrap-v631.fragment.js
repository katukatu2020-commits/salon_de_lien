  async function dealerBootstrap(session, url) {
    ...
    const pricingSearch = String(url?.searchParams.get('pricingSearch') || '').normalize('NFKC').trim().slice(0, 180)
    ...
    const productResultPromise = productPageMode
      ? (async () => {
          ...
          return {
            products,
            productPagination: { page, pageSize: DEALER_PRODUCT_PAGE_SIZE, totalCount, totalPages, query: productSearch },
            ...
            const [totals, configuredTotals] = await Promise.all([
              prisma.$queryRawUnsafe(
                `SELECT COUNT(*)::int AS "totalProductCount",
                        (COUNT(*) FILTER (WHERE $2='' OR CONCAT_WS(' ',"manufacturerName","name",COALESCE("category",''),"productCode",COALESCE("janCode",'')) ILIKE '%' || $2 || '%'))::int AS "count"
                   FROM "WholesaleDealerProduct"
                  WHERE "dealerId"=$1 AND "active"=TRUE`,
                session.id,
                pricingSearch,
    ...
    const [orders, productResult] = await Promise.all([
