...
const manifest = '/tmp/customer-style-first-paint-v606-parent.json'
const changesPath = '/tmp/customer-style-first-paint-v606-changes.json'
...
if (process.argv[2] === 'snapshot') {
  ...
  assert.match(server, /X-Lien-Customer-Style-First-Paint', 'v606'/)
  assert.match(server, /orimia-style-first-paint-v606/)
  assert.match(server, /orimia-style-first-paint-v606-guard/)
  assert.match(server, /visibility:hidden!important/)
  assert.match(server, /article::before/)
  assert.match(server, /article::after/)
  assert.match(server, /styleDetailPreflightV606/)
  assert.match(server, /style-system-integration-v602\.css\?v=606-first-paint1/)
  assert.match(server, /style-system-integration-v602\.js\?v=606-first-paint1/)
  assert.match(client, /function revealStyleV606\(state\)/)
  assert.match(client, /orimia-style-first-paint-v606-guard/)
  assert.match(client, /window\.__orimiaStyleNavigationV606 = true/)
  assert.match(client, /location\.assign\(target\.href\)/)
  assert.match(client, /revealStyleV606\('ready'\)/)
  assert.match(client, /revealStyleV606\('fallback'\)/)
  assert.match(client, /if \(!gallery \|\| !content \|\| !meta\) return false/)
  ...
  console.log(JSON.stringify({
    ...
    modified,
    added,
    oldUiHiddenBeforeData:true,
    structuredUiRevealedAtomically:true,
    failureFallback:true,
    customerStyleV605Preserved:true,
