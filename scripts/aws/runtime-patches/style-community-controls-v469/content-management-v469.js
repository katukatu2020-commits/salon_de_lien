...
function createContentManagementService({ prisma, staffSessionProvider, customerSessionProvider, canAccessThread }) {
  ...
  async function postList(res, session, audience) {
    ...
    const posts = await prisma.$queryRawUnsafe(
      'SELECT "id","postKind","caption","published","publishedAt","publishedByName","updatedAt" FROM "VisitCommunityPost" WHERE "organizationId"=$1 ORDER BY "publishedAt" DESC,"id" DESC LIMIT 300',
  ...
  async function contentGet(req, res, url, session, audience) {
    ...
    const posts = await prisma.$queryRawUnsafe(
      'SELECT "id","organizationId","customerId","postKind","caption","published","updatedAt" FROM "VisitCommunityPost" WHERE "id"=$1 AND "organizationId"=$2 LIMIT 1',
      postId,
      session.organizationId,
    )
    const post = posts[0]
  ...
  async function contentMutation(req, res, session, audience) {
    ...
    if (!postId) return json(res, 400, { error: '投稿を指定してください。' })
    const posts = await prisma.$queryRawUnsafe(
      'SELECT "id","organizationId","customerId","postKind","published" FROM "VisitCommunityPost" WHERE "id"=$1 AND "organizationId"=$2 LIMIT 1',
      postId,
      session.organizationId,
    )
    const post = posts[0]
    ...
    if (target === 'post') {
      ...
      if (req.method === 'DELETE') {
        await prisma.$executeRawUnsafe(
          'DELETE FROM "VisitCommunityPost" WHERE "id"=$1 AND "organizationId"=$2',
          post.id,
          session.organizationId,
        )
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      console.error('[style-community-controls-v469]', {
