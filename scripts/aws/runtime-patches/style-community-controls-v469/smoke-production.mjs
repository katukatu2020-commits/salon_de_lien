...
const asset = await fetch(`${baseUrl}/content-edit-delete-client-v469.js`, { redirect: 'manual' })
assert.equal(asset.status, 200)
assert.equal(asset.headers.get('x-content-type-options'), 'nosniff')
const source = await asset.text()
assert.match(source, /window\.__lienStyleCommunityControlsV469/)
assert.match(source, /投稿の公開管理/)
...
console.log(`style community controls v469 production smoke passed (${baseUrl})`)
