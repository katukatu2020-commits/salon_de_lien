...
const servicePath = process.env.LIEN_CONTENT_SERVICE || './content-management-v469.js'
...
const prisma = {
  async $queryRawUnsafe(sql) {
    statements.push(sql)
    if (sql.includes('ORDER BY "publishedAt"')) {
      return [
        { id: 'post-public', postKind: 'VISIT', caption: '公開投稿', published: true, publishedAt: new Date('2026-08-01T00:00:00Z'), publishedByName: '', updatedAt: new Date() },
        { id: 'post-hidden', postKind: 'STORE', caption: '非公開投稿', published: false, publishedAt: new Date('2026-07-01T00:00:00Z'), publishedByName: 'staff', updatedAt: new Date() },
      ]
    }
...
console.log('style community controls v469 ownership tests passed')
