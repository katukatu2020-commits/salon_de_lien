...
async function auditAdmin() {
  ...
  try {
    ...
    const campaign = await browser.evaluate(`({tabs:[...document.querySelectorAll('.workspace-tabs a')].map(a=>a.textContent.trim()),hasSearch:Boolean(document.querySelector('.top-search')),headerActions:document.querySelectorAll('.top-actions>a').length,collapseControls:document.querySelectorAll('#campaign-side-toggle').length,activeSidebarBackground:getComputedStyle(document.querySelector('.nav a.active')).backgroundColor,editable:document.querySelectorAll('[data-edit]').length,deletable:document.querySelectorAll('[data-delete]').length,fields:document.querySelectorAll('#campaign-form input,#campaign-form select,#campaign-form textarea').length})`);
    checks.push({ name: "campaign management controls", pass: campaign.tabs.length === 4 && !campaign.hasSearch && campaign.headerActions === 4 && campaign.collapseControls === 1 && campaign.fields >= 8, details: campaign });
