(() => {
  ...
  const css = `
    ...
    @media(max-width:767px){
      ...
      .bottom-nav,nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"]{position:fixed!important;z-index:80!important;right:0!important;bottom:0!important;left:0!important;width:100%!important;height:calc(var(--customer-nav-height) + env(safe-area-inset-bottom))!important;min-height:calc(var(--customer-nav-height) + env(safe-area-inset-bottom))!important;border-top:1px solid #eadfd4!important;background:rgba(255,253,249,.97)!important;padding:0 4px env(safe-area-inset-bottom)!important;box-shadow:0 -8px 24px rgba(62,42,35,.07)!important;backdrop-filter:blur(14px)!important}
      .bottom-nav,nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"]>div{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;align-items:stretch!important}
      nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"]>div{width:100%!important;height:var(--customer-nav-height)!important;min-height:var(--customer-nav-height)!important;margin:0!important;padding:0!important}
      .bottom-nav .bottom-link,nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"] a{display:flex!important;min-width:0!important;min-height:var(--customer-nav-height)!important;align-items:center!important;justify-content:center!important;gap:3px!important;border-radius:12px!important;padding:5px 2px!important;font-size:10px!important;line-height:1.15!important;text-decoration:none!important;white-space:nowrap!important}
      .bottom-nav .bottom-link svg,nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"] a svg{width:20px!important;height:20px!important;min-width:20px!important;min-height:20px!important}
      .bottom-nav .bottom-link span,nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"] a span{overflow:hidden!important;max-width:100%!important;text-overflow:ellipsis!important;white-space:nowrap!important}
      body:has(.customer-premium-topbar):not(#customer-premium-shell) [class~="bg-[#fbf7f0]"]>nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"]{height:calc(var(--customer-nav-height) + env(safe-area-inset-bottom))!important;min-height:calc(var(--customer-nav-height) + env(safe-area-inset-bottom))!important;padding:0 4px env(safe-area-inset-bottom)!important;background:rgba(255,253,249,.97)!important}
      body:has(.customer-premium-topbar):not(#customer-premium-shell) [class~="bg-[#fbf7f0]"]>nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"] a{min-height:var(--customer-nav-height)!important;border-radius:12px!important;padding:5px 2px!important;font-size:10px!important;line-height:1.15!important}
    ...
    .cx-nickname-card{margin-top:18px;border:1px solid #e8ded2;border-radius:20px;background:#fff;padding:18px;box-shadow:0 5px 18px rgba(70,50,40,.06)}
    .cx-nickname-card h2{margin:0;font-size:16px;font-weight:700;color:#382f2a}
    .cx-nickname-card p{margin:7px 0 14px;font-size:12px;line-height:1.7;color:#7c7168}
    .cx-nickname-row{display:flex;gap:8px}
    .cx-nickname-row input{min-width:0;min-height:48px;flex:1;border:1px solid #d8cbbf;border-radius:14px;background:#fff;padding:0 14px;color:#2f2a25;font:inherit;outline:none}
    .cx-nickname-row input:focus{border-color:#8f4f42;box-shadow:0 0 0 4px rgba(233,201,190,.45)}
    .cx-nickname-row button{min-height:48px;border:0;border-radius:999px;background:#8f4f42;padding:0 20px;color:#fff;font:inherit;font-weight:700}
    .cx-nickname-status{display:block;min-height:20px;margin-top:9px;font-size:12px;color:#356143}
    ...
    .cx-customer-nav-link{color:#8b8178!important}.cx-customer-nav-link:hover{background:#f6efe6!important;color:#5b332c!important}.cx-customer-nav-active{background:#f7e7e1!important;color:#8f4f42!important;box-shadow:inset 0 0 0 1px #ead0c7!important}
    @media(max-width:470px){.cx-nickname-row{display:grid}.cx-nickname-row button{width:100%}}
  `
  const style = document.createElement('style')
  style.dataset.lienCustomerExperience = '278'
  ...
  function normalizeBottomNavigation() {
    const definitions = [
      ...
      { href: '/u/chat', label: 'チャット', icon: 'chat' },
    ]
    document.querySelectorAll('.bottom-nav,nav.fixed.inset-x-0.bottom-0[aria-label="お客様アプリメニュー"]').forEach(nav => {
      ...
      const links = definitions.map(item => {
        const link = document.createElement('a')
        link.href = item.href
        link.className = nav.matches('.bottom-nav') ? 'bottom-link cx-customer-nav-link' : 'cx-customer-nav-link'
...
function replaceNewsLinks() {
  ...
  async function addNicknameEditor() {
    if (location.pathname !== '/u/profile' || document.querySelector('.cx-nickname-card,input[name="nickname"]')) return
    const main = document.querySelector('main')
    if (!main) return
    const card = document.createElement('section')
    card.className = 'cx-nickname-card'
    card.innerHTML = '<h2>ニックネーム</h2><p>スタイル共有やコメントなど、ほかのお客様に公開される場所では本名の代わりに表示されます。</p><form class="cx-nickname-row"><input name="nickname" maxlength="30" autocomplete="nickname" placeholder="例：ひなた"><button type="submit">保存</button></form><output class="cx-nickname-status" aria-live="polite"></output>'
    const container = main.querySelector('.grid') || main.firstElementChild || main
    ...
    card.querySelector('form').addEventListener('submit', async event => {
      ...
      try {
        ...
        input.value = result.nickname
        output.textContent = 'ニックネームを保存しました。'
      } catch (error) { output.textContent = error.message }
...
function staffHue(value) {
  ...
  const start = () => window.setTimeout(() => {
    boot()
    new MutationObserver(() => { applyCustomerConsistency(); ensureMobileCustomerStoreIcon(); replaceNewsLinks(); applyCommunityNickname() }).observe(document.documentElement, { childList: true, subtree: true })
  }, 250)
