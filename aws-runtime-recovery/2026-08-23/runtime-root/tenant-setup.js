...
function createTenantSetupService({ prisma, sessionProvider, customerSessionProvider, crypto }) {
  ...
  async function customerAvailability(req, res, url, session) {
    ...
    const [menu, staff, schedule, dailyOverrides, defaultCapacity] = await Promise.all([
    ...
    const days = dates.map(date => {
      ...
      const slots = []
      for (let minutes = daySchedule.openMinutes; minutes + Number(menu.durationMinutes) <= daySchedule.closeMinutes; minutes += 30) {
      ...
      return { date, available: slots.length > 0, slots, schedule: daySchedule }
    })
  ...
  async function customerBook(req, res, session) {
    ...
    const [menu, staff, schedule] = await Promise.all([tenantMenu(session.organizationId, menuKey), staffRows(session.organizationId), dailySchedule(session.organizationId, date)])
    if (!menu || !staff.length || (staffKey !== 'free' && !staff.some(row => row.staffKey === staffKey))) return json(res, 400, { error: 'メニューまたはスタッフを確認してください。' })
    ...
    try {
      const result = await prisma.$transaction(async transaction => {
        const currentSchedule = await dailySchedule(session.organizationId, date, transaction)
  ...
  async function handle(req, res, url) {
    ...
    if (url.pathname === '/api/lien-business-days' && ['GET','POST'].includes(req.method)) { await businessDays(req, res, url); return true }
