...
((exports.id = 1759),
  ...
  (exports.modules = {
    ...
    60353: (e, t, r) => {
      ...
      function u(e) {
        let t = e.visits[0];
        if (t) return `前回担当: ${i(t.stylistName) ?? "フリー"}`;
        let a=[...(e.appointments??[])].filter(e=>{let t=(e.status??'').toLowerCase();return e.staffName&&new Date(e.scheduledAt??0).getTime()<=Date.now()&&!t.includes('cancel')&&!t.includes('キャンセル')}).sort((e,t)=>new Date(t.scheduledAt??0).getTime()-new Date(e.scheduledAt??0).getTime())[0];if(a)return `前回担当: ${i(a.staffName)??"フリー"}`;let r =
