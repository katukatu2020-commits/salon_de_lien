;(() => {
  ...
  window.__lienAttendanceProductV320 = true
  ...
  style.dataset.lienAttendanceV320 = '1'
  style.textContent = `
  ...
  const duration = row => {
    if (!row.clockOutAt) return '勤務中'
    const minutes = Math.max(0, Math.round((new Date(row.clockOutAt) - new Date(row.clockInAt)) / 60000))
  ...
  function attendanceMarkup(data, view = 'clock') {
    ...
    const table = rows.length ? rows.map(row => `<tr><td><strong>${esc(row.displayName || 'スタッフ')}</strong></td><td>${esc(row.workDate)}</td><td>${formatTime(row.clockInAt)}</td><td>${formatTime(row.clockOutAt)}</td><td><span class="ca-attendance-pill ${row.clockOutAt ? '' : 'open'}">${esc(duration(row))}</span></td></tr>`).join('') : '<tr><td colspan="5" class="empty">この月の出退勤記録はまだありません。</td></tr>'
    ...
    const todayRows = (data.today || []).map(row => { const status = row.clockOutAt ? '退勤' : row.breakStartedAt && !row.breakEndedAt ? '休憩中' : row.clockInAt ? '出勤' : '未出勤'; return `<tr><td><strong>${esc(row.displayName)}</strong></td><td><span class="ca-attendance-pill ${status === '出勤' ? 'open' : status === '休憩中' ? 'break' : ''}">${status}</span></td><td>${esc(row.note || '')}</td><td>${formatTime(row.clockInAt)}</td><td>${formatTime(row.breakStartedAt)}</td><td>${formatTime(row.breakEndedAt)}</td><td>${formatTime(row.clockOutAt)}</td></tr>` }).join('')
    ...
    const clockView = `<div class="ca-attendance-grid"><section class="ca-attendance-card"><h2>今日の打刻</h2><p>スタッフを選び、店舗端末から出勤・休憩・退勤を記録します。</p><label class="ca-attendance-person"><span>打刻するスタッフ</span><select data-attendance-person>${staffOptions}</select></label><div class="ca-attendance-status"><div class="ca-attendance-status-row"><strong>${finished ? '退勤済み' : working ? onBreak ? '休憩中' : '出勤中' : '未出勤'}</strong><time>${selectedRecord?.clockInAt ? formatTime(selectedRecord.clockInAt) + ' 出勤' : '—'}</time></div><div class="ca-attendance-actions">${!working ? `<button type="button" class="ca-attendance-action" data-attendance-action="clock_in" ${finished || !selectedStaff ? 'disabled' : ''}>${icon('in')}出勤する</button>` : onBreak ? `<button type="button" class="ca-attendance-action secondary" data-attendance-action="break_end">${icon('in')}休憩から戻る</button>` : `<button type="button" class="ca-attendance-action secondary" data-attendance-action="break_start">${icon('clock')}休憩に入る</button>`}<button type="button" class="ca-attendance-action is-out" data-attendance-action="clock_out" ${working ? '' : 'disabled'}>${icon('out')}退勤する</button></div></div><div class="ca-attendance-feedback" role="status"></div></section><section class="ca-attendance-card"><h2>スタッフの最新状態</h2><p>今日の出社・休憩・退社時刻を一覧で確認できます。</p><div class="ca-attendance-table-wrap"><table class="ca-attendance-table"><thead><tr><th>スタッフ</th><th>状況</th><th>備考</th><th>出社</th><th>休憩出</th><th>休憩戻</th><th>退社</th></tr></thead><tbody>${todayRows || '<tr><td colspan="7" class="empty">スタッフが登録されていません。</td></tr>'}</tbody></table></div></section></div>`
    const listView = `<section class="ca-attendance-card"><div class="ca-attendance-month"><div><h2>${data.canViewAll ? '店舗の出退勤リスト' : '自分の出退勤リスト'}</h2><p style="margin:5px 0 0;color:#87766f;font-size:10px">月別の記録を表示します。</p></div><input type="month" value="${esc(data.month)}" data-attendance-month aria-label="表示月"></div><div class="ca-attendance-table-wrap"><table class="ca-attendance-table"><thead><tr><th>スタッフ</th><th>勤務日</th><th>出勤</th><th>退勤</th><th>勤務状況</th></tr></thead><tbody>${table}</tbody></table></div></section>`
  ...
  async function renderAttendance(month, view = window.__lienAttendanceView || 'clock') {
    ...
    document.querySelectorAll('header p').forEach(node => { if (node.textContent?.trim() === 'アカウント設定') node.textContent = '出退勤管理' })
    try {
  ...
  const observer = new MutationObserver(() => {
    ...
    if (isAttendanceRoute()) {
      ...
      else Array.from(main.children).forEach(node => { if (node !== root) { node.hidden = true; node.style.display = 'none' } })
    }
    enhanceProductImages()
  })
  observer.observe(document.documentElement, { childList: true, subtree: true })
  document.addEventListener('DOMContentLoaded', () => { document.querySelectorAll('form').forEach(setupProductImage); renderAttendance(); enhanceProductImages() })
  renderAttendance(); enhanceProductImages()
})()
