...
function createInboundEmailService({ prisma, sessionProvider, crypto, parseReservationMail, normalizePhone }) {
  ...
  async function importParsedReservation(organizationId, parsed, payload) {
    ...
    const appointment = await prisma.appointment.upsert({
      ...
      update: { customerId: customer.id, scheduledAt: parsed.scheduledAt, durationMinutes: parsed.durationMinutes, menu: parsed.menu, staffName: mergedStaffName, estimatedPrice, status: parsed.status, source: `ses:${messageHash}`, bookingProvider: parsed.provider, note },
