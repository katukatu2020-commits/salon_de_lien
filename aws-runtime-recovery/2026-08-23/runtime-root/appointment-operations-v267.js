...
function createAppointmentOperationsService({ prisma, crypto, sessionProvider, customerSessionProvider, runtimeScript }) {
  ...
  async function resolveStaff(db, organizationId, requestedName, requestedStaffKey = null) {
    ...
    let settings = await db.$queryRawUnsafe(
    ...
    if (!settings.length && organizationId === 'org_salon_de_lien') settings = LEGACY_STAFF.map(row => ({ ...row, workStartMinutes: hours.openMinutes, workEndMinutes: hours.closeMinutes }))
  ...
  async function createManual(req, res) {
    ...
    if (submittedPrice !== null && (!Number.isInteger(submittedPrice) || submittedPrice < 0 || submittedPrice > 1000000)) throw new RequestError('見込み金額を確認してください。')
