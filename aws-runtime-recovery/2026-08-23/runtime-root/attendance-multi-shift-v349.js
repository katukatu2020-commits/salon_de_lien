...
function createAttendanceNotificationProductService({ prisma, crypto, sessionProvider }) {
  ...
  async function attendance(req, res, url, session) {
    if (req.method === 'GET') {
      ...
      const today = await prisma.$queryRawUnsafe(`SELECT s."staffKey" AS "userId",s."staffKey",s."staffName" AS "displayName",r."id",r."clockInAt",r."breakStartedAt",r."breakEndedAt",r."clockOutAt",r."note",COALESCE(a."shiftCount",0)::INTEGER AS "shiftCount",COALESCE(a."totalWorkMinutes",0)::INTEGER AS "totalWorkMinutes",COALESCE(a."totalBreakMinutes",0)::INTEGER AS "totalBreakMinutes",COALESCE(p."plannedStart",'10:00') AS "plannedStart",COALESCE(p."plannedEnd",'19:00') AS "plannedEnd"
        FROM "StaffBookingSetting" s
        LEFT JOIN LATERAL (SELECT x.* FROM "StaffAttendanceRecord" x WHERE x."organizationId"=s."organizationId" AND x."staffKey"=s."staffKey" AND x."workDate"=$2 ORDER BY (x."clockOutAt" IS NULL) DESC,x."clockInAt" DESC LIMIT 1) r ON TRUE
        ...
        LEFT JOIN "StaffAttendancePolicy" p ON p."organizationId"=s."organizationId" AND p."staffKey"=s."staffKey" WHERE s."organizationId"=$1 AND s."active"=TRUE AND s."onLeave"=FALSE ORDER BY s."createdAt",s."staffName"`, session.organizationId, tokyoDate())
      ...
      return json(res, 200, { ok: true, month, canViewAll: true, people, records, summaries, today, policies, serverTime: new Date().toISOString() })
    }
    ...
    if (action === 'clock_in') {
      const existing = await prisma.$queryRawUnsafe('SELECT "id" FROM "StaffAttendanceRecord" WHERE "organizationId"=$1 AND "staffKey"=$2 AND "clockOutAt" IS NULL LIMIT 1', session.organizationId, staffKey)
      if (existing.length) return json(res, 409, { ok: false, error: 'すでに出勤中です。' })
    ...
    if (action === 'clock_out') {
      const rows = await prisma.$queryRawUnsafe('UPDATE "StaffAttendanceRecord" SET "breakSeconds"="breakSeconds"+CASE WHEN "breakStartedAt" IS NOT NULL AND "breakEndedAt" IS NULL THEN GREATEST(0,EXTRACT(EPOCH FROM (NOW()-"breakStartedAt"))::INTEGER) ELSE 0 END,"breakEndedAt"=CASE WHEN "breakStartedAt" IS NOT NULL AND "breakEndedAt" IS NULL THEN NOW() ELSE "breakEndedAt" END,"clockOutAt"=NOW(),"recordedByUserId"=$3,"updatedAt"=NOW() WHERE "id"=(SELECT "id" FROM "StaffAttendanceRecord" WHERE "organizationId"=$1 AND "staffKey"=$2 AND "clockOutAt" IS NULL ORDER BY "clockInAt" DESC LIMIT 1) RETURNING "id"', session.organizationId, staffKey, session.userId)
  ...
  async function handle(req, res, url) {
    ...
    try {
      ...
      console.error('[attendance-multi-shift-v349]', { organizationId: session.organizationId, path: url.pathname, error: error && error.message })
