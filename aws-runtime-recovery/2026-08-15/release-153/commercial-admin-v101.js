(() => {
  ...
  async function toggleNotificationPanel(button) {
    const panel = document.querySelector('[data-ca-notification-panel]')
    if (!panel || !button?.isConnected) return
    const opening = panel.hidden || button.getAttribute('aria-expanded') !== 'true'
    closeStoreMenus()
    ...
    panel.style.top = Math.min(window.innerHeight - 12, rect.bottom + 9) + 'px'
    panel.hidden = false
    button.setAttribute('aria-expanded', 'true')
    const payload = await refreshNotifications(true)
    if (!panel.hidden && payload) {
      markSystemNotificationsSeen(notificationItems(payload))
      state.notificationAppointmentsRead = true
      updateNotificationBadge(payload)
      fetch('/api/lien-staff-notifications?read=1', { credentials: 'same-origin', cache: 'no-store' })
        .then(() => { state.notificationAppointmentsRead = false })
        .catch(() => { state.notificationAppointmentsRead = false })
    }
  }
  ...
  function enhance() {
    styles(); applyAdminTheme(savedAdminTheme()); normalizeSidebarControl(); removeCommandPalette(); enhanceHeader(); enhanceMenuPage(); enhanceSettingsPage(); enhanceAccountTheme(); enhanceContextSettings()
  }
