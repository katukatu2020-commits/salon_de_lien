...
function createTenantSetupService({ prisma, sessionProvider, customerSessionProvider, crypto }) {
  ...
  async function ingestMessage(organizationId, gmailMessage) {
    ...
    if (!parsedResult.ok) {
      await prisma.$executeRawUnsafe(`INSERT INTO "GmailIngestMessage" ("id","organizationId","gmailMessageId","status","errorMessage","processedAt") VALUES ($1,$2,$3,'ignored',$4,CURRENT_TIMESTAMP) ON CONFLICT ("organizationId","gmailMessageId") DO UPDATE SET "status"='ignored',"errorMessage"=EXCLUDED."errorMessage","processedAt"=CURRENT_TIMESTAMP`, recordId, organizationId, gmailMessage.id, parsedResult.error)
    ...
    const appointment = await prisma.appointment.upsert({
      ...
      update: { customerId: customer.id, scheduledAt: parsed.scheduledAt, durationMinutes: parsed.durationMinutes, menu: parsed.menu, staffName: parsed.staffName, estimatedPrice, status: parsed.status, source: `gmail:${messageHash}`, bookingProvider: parsed.provider, note },
    ...
    await prisma.$executeRawUnsafe(`INSERT INTO "GmailIngestMessage" ("id","organizationId","gmailMessageId","status","appointmentId","processedAt") VALUES ($1,$2,$3,'imported',$4,CURRENT_TIMESTAMP) ON CONFLICT ("organizationId","gmailMessageId") DO UPDATE SET "status"='imported',"appointmentId"=EXCLUDED."appointmentId","errorMessage"=NULL,"processedAt"=CURRENT_TIMESTAMP`, recordId, organizationId, gmailMessage.id, appointment.id)
  ...
  async function syncOrganization(organizationId) {
    ...
    try {
      ...
      const knownRows = ids.length ? await prisma.$queryRawUnsafe(`SELECT "gmailMessageId" FROM "GmailIngestMessage" WHERE "organizationId"=$1 AND "gmailMessageId"=ANY($2::text[]) AND "status" IN ('imported','ignored')`, organizationId, ids) : []
  ...
  async function handle(req, res, url) {
    if (url.pathname === '/tenant-setup-client.js' && req.method === 'GET') {
      ...
      res.statusCode = 200; res.setHeader('Content-Type', 'application/javascript; charset=utf-8'); res.setHeader('Cache-Control', 'public, max-age=300'); res.end(fs.readFileSync(file)); return true
    }
