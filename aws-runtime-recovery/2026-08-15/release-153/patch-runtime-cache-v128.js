...
const oldMarker = 'admin-header-settings-v127'
const marker = 'admin-header-settings-v128'
...
function updateHelperUrls(source) {
  return source
    .replaceAll('/commercial-admin-v127.js?v=20260815-127', '/commercial-admin-v128.js?v=20260815-128')
    .replaceAll('/tenant-setup-client.js?v=20260815-127', '/tenant-setup-client.js?v=20260815-128')
}
...
function patchRuntimeCache() {
  ...
  if (layouts.length !== 1) throw new Error(`expected one active v127 admin layout, found ${layouts.length}`)
  const oldFile = layouts[0]
  const oldName = path.basename(oldFile)
  let layoutSource = updateHelperUrls(fs.readFileSync(oldFile, 'utf8'))
  if (!layoutSource.includes('/commercial-admin-v128.js?v=20260815-128') || !layoutSource.includes('/tenant-setup-client.js?v=20260815-128')) throw new Error('v128 helper URLs missing')
  if (!layoutSource.includes(marker)) layoutSource += `\n/* ${marker} */\n`
  new Function(layoutSource)
  const newName = oldName.replace(/\.tenant-runtime-v127\.js$/, '.tenant-runtime-v128.js')
  if (newName === oldName) throw new Error(`unexpected v127 layout name: ${oldName}`)
  ...
  if (serverTargets.length !== 1) throw new Error(`expected one v127 server shell, found ${serverTargets.length}`)
