...
function createStoreProfileService({ prisma, crypto }) {
  ...
  async function profile(session) {
    ...
    const [organization, user, staffCountRows, menuRows, inboundRows, storeRows] = await Promise.all([
      prisma.organization.findUnique({ where: { id: session.organizationId }, select: { id: true, name: true, slug: true, updatedAt: true } }),
      currentUser(session),
      prisma.$queryRawUnsafe('SELECT COUNT(*)::int AS count FROM "StaffBookingSetting" WHERE "organizationId"=$1', session.organizationId),
