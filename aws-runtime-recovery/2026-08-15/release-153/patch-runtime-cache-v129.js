...
const oldMarker = 'admin-header-settings-v128'
const marker = 'admin-header-settings-v129'
...
function updateHelperUrls(source) {
  return source
    .replaceAll('/commercial-admin-v128.js?v=20260815-128', '/commercial-admin-v129.js?v=20260815-129')
    .replaceAll('/tenant-setup-client.js?v=20260815-128', '/tenant-setup-client.js?v=20260815-129')
}
...
function patchRuntimeCache() {
  ...
  if (layouts.length !== 1) throw new Error(`expected one active v128 admin layout, found ${layouts.length}`)
  const oldFile = layouts[0]
  const oldName = path.basename(oldFile)
  let layoutSource = updateHelperUrls(fs.readFileSync(oldFile, 'utf8'))
  if (!layoutSource.includes('/commercial-admin-v129.js?v=20260815-129') || !layoutSource.includes('/tenant-setup-client.js?v=20260815-129')) throw new Error('v129 helper URLs missing')
  if (!layoutSource.includes(marker)) layoutSource += `\n/* ${marker} */\n`
  new Function(layoutSource)
  const newName = oldName.replace(/\.tenant-runtime-v128\.js$/, '.tenant-runtime-v129.js')
  if (newName === oldName) throw new Error(`unexpected v128 layout name: ${oldName}`)
